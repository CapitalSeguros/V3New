...
class Contenedor extends React.Component{
    ...
    removePerson = (e) => {
        ...
        this.state.personas_table.map((arr, i) => { //this.state._personas
        ...
        this.setState({
            personas_table: new_persons_array
        });
        ...
        for(var aa in personas){
            ...
            if(aa.toLowerCase() == texto.toLowerCase()){
                ...
                personas[aa].map((arr, i) => {
                        ...
                        obj_persona["area_ranking"] = texto;
    ...
    //-------------------
    handleChangeCheckboxIdPersona = (e) => {

        const idPersona = e.target.value;
        const check = e.target.checked;

        this.setState({personas_checked: {
            ...this.state.personas_checked,
            [idPersona]: check
        }});
    }
    //-------------------
    handleClickConfirmaPersonas = () => {

        const _personas_c = this.state.personas_seleccionado;
        const _personas_checked = this.state.personas_checked;
        //console.log(_personas_checked);

        //Validar existencia de la persona en la tabla
        if(this.state.personas_table.length > 0){

            this.state.personas_table.map((arr, i) => {
                
                for(var a in _personas_checked){

                    if(a == arr.idPersona){
                        alert("El usuario ya se encuentra seleccionado en la tabla");
                        delete _personas_checked[a];
                    }
                }
            });
        }
        ...
        _personas_c.map((arr, i) => {
            ...
            for(var a in _personas_checked){
                ...
                if(_personas_checked[a] && arr.idPersona.toLowerCase() == a.toLowerCase()){
                    //console.log(_personas_checked[a]);
                    this.setState(
                        estadoPrevio => ({
                            personas_table: [...estadoPrevio.personas_table, arr]
                        })
                    )
                }
            }
        ...
        $("#modalPersona input[type='checkbox']").prop('checked', false).change();
        $("#modalPersona").trigger("reset");
        $("#modalPersona").modal("hide");
    }
    //-------------------
    render(){
                        ...
                        <div className="card mt-3 mb-3">
                            ...
                            <div className="card-body">
                                ...
                                <select 
                                    ...
                                    //onChange={this.handleChangePersons.bind(this, "persona")}
                                >
                                    <option value="0">Seleccione</option>
                                    {   
                                ...
                                <div className="mt-2 text-center">
                                    <button 
                                        ...
                                        //onClick={this.anexaALista.bind(this, "persona")}
                                    >
                                        Buscar personas
                                    </button>
                                </div>
            ...
            <div class="modal fade bd-persona-modal-lg" id="modalPersona" tabindex="-1" role="dialog" aria-labelledby="LabelPersona" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                                ...
                                <tbody>
                                    {
                                        this.state.personas_seleccionado.map((arr, i) =>
                                                ...
                                                <tr>
                                                    <td className="text-center"><input key={i} type="checkbox" name={"personaSeleccionada_"+this.state.ranking_seleccionado.replace(" ","_").toLowerCase()+"[]"} id={"check_"+arr.idPersona} value={arr.idPersona} onChange={this.handleChangeCheckboxIdPersona.bind(this)}/></td>
                                                    <td>{arr.nombre}</td>
                                                    <td>{arr.correo}</td>
                                                </tr>
                                        )
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" onClick={this.handleClickConfirmaPersonas.bind()}>Seleccionar personas</button>
                    </div>
                    </div>
                </div>
            </div>
