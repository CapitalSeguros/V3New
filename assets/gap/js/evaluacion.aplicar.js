...
function showResult(pages, ob_respuesta) {
    //console.log('Resultpages',pages)
    //console.log('Resultob_respuesta',ob_respuesta)
        ...
        columns: [{
            ...
            render: function (data, type, row) {
                ...
                switch (data.type) {
                    ...
                    case "comment":
                        var comp_respuesta = ob_respuesta[data.name];
                        bdyText =  `<td class='info'>${comp_respuesta}</td>`;
                        bdyCol = "";
                        bodyTemplate += `<tbody>
                                        <tr class="tb-ev-header">
                                            <th class="tb-head">${data.name}</th>
                                        </tr>
                                        <tr>
                                        ${bdyText}
                                        </tr>
                    ...
                    default:
                        ...
                        bdyText = "";
                        bdyCol = "";
                        _.forEach(data.choices, function (v) {
                            bdyText += `<td class='${v==comp_respuesta?'info':'t'}'>${v}</td>`;
                        });
                       /*  console.log('bdyText', bdyText);
                        console.log("Cuando no es matrix", comp_respuesta);
                        console.log('data.rows', data.rows);
                        console.log("complete data", data);
                        console.log("datasource",dataSource) */
                        bodyTemplate += `<tbody>
                                        <tr class="tb-ev-header">
                                            <th class="tb-head" colspan="${data.choices.length}">${data.name}</th>
                                        </tr>
                                        <tr>
                                        ${bdyText}
                                        </tr>
                        ...
                        finalTemplate += `<table class="table table-condensed tb-table">${bodyTemplate}</table>`;
                        //finalTemplate=`${data.name}<br /> `+bdyText;
                        break;
                }
            ...
            api.column(0, {
                ...
                if (last !== group) {
                    if (group.title != undefined) {
                        $(rows).eq(i).before(
                            `<tr class="group"><td colspan="3"><strong>${group.title == undefined ? 'Sin grupo' : group.title}</strong><span class="pull-right"><strong>Grado requerido: ${group.grade}</strong></span></td></tr>`
                        );
                        last = group;
                    }
                }
