...
const URL_DEV = 'http://localhost/Capsys/www/V3/';
const URL_PROD = 'https://capsys.com.mx/V3/';

const URL_BASE = URL_DEV;
...
var datatable = jQuery(".nc-list-table").DataTable({
    ordering: false,
    ajax: {
        url: URL_BASE + `binconformidad/getNCList?getOnlyMyList=true&year=${date.getFullYear()}`,
        type: `GET`,
        dataSrc: ``
    },
    columns: [
        { data: "folio", orderable: false},
        { data: "idCBITipo", orderable: false},
        { data: "idCBIOpcion", orderable: false},
        { data: "idCBIArea", orderable: false},
        { data: "fechaRegistro", orderable: false},
        { 
            data: "bitacora", 
            orderable: false,
            render: function(data, type, row){
                return data.length > 0 ?  `<a class="show-all-comment" data-id="${row.idTablaNoConformidad}">${(data.substring(0, 10))}...</a>` : `Sin comentario`; //row.descripcion;
            }
        },
        {
            data: "status",
            ...
            render: function(data, type, row){
                ...
                var labelStatus = ``;
                switch(parseInt(row.aFavor)){
                    case 0:
                    case 1: 
                        labelStatus = `<button class="btn btn-${row.label} btn-xs text-white">${row.status}</button></span>`;
                        break;
                    case 3:
                        labelStatus = `
                            <div class="dropdown">
                                <button class="btn btn-${row.label} btn-xs text-white dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">${row.status}</button>
                                <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                                    <li role="presentation"><a role="menuitem" class="upload-more-files" data-root="resuelto" finally-nc="true" tabindex="-1" href="#" data-id="${row.id}">Confirmar de completado</a></li>
                                </ul>
                            </div>
                        `;
                        break;
        ...
        { 
            ...
            orderable: false,
            render: function(data, type, row){
                return data.length > 0 ? data.reduce((acc, curr) => acc += `<div class="mb-2">${curr.email}</div>`, ``) : `Sin responsable(s)`;
            }
        },
        {
            data: `id`,
            ...
            render: function(data, type, row){
                ...
                const root = [1].includes(parseInt(row.aFavor)) ? "resuelto" : "inconforme";
                return `
                    <div class="dropdown">
                        <button class="btn btn-link btn-sm dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></button>
                        <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                            <li role="presentation"><a role="menuitem" tabindex="-1" href="#" class="show-binnacle" data-id="${row.idTablaNoConformidad}">Ver bitácora</a></li>
                            <li role="presentation" class="divider"></li>
                            <li role="presentation"><a role="menuitem" tabindex="-1" href="#" data-root="${row.id}" class="show-nc-files">Ver archivos anexos</a></li>
                            <li role="presentation"><a role="menuitem" tabindex="-1" href="#" class="upload-more-files" data-root="${root}" finally-nc="false" data-id="${row.id}">Agregar archivos</a></li>
                        </ul>
                    </div>  
                `;
            }
...
jQuery(document).on("click", ".show-binnacle", function(e){
    ...
    const axios_ = axios.get(URL_BASE + `binconformidad/getBinnacle/${id}`).
    then((data) => {
...
jQuery("#filter-form").on("submit", function(e){
    ...
    e.preventDefault();
    const paramsGet = jQuery(this).serialize();
    datatable.ajax.url(jQuery(this).attr("action") + `?${paramsGet}`).load();
});
//-------------------------------
jQuery(document).on("click", ".show-all-comment", function(e){
    ...
    const axios_ = axios.get(URL_BASE + "binconformidad/getComment", { params: { id: id } }).
    then((response) => {
        
        const bodyModal = `
            <div class="panel panel-default">
                <div class="panel-body border">
                    <h5><span class="label label-primary">Comentario</span></h5><h5><span class="label label-primary">${response.data.fechaMovimiento}</span></h5>
                    <p class="text-justify">${response.data.movimiento}</p>
                </div>
            </div>
        `;
...
function updateNC(id){
    ...
    if(uppy2.getFiles().length == validFinally.length && validFinally.includes(true)){
        ...
        const axios_ = axios.post(URL_BASE + `binconformidad/updateNC/${id}`).
        then((data) => {
...
//-------------------------------
jQuery(".show-all-elements").on("click", function(e){

    datatable.ajax.url(URL_BASE + `binconformidad/getNCList?getOnlyMyList=true&year=${date.getFullYear()}`).load();
});
//------------------------------