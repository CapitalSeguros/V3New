    ...
    var ajax = $.ajax({
        ...
        success: (data) => {
            ...
            if(response.length > 0){
                ...
                cont4=1;
                cont5=1;
                var table1 = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==1 && curr.tipoPersona==1){
                        if(titulo1==""){
                            acc1 += `<div class="col-md-4"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                            </thead>
                            <tbody>
                        `;
                ...
                var table2 = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==2 && curr.tipoPersona==1){
                        if(titulo2==""){
                            acc2 += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                ...
                var table3 = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==3 && curr.tipoPersona==1){
                        if(titulo3==""){
                            acc3 += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}${curr.tipoPersona}</th>
                            </tr>
                            </thead>
                            <tbody>
                        `;
                ...
                var table4 = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==4 && curr.tipoPersona==1){
                        if(titulo4==""){
                            acc4 += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                ...
                var table5 = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==5 && curr.tipoPersona==1){
                        if(titulo5==""){
                            acc5+= `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                 ...
                 $(".contenedor_colab_nombres").html(`<div class="card"><div class="card-body"><div class="adjust"><h3>Colaboradores que tomaron la capacitación</h3><br>${table1} ${table2} ${table3} ${table4} ${table5}</div></div></div>`);
            }
    ...
    var ajax2 = $.ajax({
        ...
        success: (data) => {
            ...
            if(response.length > 0){
                ...
                cont4Agent=1;
                cont5Agent=1;
                var table1Agent = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==1 && curr.tipoPersona==3){
                        if(titulo1Agent==""){
                            acc1Agent += `<div class="col-md-4"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                        ...
                        acc1Agent += `
                        <tr>
                            <td>${(cont1Agent++)}</td><td>${curr.name}</td>
                        </tr>
                    `;
                    }
                     return acc1Agent;
                ...
                var table2Agent = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==2 && curr.tipoPersona==3){
                        if(titulo2Agent==""){
                            acc2Agent += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                        ...
                        acc2Agent += `
                        <tr>
                            <td>${(cont2Agent++)}</td><td>${curr.name}</td>
                        </tr>
                    `;
                    }
                    return acc2Agent;
                ...
                var table3Agent = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==3 && curr.tipoPersona==3){
                        if(titulo3Agent==""){
                            acc3Agent += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}${curr.tipoPersona}</th>
                            </tr>
                        ...
                        acc3Agent += `
                        <tr>
                            <td>${(cont3Agent++)}</td><td>${curr.name}</td>
                        </tr>
                    `;
                    }
                    return acc3Agent;
                ...
                var table4Agent = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacionAgent==4 && curr.tipoPersona==3){
                        if(titulo4Agent==""){
                            acc4Agent += `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                        ...
                        acc4Agent += `
                        <tr>
                            <td>${(cont4Agent++)}</td><td>${curr.name}</td>
                        </tr>
                    `;
                    }
                    return acc4Agent;
                ...
                var table5Agent = response.reduce((acc, curr, idx) => {
                    
                    if(curr.id_capacitacion==5 && curr.tipoPersona==3){
                        if(titulo5Agent==""){
                            acc5Agent+= `<div class="col-md-3"><table class="table">
                            <thead>
                            <tr>
                            <th>#</th><th>${curr.tipoCapacitacion}</th>
                            </tr>
                        ...
                        acc5Agent += `
                        <tr>
                            <td>${(cont5Agent++)}</td><td>${curr.name}</td>
                        </tr>
                    `;
                    }
                    return acc5Agent;
                   ...
                   if(table1Agent!=""){
                    table1Agent+="</tbody></table></div>";
                   }
                   if(table2Agent!=""){
                    table2Agent+="</tbody></table></div>";
                   }
                   if(table3Agent!=""){
                    table3Agent+="</tbody></table></div>";
                   }
                   if(table4Agent!=""){
                    table4Agent+="</tbody></table></div>";
                   }
                   if(table5Agent!=""){
                    table5Agent+="</tbody></table></div>";
                   }
                 $(".contenedor_colab_nombres").html(`<div class="card"><div class="card-body"><div class="adjust"><h3>Colaboradores que tomaron la capacitación</h3><br>${table1} ${table2} ${table3} ${table4} ${table5}</div></div></div>`);
                 $(".contenedor_agent").html(`<div class="card"><div class="card-body"><div class="adjust"><h3>Agentes que tomaron la capacitación</h3><br>${table1Agent} ${table2Agent} ${table3Agent} ${table4Agent} ${table5Agent}</div></div></div>`);
            }
...
$(document).on("click", ".apply-search", function(){
    ...
    const year = $("#year-for-report option:selected");
    const baseUrl = $("#base-url").data("url");
    ...
    const ajax = $.ajax({
        ...
        success: (data) => {
            ...
            if(response.length > 0){
                ...
                const tbody = response.reduce((acc, curr, idx) => {
                    ...
                    acc += `
                        <tr>
                            <td>${(idx + 1)}</td><td>${curr.name}</td>${td}<td>${total}</td>
                        </tr>
                    `;
                    return acc;
                }, ``);
                ...
                const totalPersons = response.length;
                ...
                $(".content-training-1").html(`
                    <div class="card card-body table-responsive">
                        ...
                        <p class="text-left">Total de participantes: ${totalPersons}</p>
                    </div>
                `);
            } else{
...
//-----------------------------------
$(document).on("click", ".apply-search-agent", function(){

    const training = $("#training-for-report-agent option:selected");
    const subTraining = $("#sub-training-for-report-agent option:selected");
    const month = $("#month-for-report-agent option:selected");
    const year = $("#year-for-report-agent option:selected");
    const baseUrl = $("#base-url").data("url");
    ...
    const ajax = $.ajax({
        ...
        success: (data) => {
            ...
            if(response.length > 0){
                ...
                const gmm = response.map(arr => arr.data.gmm).reduce((acc, curr) => acc += parseFloat(curr), 0);
                const fianzas = response.map(arr => arr.data.fianzas).reduce((acc, curr) => acc += parseFloat(curr), 0);
                const externos = response.map(arr => arr.data.externos).reduce((acc, curr) => acc += parseFloat(curr), 0);
                const totalPersons = response.length;

                console.log(totalPersons);

                $(".content-training-2").html(`
                    <div class="card card-body table-responsive">
                        <table class="table" id="report-agent">
                            <thead>
                                <tr>
                                    <th></th>
                                    <th>Agente / Colaborador</th>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-center">${gmm}</td>
                                    <td class="text-center">${fianzas}</td>
                                    <td class="text-center">${externos}</td>
                                    <td class="text-center">${(profesional + vida + danos + autos + gmm + fianzas+externos)}</td>
                                </tr>
                            </tfoot>
                        </table>
                        <p class="text-left">Total de participantes: ${totalPersons}</p>
                ...
                $(".content-training-2").html(`<h4>Sin resultados</h4>`).addClass("text-center");
            }
        }
    });
});
