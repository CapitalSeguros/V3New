...
$(document).on("click", ".get-vacation-request", function(){
    ...
    var ajax = $.ajax({
        ...
        success: (data) => {
            const response = JSON.parse(data);
                 response.forEach(d=>{
                    d.fecha_salida=d.fecha_salida.replace(/\//g,"-");
                    d.fecha=d.fecha.replace(/\//g,"-");
                    d.fecha_retorno=d.fecha_retorno.replace(/\//g,"-");
                 })
            ...
            if( response.length > 0 ){
                ...
                $(".request-list").html(`
                    <div role="tabpanel">
                        ...
                        <label style="color:red !important;"><b>*Si no puedes visualizar la solicitud, preciona click derecho sobre ver solicitud y luego selecciona abrir enlace en nueva ventana</b></label>
...
//---------------------------- // .for-export-aniversario
$(".export-xls-aniversario").on("click", function(e){
    console.log("Here");
    var downloadLink;
    var filename = "AniversarioVacaciones";
    var dataType = 'application/vnd.ms-excel';
    var tableSelect = document.getElementById("for-export-aniversario");
    var tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
...
//---------------------------- // .for-export-solicitud
$(".export-xls-solicitud").on("click", function(e){
    console.log("Here");
    var downloadLink;
    var filename = "SolicitudVacaciones";
    var dataType = 'application/vnd.ms-excel';
    var tableSelect = document.getElementById("for-export-solicitud");
    var tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
