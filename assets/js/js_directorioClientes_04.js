...
function consultarNotasDelCliente(elemento_html){
            ...
            <div id="notas_up" class="col-md-12">
                <div class="">
                    ...
                    <div class="collapse" id="asignar_notas">
                        <div class="form-group">
                            <label for="comentario_asigna" class="text-left">Agrega comentario para <span id="nombreCliente">`+nombreCliente+`</span></label>
                        ...
                        <div class="row">
                            ...
                            <div class="dropdown col-md-2 text-center">
                                ...
                                <ul class="dropdown-menu" role="menu" aria-labelledby="lista_agentes_" id="lista_agentes_list"></ul>

                            </div>
                            
                            <div class="col-md-9" id="contenedor_agentes_para_notas" style="border: 1px black solid; border-radius: 5px">
                                <form action="" id="formulario_agentes"></form>
                            </div>

                            <button class="btn btn-link" id="boton_asigna_notas" onclick="agregaNotaCliente(`+idCliente+`,1)"><i class="fa fa-upload" aria-hidden="true"></i> Crear nota</button>
                        </div>
                    </div>
                    <!--Fin de crear notas-->
    ...
    xmlhhtp.onreadystatechange=function(){
        if(this.readyState==4 && this.status==200){
            ...
            //--------------------------------------------------------------------
            for(var i in res.personas){ //Imprime en el dropdown solo los vendedoes.
                if(res.personas[i].tipoPersona=="Vendedor"){
                    for(var b in res.personas[i].Data){

                        li_agentes.innerHTML+=`
                        
                            <li role="presentation"><a href="javascript:void(0)" onclick="anexaALista(this)" id_persona="`+res.personas[i].Data[b].idPersona+`">`+res.personas[i].Data[b].nombres+` (`+res.personas[i].Data[b].email+`)</a></li>
                        `;
            ...
            if(res.mensaje!=""){
                ...
                for(var a in res.datos){
                    ...
                    if(res.datos[a].comentario.length>10){
                        cadena_trunca+=(res.datos[a].comentario).substring(0, 10)+"...";
                    } else {
                        cadena_trunca+=res.datos[a].comentario;
                    }

                    notas_consultadas+=`
                        <div class="btn-group" style="display:inline-block" id="dropdown_`+res.datos[a].idNota+`">
                ...
                contenedor_notas.innerHTML+=notas_consultadas+`<div class="collapse" id="comentarioNota_"><br>
...
//-------------------------------------
function removerNotaDelCliente(idNota){

    var drop_hijo=document.getElementById("dropdown_"+idNota);
    var padre=drop_hijo.parentNode;

    //console.log(padre);
    padre.removeChild(drop_hijo);
...
function actualizarNotaDelCliente(idNota, array_agentes,comentario){
    ...
    document.getElementById("boton_asigna_notas").innerHTML=`<i class="fa fa-upload" aria-hidden="true"></i>&nbspActualiza nota`;
