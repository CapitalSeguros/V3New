...
import { ShowLoading, FocusInput, LockEnter, colourStyles, colourStylesCustomRecibos, FComisiones, CComisiones, FComisionesP, mapitems, displayitem, UpperCaseField, round, Comisiones, ComisionesRecibos, truncate } from '../../Helpers/FGeneral.js';
...
import ModalPagoHon from '../Acciones/ModalPagoHon.jsx';
...
export default function Recibos(props) {
    ...
    const formikModalComisionesObj = useRef(null);
    const [IsChange, SetIsChange] = useState(false);
    const [TComisines, SetTComisones] = useState(0);
    const ModalLiquidarRef = useRef(null);
    const [TipoMovComision, SetTipoMovComision] = useState('');
    const [ItemMovComision, SetItemMovComision] = useState({
        FDocumento: '',
        Documento: '',
        Folio: ''
    });

    const [PagoHon, setPagoHon] = useState({
        Documento: '',
        FDocumento: '',
        Folio: '',
        Id: 0
    ...
    async function InitialData() {
        ...
        if (res.status != 200) {
            ...
            setTimeout(() => {
                ...
                let Recibo = res.success.Datos.ItemInfoRecibo;
                let SubTotal = parseFloat(Recibo.SubTotal == null ? 0 : Recibo.SubTotal);
                if (SubTotal == 0) {
                    switch (Modulo) {
                        case "P":
                            SubTotal = (parseFloat(Recibo.PrimaNeta) + parseFloat(Recibo.Recargos) + parseFloat(Recibo.Derechos)) - parseFloat(Recibo.Descuento);
                            break;
                        case "F":
                            SubTotal = (parseFloat(Recibo.PrimaNeta) + parseFloat(Recibo.GastosMaq) + parseFloat(Recibo.GastosAdm) + parseFloat(Recibo.Derechos)) - parseFloat(Recibo.Descuento);
                            break;
                        case "E":
                            if (_moduloOrigen == "P") {
                                SubTotal = (parseFloat(Recibo.PrimaNeta) + parseFloat(Recibo.Recargos) + parseFloat(Recibo.Derechos)) - parseFloat(Recibo.Descuento);
                ...
                //let Copy=res.success.Datos.ItemInfoRecibo;
                ...
                ConvertImporte(res.success.Datos.Recibo, res.success.Datos.Comisiones, "C", res.success.Datos.Submodulo);
                ConvertImporte(res.success.Datos.Recibo, res.success.Datos.Honorarios, "H", res.success.Datos.Submodulo)
                //setHonorarios();
    ...
    async function SaveData(values) {
        ...
        if (values.Id > 0) {
            ...
            values.FechaAplica = moment().format('YYYY-MM-DD HH:mm:ss');
        }
    ...
    function ConvertirImporte(value) {
        //console.log(`Pago1 ${truncate(parseFloat(value), 2)} | TC1 w ${parseFloat(pagoO.TipoCambio2)} |TC1 ${truncate(parseFloat(pagoO.TipoCambio2), 4)} | Valor enviado ${truncate(truncate(parseFloat(value), 2) * truncate(parseFloat(pagoO.TipoCambio2),2), 4)}`)
        //console.log(`Pago ${truncate(parseFloat(value), 2)} | TC w ${parseFloat(pagoO.TipoCambio2)} |TC ${truncate(parseFloat(pagoO.TipoCambio2), 4)} | Valor enviado ${truncate(truncate(parseFloat(value), 2) * truncate(parseFloat(pagoO.TipoCambio2),2), 4)}`)
        return (value) * parseFloat(pagoO.TipoCambio2);
        //return truncate(truncate(parseFloat(value), 2) * parseFloat(pagoO.TipoCambio2), 4);
        //return round(parseFloat(value), 4) * round(parseFloat(pagoO.TipoCambio2), 4);
    ...
    function ConvertImporte(recibo, lista, tipo, submodulo = null) {
        let _lista = [];
            ...
            lista.forEach(x => {
                if (Modulo == 'P') {
                    ...
                    if (find) {
                        ...
                        if (find.Id === 2) {
                            x.Base = recibo.ComN + recibo.ComR;
                            //console.log("2", `${recibo.ComN} | ${recibo.ComR}`);
                            x.ImporteCal = (x.Base * x.Porcentaje) / 100 ? (x.Base * x.Porcentaje) / 100 : 0;
                        }
            ...
            lista.forEach(x => {
                if (Modulo == 'P') {
                    ...
                    if (x.Formula === 1) {
                        ...
                        //console.log(`Gastos Maquila ${recibo.GastosMaq}| ${x.Participacion} | ${ recibo.GtosMaq}| ${JSON.stringify(x)}`)
    ...
    function returnCantidadHon(Tipo, Porcentaje) {
        ...
        switch (parseInt(Tipo)) {
            ...
            case 2:
                //console.log('entro');
                var Val1 = values.ComN ? values.ComN : 0;
                var Val2 = values.ComR ? values.ComR : 0;
            ...
            case 3:
                if (Modulo == "F") {
                    ...
                    var Val1 = values.ComD ? values.ComD : 0;
                    var Val2 = values.ComR ? values.ComR : 0;
                    rReturn = ((Val1 + Val2) / 100) * (Porcentaje ? Porcentaje : 0);
                }
                break;
    ...
    async function SaveHonObj(Tipo) {
        ...
        var Lista = Tipo == 1 ? { ...comisiones } : { ...honorarios };
    ...
    async function clicObjeto(item, tipo) {
        if (tipo == 1) {
            ...
            obj.Registros = await getPagoComIndividual(item.Id, obj.TipoComision);
            setObjCom(obj);
            //CalculateObjCom();
        } else {
            var obj = { ...InitialModCom };
    ...
    async function getPagoComIndividual(Idr, Tipo) {
        var URL = `${UrlServicio}conciliacion/getpagocomindividual`;
        var RetunDta = [];
        var dta = { Id: Idr, IdRecibo: Id, Serie: recibo.Serie, Tipo: Tipo };
        const res = await CallApiGet(URL, dta, null);
        if (res.status != 200) {
            toastr.error(`Error ${res.error.Mensaje}`);
        } else {
    ...
    function NewReloadPrices(field, value, values) {
        ...
        setTimeout(() => {
            TotalComisiones(formikRef.current.values);
        }, 100);
    }
    ...
    async function CancelEdition() {
        ...
        if (res.status != 200) {
            ...
            console.log("Success", res.success.Datos.Recibo);

            let _recibo = res.success.Datos.Recibo;

            formikRef.current.setFieldValue("PrimaNeta", _recibo.PrimaNeta || 0);
            formikRef.current.setFieldValue("Descuento", _recibo.Descuento || 0);
            formikRef.current.setFieldValue("PDescuento", _recibo.PDescuento || 0);
            formikRef.current.setFieldValue("Recargos", _recibo.Recargos || 0);
            formikRef.current.setFieldValue("PorRecargos", _recibo.PorRecargos || 0);
            formikRef.current.setFieldValue("GastosMaq", _recibo.GastosMaq || 0);
            formikRef.current.setFieldValue("GastosAdm", _recibo.GastosAdm || 0);
            formikRef.current.setFieldValue("Derechos", _recibo.Derechos || 0);
            formikRef.current.setFieldValue("Subtotal", _recibo.Subtotal || 0);
            formikRef.current.setFieldValue("IVA", _recibo.IVA || 0);
            formikRef.current.setFieldValue("PorIVA", _recibo.PorIVA || 0);
            formikRef.current.setFieldValue("Ajuste", _recibo.Ajuste || 0);
            formikRef.current.setFieldValue("PrimaTotal", _recibo.PrimaTotal || 0);


            formikRef.current.setFieldValue("PComN", _recibo.PComN || 0);
            formikRef.current.setFieldValue("PComR", _recibo.PComR || 0);
            formikRef.current.setFieldValue("PComD", _recibo.PComD || 0);
            formikRef.current.setFieldValue("CGastosMaq", _recibo.CGastosMaq || 0);
            formikRef.current.setFieldValue("CGastosAdm", _recibo.CGastosAdm || 0);
            formikRef.current.setFieldValue("PEspecial", _recibo.PEspecial || 0);

            formikRef.current.setFieldValue("ComN", _recibo.ComN || 0);
            formikRef.current.setFieldValue("ComR", _recibo.ComR || 0);
            formikRef.current.setFieldValue("ComD", _recibo.ComD || 0);
            formikRef.current.setFieldValue("PCGastosMaq", _recibo.PCGastosMaq || 0);
            formikRef.current.setFieldValue("PCGastosAdm", _recibo.PCGastosAdm || 0);
            formikRef.current.setFieldValue("Especial", _recibo.Especial || 0);
    ...
    function OpenModalComision(Tipo) {
        if (pago.Id == "") {
            return toastr.error(`Se necesita que el pago este aplicado.`);
        }
        SetTipoMovComision(Tipo);
        var Item = null;
        if (Tipo == "HON") {
            Item = {
                Documento: ObjHon.Registros.length > 0 ? ObjHon.Registros[0].CDocumento : '',
                FDocumento: ObjHon.Registros.length > 0 ? ObjHon.Registros[0].FDocumento : '',
                Folio: ObjHon.Registros.length > 0 ? ObjHon.Registros[0].CFolio : '',
                IdRegistro: ObjHon.Registros.length > 0 ? ObjHon.Registros[0].Id : ''
            }
        } else {
            Item = {
                Documento: ObjCom.Registros.length > 0 ? ObjCom.Registros[0].CFolio : '',
                FDocumento: ObjCom.Registros.length > 0 ? ObjCom.Registros[0].FDocumento : '',
                Folio: ObjCom.Registros.length > 0 ? ObjCom.Registros[0].CFolio : '',
                IdRegistro: ObjCom.Registros.length > 0 ? ObjCom.Registros[0].Id : ''
            }
        }
        SetItemMovComision(Item);
        formikModalComisionesObj.current.Initial(Item);
        $('#ModalPagoHon').modal({ backdrop: 'static', keyboard: true, show: true })
    }

    function PagoHonfunction(values) {
        var ObjSend = {};
        var RefId = TipoMovComision == "HON" ? ObjHon.Id : ObjCom.Id;
        var URL = `${UrlServicio}conciliacion/crearAccionConciliacion`;
        var RetunDta = null;
        var IdRegistro = 0;
        if (TipoMovComision == "HON") {
            var Hon = honorarios.find(x => x.Id == ObjHon.Id);
            ...
            ObjSend = {
                ...
                FDesde: recibo.FDesde,
                Serie: recibo.Serie,
                TipoComision: Hon.Formula,
                //TipoComision: '%',
                TipoValor: Hon.TipoValor,
                Participacion: Hon.Porcentaje,
                PrimaNeta: recibo.PrimaNeta,
                //Importe: ImporteHonorarios(Hon, "honorario"),
                Importe: Hon.ImporteCal,
                MonedaDoc: pago.DMonedaDcto,
                IDMon: pago.IdMonedaDcto,
                TCDocumento: pago.TipoCambio2,
                TCPago: pago.TipoCambio1,
                ...
                IDMonPago: pago.IdMonedaPago,
                MonedaPago: pago.DMonedaPago,
                IDVend: Hon.Id_vendedor,
                VendNom: Hon.NombreCompleto,
                Tipo: Hon.Id_formula,
                //ImporteConvertido: ConvertirImporte(ImporteHonorarios(Hon, "honorario"))
                ImporteConvertido: (ConvertirImporte(Hon.ImporteCal)),
                IdRef: Hon.Id
            }
        } else {
            //console.log("COmisiones", comisiones);
            var Com = comisiones.find(x => x.Id == ObjCom.Id);
            //console.log("FindCom", Com);
            ...
            var ObjSend = {
                ...
                FDesde: recibo.FDesde,
                Serie: recibo.Serie,
                TipoComision: Com.TipoC,
                //TipoComision: Com.TipoComision,
                TipoValor: '%',
                Participacion: Com.Participacion,
                PrimaNeta: recibo.PrimaNeta,
                Aplicado: pago.ImporteReal,
                //Importe: ImporteHonorarios(Com, "comision"),
                Importe: Com.ImporteCal,
                MonedaDoc: pago.DMonedaDcto,
                Monedapago: pago.DMonedaPago,
                IDMonDoc: pago.IdMonedaDcto,
                IDMonPago: pago.IdMonedaPago,
                TipoAgente: Com.TipoAgente,
                TCDocumento: pago.TipoCambio2,
                TCPago: pago.TipoCambio1,
                ImportePago: pago.ImporteReal ? pago.ImporteReal : 0,
                Cliente: ItemInfoRecibo.Cliente,
                ...
                FAplicado: moment().format('YYYY-MM-DD'),
                IDVend: 0,
                VendNom: Com.AgenteNombre,
                Tipo: Com.Formula,
                //ImporteConvertido: ConvertirImporte(ImporteHonorarios(Com, "comision"))
                ImporteConvertido: ConvertirImporte(Com.ImporteCal),
                IdRef: Com.Id
            }
        }
        var Envio = {
            ...
            FDocumento: values.FDocumento,
            Tipo: TipoMovComision,
            IdRegistro: values.IdRegistro
        }
        swal({
            title: "¿Está seguro de que quiere aplicar la accion?",
            text: "Se registrará la accion realizada",
            icon: "warning",
            buttons: ["Cancelar", "Aceptar"],
        }).then(async (value) => {
            if (value) {
                const res = await CallApiPost(URL, Envio, null);
                if (res.status != 200) {
                    toastr.error(`Error ${res.error.Mensaje}`);
                } else {
                    $("#ModalPagoHon").modal('hide');
                    RetunDta = res.success.Datos;
                    TipoMovComision == "HON" ? setObjHon(InitialModCom) : setObjCom(InitialModCom);//Reiniciar el item seleccionado
                    await ReloadPageInfo();
                }
                return RetunDta;
            }
    ...
    function EleminarObjPagoCon(Tipo) {
        //eliminarobjPagoConciliacion
        if (pago.Id == "") {
            return toastr.error(`Se necesita que el pago este aplicado.`);
        }
        var URL = `${UrlServicio}conciliacion/eliminarobjPagoConciliacion`;
        var IdRegistro = Tipo == "HON" ? (ObjHon.Registros.length > 0 ? ObjHon.Registros[0].Id : '') : ObjCom.Registros.length > 0 ? ObjCom.Registros[0].Id : '';
        var Envio = {
            Id: IdRegistro,
            Tipo: Tipo
        }
    ...
    //funcion que pasa pago directo a conciliacion
    async function SendConciliacion(Tipo, IdItem) {

        var Obj = null;
        var lsObjeto = [];
        if (Tipo == "C") {//Comisiones
            Obj = comisiones.find(x => x.Id == IdItem);
            if (Obj != null) {
                var comision = {
                    Id: 0,
                    IdRecibo: Id,
                    ...
                    FDesde: recibo.FDesde,
                    Serie: recibo.Serie,
                    TipoComision: Obj.TipoC,
                    //TipoComision: Obj.TipoComision,
                    TipoValor: '%',
                    Participacion: Obj.Participacion,
                    PrimaNeta: recibo.PrimaNeta,
                    Aplicado: pago.ImporteReal,
                    //Importe: ImporteHonorarios(Obj, "comision"),
                    Importe: Obj.ImporteCal,
                    MonedaDoc: pago.DMonedaDcto,
                    Monedapago: pago.DMonedaPago,
                    IDMonDoc: pago.IdMonedaDcto,
                    IDMonPago: pago.IdMonedaPago,
                    TipoAgente: Obj.TipoAgente,
                    TCDocumento: pago.TipoCambio2,
                    TCPago: pago.TipoCambio1,
                    ImportePago: pago.ImporteReal ? pago.ImporteReal : 0,
                    Cliente: ItemInfoRecibo.Cliente,
                    ...
                    FAplicado: moment().format('YYYY-MM-DD'),
                    IDVend: 0,
                    VendNom: Obj.AgenteNombre,
                    Tipo: Obj.Formula,
                    //ImporteConvertido: ConvertirImporte(ImporteHonorarios(Obj, "comision"))
                    ImporteConvertido: ConvertirImporte(Obj.ImporteCal),
                    IdRef: Obj.Id
                }

                lsObjeto.push(comision);
            }

        } else {//Honorarios
            Obj = honorarios.find(x => x.Id == IdItem);
            if (Obj != null) {
                var honorario = {
                    Id: 0,
                    IdRecibo: Id,
                    ...
                    FDesde: recibo.FDesde,
                    Serie: recibo.Serie,
                    TipoComision: Obj.Formula,
                    //TipoComision: '%',
                    TipoValor: Obj.TipoValor,
                    Participacion: Obj.Porcentaje,
                    PrimaNeta: recibo.PrimaNeta,
                    //Importe: ImporteHonorarios(Obj, "honorario"),
                    Importe: Obj.ImporteCal,
                    MonedaDoc: pago.DMonedaDcto,
                    IDMon: pago.IdMonedaDcto,
                    TCDocumento: pago.TipoCambio2,
                    ...
                    IDMonPago: pago.IdMonedaPago,
                    MonedaPago: pago.DMonedaPago,
                    IDVend: Obj.Id_vendedor,
                    VendNom: Obj.NombreCompleto,
                    Tipo: Obj.Id_formula,
                    //ImporteConvertido: ConvertirImporte(ImporteHonorarios(Obj, "honorario"))
                    ImporteConvertido: (ConvertirImporte(Obj.ImporteCal)),
                    IdRef: Obj.Id
                }

                lsObjeto.push(honorario);
            }
        }
        ...
        swal({
            ...
            if (value) {
                ShowLoading(true);
                const res = await CallApiPost(`${UrlServicio}conciliacion/pasarConciliacion`, dta, null);
                if (res.status != 200) {
                    swal({
                        title: "Advertencia",
    ...
    return (
        <>
                                                        ...
                                                        <div className='col-md-12 text-right'>
                                                            {
                                                                isEditar ?
                                                                    <>
                                                                        <a className='btn btn-primary btn-s'
                                                                            disabled={recibo.Status_TXT != 'Pendiente' ? true : false}
                                                                            data-toggle="tooltip" data-placement="bottom" title="Aplicar pago"
                                                                            onClick={() => { SaveRecibos(), setIsEditar(!isEditar) }}>
                                                                            <i className="fa fa-credit-card" aria-hidden="true"></i>&nbsp;Guardar recibo
                                                                        </a>
                                                                        <a className="btn btn-primary btn-s"
                                                                            disabled={recibo.Status_TXT != 'Pendiente' ? true : false}
                                                                            data-toggle="tooltip" data-placement="bottom" title="Cancelar"
                                                                            onClick={() => { CancelEdition(), setIsEditar(!isEditar) }}>
                                                                            <i className="fa fa-close" aria-hidden="true"></i>&nbsp;Cancelar
                                                                        </a>
                                                                    </>
                                                                    :
                                                                    <a className="btn btn-primary btn-s"
                                                            ...
                                                            <a className='btn btn-primary btn-s' disabled={recibo.Status_TXT == 'Cancelado' ? true : false} data-toggle="tooltip" data-placement="bottom" title="Aplicar pago" onClick={() => RecalcularRecibo()}><i className="fa fa-refresh" aria-hidden="true"></i>&nbsp;Recalcular recibo</a>
                                                            <a className="btn btn-primary btn-s" onClick={() => { window.location = UrlPagina + `servicioSistema/${'recibos/' + (recibo.IDDocto ? recibo.IDDocto : 0) + '/' + Modulo}`; }} data-toggle="tooltip" data-placement="bottom" title="Regresar" ><i className="fa fa-reply" aria-hidden="true"></i>&nbsp;Regresar</a>
                                                        </div>
                                                    </div>
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    {comisiones && comisiones.map((item, key) => (
                                                                        <tr key={key} style={item.Id == ObjCom.Id ? { cursor: 'pointer', backgroundColor: '#6f42c1', color: 'white' } : { cursor: 'pointer' }} onClick={() => clicObjeto(item, 1)}>
                                                                            ...
                                                                            <td>{item.TipoC}</td>
                                                                            <td>{item.Participacion ? item.Participacion : 0}</td>
                                                                    ...
                                                                    <div className='col-md-12'>
                                                                        ...
                                                                        <div className='row'>
                                                                            <div className='col-md-12 text-right pb-4'>
                                                                                <a className="btn btn-primary btn-s" disabled={ObjCom.Id > 0 && pago.Id != '' ? false : true} data-toggle="tooltip" data-placement="bottom" title="Aplicar Directo" onClick={() => { OpenModalComision("COM") }}><i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                                <a className="btn btn-primary btn-s" disabled={(ObjCom.Id > 0 && pago.Id != '' && ObjCom.Registros.length == 0) ? false : true} data-toggle="tooltip" data-placement="bottom" title="Aplicar a conciliación" onClick={() => { SendConciliacion("C", ObjCom.Id) }}><i className="fa fa-arrow-right" aria-hidden="true"></i></a>
                                                                                <a className="btn btn-primary btn-s" disabled={ObjCom.Id > 0 && pago.Id != '' ? false : true} data-toggle="tooltip" data-placement="bottom" title="Eliminar" onClick={() => { EleminarObjPagoCon("COM") }}><i className="fa fa-times-circle-o" aria-hidden="true"></i></a>
                                                                            </div>
                                                                        </div>
                                                                        <div className='row'>
                                                                                ...
                                                                                <tbody style={{ maxHeight: '400px', overflow: 'auto' }}>
                                                                                    ...
                                                                                    {ObjCom.Registros && ObjCom.Registros.map((item, key) => (
                                                                                        <tr key={key} >
                                                                                            ...
                                                                                            <td>{item.FDocumento ? moment(item.FDocumento).format("DD/MM/YYYY") : 'N/A'}</td>
                                                            ...
                                                            <div className='row'>
                                                                ...
                                                                <div className='col-md-6'>
                                                                    <div className='col-md-12'>
                                                                        ...
                                                                        <div className='col-md-12 text-right'>
                                                                            <a className="btn btn-primary btn-s" disabled={ObjHon.Id > 0 && pago.Id != '' ? false : true} data-toggle="tooltip" data-placement="bottom" title="Editar" onClick={() => { OpenModalComision("HON") }}><i className="fa fa-edit" aria-hidden="true"></i></a>
                                                                            <a className="btn btn-primary btn-s" disabled={ObjHon.Id > 0 && pago.Id != '' && ObjHon.Registros.length > 0 ? true : false} data-toggle="tooltip" data-placement="bottom" title="Aplicar a conciliación" onClick={() => { SendConciliacion("H", ObjHon.Id) }}><i className="fa fa-arrow-right" aria-hidden="true"></i></a>
                                                                            <a className="btn btn-primary btn-s" disabled={ObjCom.Id > 0 && pago.Id != '' ? false : true} data-toggle="tooltip" data-placement="bottom" title="Eliminar" onClick={() => { EleminarObjPagoCon("HON") }}><i className="fa fa-times-circle-o" aria-hidden="true"></i></a>
                                                                            {/* <a className="btn btn-primary btn-s" disabled={ObjHon.Id > 0 ? false : true} data-toggle="tooltip" data-placement="bottom" title="Aplicar" onClick={() => $('#ModalPagoHon').modal({ backdrop: 'static', keyboard: true, show: true })}><i className="fa fa-check" aria-hidden="true"></i></a> */}
                                                                        </div>
                                                                        <div className='row'>
                                                                            <div className='col-md-12'>
                                                                                <div className='form-group'>
            ...
            <ModalPagoHon ref={formikModalComisionesObj} PagoHonfunction={PagoHonfunction} Tipo={TipoMovComision} Item={ItemMovComision} />
        </>
    )
}
