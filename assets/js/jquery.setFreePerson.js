$(function(){

    console.log("set me free");
    
    if($("#setfree").length > 0){
        //$('#tipoPersona').change();
        var base_url = $("#base_url").data("base-url");
        var freePerson = $("#setfree").val();
        
        $(`#idPersonas option[value=${freePerson}]`).prop("selected", true);
        $(`#alert`).modal({
            keyboard: false,
            backdrop: "",
            show: true
        });
        axios.get(`${base_url}persona/getPersonalData`, {
            params: {
                id: freePerson
            }
        }).then(function(data){
            console.log(data);

            var data_ = data.data.datosAgente;
            var typePerson = data_.tipoPersona == 3 ? `AGENTE` : "COLABORADOR";
            var isEmploye = data_.tipoPersona == 1 ? `<p>De igual manera, seleccionar un puesto y área de trabajo para el nuevo colaborador</p>` : "";
            for(var a in data_){

                data_.idColaboradorArea = 1;
                if(a == "emailUsuario"){

                    $("#divUsuarioPersona").html(`<input class="formEnviar" style="width: 300px" type="text" id="usuarioPersona" name="usuarioPersona" value="${data_.emailUsuario}" disabled="">`);
                } else if(a == "passwordUsuario"){

                    $("#divPasswordPersona").html(`<input class="formEnviar" style="width: 300px" type="text" id="usuarioPassword" name="usuarioPassword" value="${data_.passwordUsuario}" disabled="">`);
                } else if(a == "esAgenteNuevo"){

                    $(`#${a}`).html(`<option value="0">PERMANENTE</option>`);
                } else if(a == "IDVend"){

                    $(`#divIdVendedor`).html(`<input class="formEnviar" style="width: 300px" type="text" id="IDVend" name="IDVend" value="${data_.IDVend}">`)
                } /*else if(a == "idPersonaPuesto"){

                    $(`#${a}`).val(98).prop("selected", true);
                }*/ else if(a == "tipoPersona"){

                    $(`#tipoPersona`).html(`<option value="${data_.tipoPersona}">${typePerson}</option>`);
                } else if(a == "arrayTypeAgent"){

                    const option = data_[a].reduce((acc, curr) => acc += `<option value="${curr.idPersonaTipoAgente}">${curr.personaTipoAgente}</option>` , ``);
                    $(`#personaTipoAgente`).html(`<option value="0">Seleccione papu</option>${option}`);
                    $("#personaTipoAgente").val(data_.personaTipoAgente);
                } else if(a == "arrayTypeChannel"){

                    const option = data_[a].reduce((acc, curr) => acc += `<option value="${curr.IdCanal}">${curr.nombreTitulo}</option>` , ``);
                    $(`#id_catalog_canales`).html(`<option value="0">Seleccione</option>${option}`);
                    $("#id_catalog_canales").val(data_.id_catalog_canales);
                } else if(a == "arrayTypeJobs"){

                    const option = data_[a].reduce((acc, curr) => acc += `<option value="${curr.idPuesto}">${curr.personaPuesto}</option>` , ``);
                    $(`#idPersonaPuesto`).html(`<option value="0">Seleccione</option>${option}`);
                }

                $(`#${a}`).val(data_[a]);
            }

            if(data.data.availibleAccounts.length > 0){
                $(".count-accounts").html(data.data.availibleAccounts.length);
                $(".avalibe-accounts-list").html(
                    data.data.availibleAccounts.reduce((acc, curr) => acc += `<li role="presentation"><a class="get-account" role="menuitem" tabindex="-1" href="javascript: void(0)">${curr.email}</a></li>`, ``)
                );
            }

            if(parseInt(data.data.puestoColaborador.idPuesto) > 0){
                $("#idPersonaPuesto").append($("<option>", {
                    value: data.data.puestoColaborador.idPuesto,
                    text: `${data.data.puestoColaborador.personaPuesto} (Para asignar)`,
                }).attr(`data-puesto`, 1).prop("selected", true));
                console.log("si hay puesto");
            }

            var modalContent = `
                <h4 class="text-dark">Convertir persona temporal a fija</h4>
                <div class="col-md-12 text-dark mt-4">
                    <p><b>Datos del usuario</b>: pre-cargado</p>
                    <p><b>Nombre</b>: ${data_.nombres} ${data_.apellidoPaterno} ${data_.apellidoMaterno}</p>
                    <p><b>Tipo de persona</b>: ${typePerson}</p>
                    <div class="dropdown-divider"></div>
                    <div class="mt-4">
                        <p><b>Procedimiento:</b></p>
                        <p>Para convertir al usuario que ha cumplido de manera exitosa con su inducción, es necesario cambiar su usuario y contraseña de acceso al 
                            V3 de temporal a una fija para que el usuario pueda tener acceso a las herramientas de trabajo.    
                        </p>
                        ${isEmploye}
                        <p>Guardar cambios dando clic al boton de guardar.</p>
                    </div>
                </div>
            `;

            $(`.body-alert`).html(modalContent);
            showOptionsPerson();
            //$('#tipoPersona').change();

        }).catch(function(error){
            console.log(error);
        });
    }

    //--------------------------

    if($("#permit").length > 0){
        $('#tipoPersona').change();
        var base_url = $("#base_url").data("base-url");
        var permitPerson = $("#permit").val();

        $(`#alert`).modal({
            keyboard: false,
            backdrop: "",
            show: true
        });
        axios.get(`${base_url}persona/getTemporalPersonData`, {
            params: {
                id: permitPerson
            }
        }).then(function(data){
    
            //console.log(data.data);
            var onlyObject = data.data[0];
            delete onlyObject.idPersona;
            
            nuevoAgente();
            $("#idPersonaPuesto").html("");

            for(var a in onlyObject){

                if(a == "fecAltaSistemPersona"){

                    var date_ = onlyObject[a].split(" ");
                    var onlyDate = date_[0].split("-");
                    $(`#${a}`).val(`${onlyDate[2]}/${onlyDate[1]}/${onlyDate[0]}`);
                } else{
                    $(`#${a}`).val(onlyObject[a]);
                }
            }

            $("#idPersonaPuesto").html(`<option value="${onlyObject.idPuesto}">${onlyObject.personaPuesto}</option>`);
            var typePerson = onlyObject.tipoPersona == 1 ? "COLABORADOR" : "AGENTE";
            //var job = $(`#idPersonaPuesto option:selected`).text();
            var area = $(`#idColaboradorArea option:selected`).text();
            var message = `
                <h4 class="text-dark">Solicitud de alta de usuario</h4>
                <div class="col-md-12 text-dark mt-4">
                    <p><b>Datos del usuario</b>: pre-cargado</p>
                    <p><b>Nombre</b>: ${onlyObject.nombres} ${onlyObject.apellidoPaterno} ${onlyObject.apellidoMaterno}</p>
                    <p><b>Tipo de persona</b>: ${typePerson}</p>
                    ${(onlyObject.tipoPersona == 1 ? `<p><b>Puesto a cubrir</b>: ${onlyObject.personaPuesto}</p>` : ``)}
                    <p><b>Área</b>: ${area.toUpperCase()}</p>
                    <div class="dropdown-divider"></div>
                    <div class="mt-4">
                        <p><b>Detalles:</b></p>
                        <p>
                            La cuenta de correo <b>${onlyObject.creator}</b> esta solicitando la alta de este usuario tipo ${typePerson}, ya que no cuenta con los permisos necesarios para darlo de alta en el sistema.
                        </p>
                        <p>Recuerde <b>Guardar</b> cambios dando clic al boton de guardar.</p>
                    </div>
                </div>
            `; 

            $(`.body-alert`).html(message);
        }).catch(function(error){
            console.log(error);
        })
    }

    //-------------------------
    //var countDocs = $("#validateDocuments").data("totalcount");
    //var countDocsUploaded = $("#validateDocuments").data("progressdocuments");
    //var isProspective = $("#isProspective").data("prospective");
    //var listMail = ["SISTEMAS@ASESORESCAPITAL.COM", "COORDINADORCOMERCIAL@FIANZASCAPITAL.COM", "CAPITALHUMANO@AGENTECAPITAL.COM"];
    //var typeProgress = $("#typeProgress").val();
    //var mail = $("#usuario_").val();
    //var validate = $("#showInducctionModal").val();
	var showModal = $("#pass-to-inducction").val();
    if(showModal){ // //isProspective == 0 && countDocsUploaded == countDocs && validate && typeProgress == "documento"
		console.log("CROSS_TO_INDUCCTION");
        var typePerson = $("#tipoPersona option:selected").val() == 1 ? "COLABORADOR" : "AGENTE";
        var class_ = ``;//`crossToSystemUser`;
        var html_ = ``;//`Escalar ${typePerson.toLowerCase()} a sistemas`;
        var mostrar = false;

        if($("#tipoPersona option:selected").val() == 1){ //sistemas - colaborador //listMail.includes(mail) && 
            var class_ = `crossToInducction`;
            var html_ = `Pasar ${typePerson.toLowerCase()} a inducción`;
            mostrar = true;
        }

        if(mostrar){
            $(`#alert`).modal({
                keyboard: false,
                backdrop: "",
                show: true
            });
        }

        var name = $("#labelNombrePersona").text();
        crossToInducction = `
            <h4 class="text-dark">Pasar usuario a curso de inducción</h4>
            <div class="col-md-12 text-dark mt-4">
                <p>El usuario ${typePerson} ${name} ha cumplido con la documentación requerida para su proceso en inducción.</p>
                <p>Por favor dar click al botón para avisar al usuario de su curso de inducción.</p>
                <div class="text-center"><button class="btn btn-primary ${class_}">${html_}</button></div>
            </div>
        `;

        $(".body-alert").html(crossToInducction);
        $('#tipoPersona').change();
    }
    //-------------------------
    //$("#tipoPersona").change();
    //$("#tipoPersona").trigger("change");
    //------------------------
});

$(document).on("click", ".crossToInducction", function(){

    console.log("inducción");
    var base_url = $("#base_url").data("base-url");
    var idPersona = $("#idPersonas option:selected").val();
    var puestoSeleccionado = $("#idPersonaPuesto option:selected").val();
    
    if(puestoSeleccionado == 0){
        alert("Seleccione el puesto correspondiente al usuario");
        return false;
    }

    $.ajax({
        type: `POST`,
        url: `${base_url}persona/registerTemporalUserDirectly`,
        data: {
            idPerson: idPersona,
            employment: puestoSeleccionado
        },
        success: function(response){
            console.log(response);
            var resp = JSON.parse(response);

            alert(resp.message);
            if(resp.success){
                window.location.reload();
            }
        },
        error: function(error){
            console.log(error);
        }
    });

});

/*$("#tipoPersona").change(function(){ //Obsoleto

    var value_ = $(this).val();
    var typePerson_ = value_ == 1 ? "Employee" : "Agent";

    $(`.requiredShow`).each(function(){

        var typeDiv = $(this).attr(`data-typeUser`);
        
        if(value_ == ""){
            $(this).prop("onclick", null).off("click");
            $(this).parent().children(`.divPersonSub`).addClass("ocultar").removeClass("ver");
        } else{
            if(typeDiv == typePerson_){
                $(this).parent().children(`.divPersonSub`).addClass("ver").removeClass("ocultar");
            } else{
                $(this).parent().children(`.divPersonSub`).addClass("ocultar").removeClass("ver");
                $(this).prop("onclick", null).off("click");
            }
        }      
    })

});*/

//-------------------------------

const validateGetParam = (param) => {

    var URL = new URLSearchParams(window.location.search);
    var validate = URL.has(param);

    return validate;
}