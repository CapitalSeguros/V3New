...
import {
  ...
  mapState,
  mapActions,
} from "https://cdn.jsdelivr.net/npm/vuex@3.6.2/dist/vuex.esm.browser.js";
import { createVuetify } from "https://cdn.jsdelivr.net/npm/vuetify@3.0.0-alpha.11/dist/vuetify.esm.js";
import "https://cdn.jsdelivr.net/npm/vuetify@3.0.0-alpha.11/dist/vuetify.min.css";
...
const store = createStore({
  ...
  mutations: {
    ...
    module_(state, module) {
      state.module_ = module;
    },
  },
  actions: {
    openDialog({ commit }) {
    ...
    createEvent({ commit }) {
      ...
      const request = {
        ...
        ...eventData,
        ...(meetData.code !== "n" ? { meet: { ...form } } : null),
      };
      console.log({ request });
...
const loadVueApp = () => {
  ...
  for (var a = 0; a < prospecters.length; a++) {
    prospecters[a].addEventListener("click", (event) => {
      ...
      store.dispatch("setModule", event.target.getAttribute("prpt"));
