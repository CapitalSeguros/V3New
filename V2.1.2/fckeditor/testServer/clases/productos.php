<?php
 class producto extends basedatos {
	...
	function Nuevo($Nombre,$ApePat,$ApeMat,$Direccion,$Tel,$Empresa,$User,$Pw,$Vehiculos,$Maquinaria,$Computo,$Motos){
		$CadenaSQL = " INSERT INTO productos (nombre,apepat,apemat,direccion,telefono,empresa,user,pw,vehiculos,maquinaria,computo,motos,permiso)";
	...
	function Catalogo($Idproducto=NULL){	
		//$CadenaSQL = " SELECT * FROM productos where permiso=-1 ";
		$CadenaSQL = " SELECT * FROM productos";
		if(isset($Idproducto)){
			$CadenaSQL.= " WHERE id=".$IdProducto;
		}
