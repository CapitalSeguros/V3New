...
/* Proceso Para Cuadro Comparativo */
 if(file_exists($fileXML)){ // el archivo es necesario que se depuere hasta dejarlo lo mas limpio en Formato XML
	...
	if($datosCotizacion){
		...
		foreach($datosCotizacion as $coti){
			$COTID = $coti['COTID'];
		}
...
$idCliente = $idEmpresa;
$idUsuario = $idUsuario;
$descripcion = "[".$cotizacion->INCISO->CVEVEH."] "."*".$wsMarca."*".$cotizacion->INCISO->DESC; // $row_calculamos_vehdsc['VEHDSC']; //$wsMarca." - ".$wsModelo;
$modelo = $wsYear;
$uso = $tipo_uso;
$aseguradora = "ABA";
...
$NoCotizacion = $COTID; //--> $cotizacion->COT->COTID;

$formaPago = $forma_pago;
$total = $primaTotal = $cotizacion->INCISO->PTOTAL;
$primerRecibo = $cotizacion->INCISO->RECIBOS->RECIBO[0]->PTOTAL;
$subSecuenteRecibo = $cotizacion->INCISO->RECIBOS->RECIBO[1]->PTOTAL;
...
$wsAseguradoraParticular = "ANA"; //ANA //HDI
