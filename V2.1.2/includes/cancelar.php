...
if($_REQUEST['tipoCancelar'] == 'citaCalendarioSerializada'){
	...
	$idPadre = $rowConsultaDatosCitaPadre['idAgenda'];
	switch($rowConsultaDatosCitaPadre['periodicidad']){
		case "year":
...
//** Inicio de Textos
		$asuntoInicio = "Cancelacion Cita ".$tituloPeriodicidad." - Inicia el:".DreFechaEsp($rowConsultaDatosCitaPadre['fechaStart']).substr($rowConsultaDatosCitaPadre['fechaStart'],10,10)."Hrs ";
		$mensaje = "";
		$mensaje.="<strong>Organizador:</strong> ".nombreVendedor($rowConsultaDatosCitaPadre['usuario'])."<br />";
		$mensaje.="<strong>Asunto:</strong> ".urldecode($rowConsultaDatosCitaPadre['cita'])."<br />";
		$mensaje.="<strong>Lugar:</strong> ".$rowConsultaDatosCitaPadre['lugar']."<br />";
		$mensaje.="<p>".$rowConsultaDatosCitaPadre['texto']."</p>";		
		$mensaje.= "<strong>&bull;Periodicidad Citas Canceladas:</strong><br>";
		$mensaje.= "<blockquote>";		
//** Inicio de Textos
	...
	while($rowRecorridoSerie = mysql_fetch_assoc($resRecorridoSerie)){ // inicio del While Seriealizado
		$asuntoFinal = "Terminando el: ".DreFechaEsp($rowRecorridoSerie['fechaEnd']).substr($rowRecorridoSerie['fechaEnd'],10,10)."Hrs";
		$mensaje.= "<strong>Inicio:</strong> ".DreFechaEsp($rowRecorridoSerie['fechaStart'])." ".substr($rowRecorridoSerie['fechaStart'],10,10)."Hrs ";
		$mensaje.= "<strong>Finalizaci&oacute;n:</strong> ".DreFechaEsp($rowRecorridoSerie['fechaEnd'])." ".substr($rowRecorridoSerie['fechaEnd'],10,10)."Hrs";
		$mensaje.= "<br />";
	...
	while($rowInfoCitaInvitados = mysql_fetch_assoc($resInfoCitaInvitados)){
		...
		if(strpos($rowInfoCitaInvitados['usuario'],'@')){
			$correoEnvio = $rowInfoCitaInvitados['usuario'];
		} else {
			$correoEnvio = DreCorreoUsuario($rowInfoCitaInvitados['usuario']);
		}
		...
		$asunto = $asuntoInicio.$asuntoFinal;
