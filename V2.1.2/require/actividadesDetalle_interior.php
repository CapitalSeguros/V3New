...
$sqlActividad = "
		Select 
			*
			...
			,date_format(`actividades`.`fechaPago`, '%d-%m-%Y') As `actividadFechaPago`
		From 
			`actividades` 
		Where 
...
if(false){
	$buscarPolizaSi = "
			And
			(
				MATCH (`VALOR`) AGAINST ('$buscarPoliza' IN BOOLEAN MODE)
				Or
				MATCH (`POLIZA`) AGAINST ('$buscarPoliza' IN BOOLEAN MODE)
			)
					  ";
} else {
	$buscarPolizaSi = "";
        	...
        	<table width="900" cellpadding="1" cellspacing="1" border="0" align="center" style="font-size:12px;">
				...
				<tr align="left">
					...
					<td>
						...
						<?php 
							echo $rowActividad['actividad'];
							echo "<br>";
							if($rowActividad['subRamoInterno'] != ""){
								echo "<strong>SubRamo: </strong>";
								echo urldecode($rowActividad['subRamoInterno']); 
							}
						?>
					</td>    
				</tr>
...
if(	
	...
	||
	$rowActividad['actividadInterno'] == "Pago+Cobranza"
){
?>
				<tr>
					<td colspan="2">
...
?>
				...
				<tr>
					<td colspan="2">
						...
						<font style="font-style:italic;">
						<?php echo $rowActividad['actividadFechaPago']; ?>
                        </font>
					</td>
				</tr>
				<tr>
					<td colspan="2">
						...
						<font style="font-style:italic;">
						<?php echo "$".number_format($rowActividad['importePago'],2,'.',','); ?>						
                        </font>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="2">
						...
						<font style="font-style:italic;">
						<?php echo "$".number_format($rowActividad['tipoCambio'],4,'.',','); ?>						
                        </font>
					</td>
				</tr>
<?php
...
// Botones Navegacion Primera Linea
	switch($rowActividad['actividadInterno']){
		...
		case "Cancelacion": ## 6
			...
			$urlactividadCancelada = "includes/guardar.php?tipoGuardar=ActividadCancelada&recId=".$recId."&tipoActividad=Cancelacion&usuarioCancela=".$Usuario;
					
			//**  Boton - Cancelar Actividad
			if(
				(
					// 'actividades-cotizarActividad-cotizar-si'
					DrePermisoUsuario('actividades-cancelarActividad-cancelar-si', $nodosPermisos)
					||
					mysql_result(DreQueryDB("Select `usuario` From `actividades` Where `recId` = '$recId' And `inicio` = '0'"),0) == $Usuario
				)
				&&
				isset($nodosPermisos)
			){
				if(
					$rowActividad['fin'] != "1"
					&&
					$rowActividad['prioridad'] != "5"
				){
					?>
                    <input 
                    	type="button" class="buttonGeneral" 
                        value="+ Actividad Cancelada" title="Actividad Cancelada" 
                        onClick="java:window.open('<?php echo $urlactividadCancelada;  // urlactividadCotizada; ?>','_self');"
                    />
                    <?php
				}
			}
...
<?php
	switch($rowActividad['prioridad']){
		case "1":
        	echo '<font style="color:#F00; font-weight:bold; font-size:14px;">';
            echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			echo 'Actividad Cotizada !!!';
            echo '</font>';
		break;
...
?>
                            ...
                            <table width="850" cellpadding="1" cellspacing="1" border="0" align="center">
                            	<tr style="background-image:url(img/fondoTablaDegradadoGris.png); color:#273B71; font-weight:bold; font-size:12px;">
                                    ...
                                    <td width="60" align="center"><strong>Enviar</strong></td>
                                    <td width="160"><strong>Tipo Img</strong></td>
                                    <td width="335"><strong>Descripcion</strong></td> <!-- width="325" --> 
                                    <td width="150"><strong>Fecha</strong></td>
                                	<td width="85"><strong>No Archivo</strong></td>
                                </tr>
<?php 
...
?>
                        	...
                        	<a href="<?php echo tipoUrl($_SERVER['REMOTE_ADDR']).$rutaDocumento.$rowDocumentosPoliza['NO_ARCHIVO'].$extensionDocumento; ?>" target="_blank" title="Clic Descargar">
									...
									<td align="justify">
										<?php 
											echo $rowDocumentosPoliza['DESCRIPCION']; 
											//** DrePolizaVigente($rowDo);
										?>
									</td>
									<td align="justify">
