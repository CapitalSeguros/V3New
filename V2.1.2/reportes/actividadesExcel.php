<?
	...
	$xls->OpenRow();
		...
		$xls->NewCell($fechaInicioActi,true,$xls_normal);
	$xls->CloseRow();
	
	$xls->OpenRow();
		$xls->NewCell('FECHA FINAL:',true,$xls_fondoAzul);
		$xls->NewCell($fechaFinActi,true,$xls_normal);
	$xls->CloseRow();
/*	
	$xls->OpenRow();
...
$sqlConsultaAct = "
	...
	From 
		...
		On 
		`semactividad`.`idActividad` = `actividades`.`idInterno` Inner Join `empresas`
		On 
		`actividades`.`idRef` = `empresas`.`CLAVE`
	Where
		(`actividades`.`fechaCreacion` Between '$fechaInicioActi' And '$fechaFinActi')
		And
		(`actividades`.`usuarioCreacion` Like '%$sqlFiltraVendedor%' Or `actividades`.`usuario` Like '%$sqlFiltraVendedor%')
	Order By 
		`actividades`.`recId` Asc
				  ";	
//echo "<pre>";
	//echo $sqlConsultaAct;
//echo "</pre>";
