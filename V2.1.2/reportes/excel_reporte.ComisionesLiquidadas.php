<?
			$xls->OpenRow();
				$xls->NewCell('Reporte De Comisiones Liquidadas ',false,$xls_encabezado);
			$xls->CloseRow();
...
//				$xls->NewCell('% RECIBO',true,$xls_fondoAzul);
				$xls->NewCell('% VENDEDOR',true,$xls_fondoAzul);
				$xls->NewCell('LIQUIDADO',true,$xls_fondoAzul);
				$xls->NewCell('FECHA LIQUIDACION',true,$xls_fondoAzul);
			$xls->CloseRow();
...
--					`ASEGURADORA` Like '%".str_replace($quitarCosas, $ponerCosas, strstr($filtroAseguradora, '[',0))."%'
					And
					`LIQUIDADO_VEND` Between '$fechaInicio' And '$fechaFin'
					Order By
						`LIQUIDADO_VEND` Desc
							  ";
	...
	$total_LIQUIDADO_VEND  = 0;
			...
			while($rowComisionesPenLiq = mysql_fetch_assoc($resComisionesPenLiq)){
				...
				$FECHA_LIQ_VENDX = explode('-',$rowComisionesPenLiq['LIQUIDADO_VEND']);
				$FECHA_LIQ_VENDExcel = $FECHA_LIQ_VENDX[2]."/".$FECHA_LIQ_VENDX[1]."/".$FECHA_LIQ_VENDX[0];
...
//** Totales Comision
					$total_PRIMA_NETA_COMISION = $rowComisionesPenLiq['CT_PCTJE'] + $total_PRIMA_NETA_COMISION;
					$total_RECARGO_COMISION = $rowComisionesPenLiq['PRIMA_NETA_COMISION'] + $total_RECARGO_COMISION;
					$total_GASTOS_COMISION = $rowComisionesPenLiq['RECARGO_COMISION'] + $total_GASTOS_COMISION;
					$total_TOTAL_COMISION = $rowComisionesPenLiq['GASTOS_COMISION'] + $total_TOTAL_COMISION;
					$total_LIQUIDADO_VEND = $rowComisionesPenLiq['PCTJE_DISPERSION_VEND'] + $total_LIQUIDADO_VEND;
...
//**
				$xls->NewCell($PRIMA_NETA,true,$xls_normal);
				$xls->NewCell($RECARGO,true,$xls_normal);
				$xls->NewCell($GASTOS,true,$xls_normal);
				$xls->NewCell($IVA,true,$xls_normal);
				$xls->NewCell($PRIMA_TOTAL,true,$xls_normal);
//**
				$xls->NewCell("$".number_format($CT_PCTJE,2),true,$xls_normal);
...
//				$xls->NewCell("$".number_format($PCTJE_DISPERSION_VEND,2),true,$xls_normal);
				$xls->NewCell($TOTAL_COMISION,true,$xls_normal);
				$xls->NewCell("$".number_format($PCTJE_DISPERSION_VEND,2),true,$xls_normal);
				$xls->NewCell($FECHA_LIQ_VENDExcel,true,$xls_date);
			$xls->CloseRow();
}
...
//				$xls->NewCell('',true,$xls_int_negrita);
				$xls->NewCell('',true,$xls_int_negrita);
				$xls->NewCell("$".number_format($total_LIQUIDADO_VEND,2),true,$xls_normal_negrita);
				$xls->NewCell('',true,$xls_int_negrita);

			$xls->CloseRow();			
	$xls->GetXLS('comisionesLiquidadas');
/*
	echo "<pre>";
		print_r($_REQUEST);
