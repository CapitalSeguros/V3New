            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        var colorS= Colorbarra(row.dias,row.progreso,row.inicio_ajuste,row.fecha_fin,row.siniestro_estatus);
                        // ${(row.order_tipo_t<=(_Tram.length-1) || row.estatus==null)&& _FechaFin==false?`<li><a class="bn-bono-view"  style="cursor: pointer;" onclick="Tramite(${row.id},${tramite.id},${row.id_tramite})">${tramite.nombre} asas</a></li>`:``}
                        //${(row.order_tipo_t<=(_Tram.length-1) || row.estatus==null)&& _FechaFin==true && (row.id_est_tra=='3'||row.id_est_tra==null)?`<li><a class="bn-bono-view"  style="cursor: pointer;" onclick="Tramite(${row.id},${tramite.id},${row.id_tramite},'${row.f_lasttram}')"> ${tramite.nombre} e</a></li>`:``}
                        //${row.order_tipo_t>0 && _FechaFin && row.order_tipo_t<=_Tram.length  && !permisos.includes(row.estatus)?`<li><a class="bn-bono-view"  style="cursor: pointer;" onclick="ChangeStatus(${row.id_tramite},0,0,${row.id},${row.order_tipo_t})">Cambiar estatus</a></li>`:''}
                        //editado
                        ...
                        ${row.order_tipo_t==_Tram.length && _FechaFin==false ?`<li><a class="bn-bono-view"  style="cursor: pointer;" onclick="Tramite2(${row.id},11,${row.id_tramite})" data-permiso="permiso" data-accion-permiso="Cambiar-estatus">Cerrar evento: <br> ${_Tram[9].nombre}</a></li>`:''}
