...
<div class="datosParaEmisionCapturaCambio elementosReqActividades"><label>RFC:</label><div><input type="text" id="rfcCliente" name="rfcCliente"><a class="label label-info" href="https://agsc.siat.sat.gob.mx/PTSC/ValidaRFC/index.jsf" target="_blanck">VALIDADOR DE RFC</a>-<a href="https://consisa.com.mx/rfc" class="label label-info" target="_blanck">CREAR RFC PROVISIONAL</a></div></div>
...
function guardarActividad(datos)
{
 ...
 else
 {
  ...
  else
  {
    ...
    {console.log(datos)
      var confirmacion = confirm(datos.mensaje+' \n Nombre:'+datos.clienteExistente.Nombre+' '+datos.clienteExistente.ApellidoP+' '+datos.clienteExistente.ApellidoM+' \n¿Desea continuar la actividad en el modulo de actividades para clientes existentes?');
      if(confirmacion)
      {
       let dir="<?=base_url()?>";
       window.location.replace(`${dir}actividades/agregar/${document.getElementById('tipoActividad').value}/${document.getElementById('selectRamo').value}/${document.getElementById('selectSubRamo').value}/Existente?idCliente=${datos.clienteExistente.IDCli}-${datos.clienteExistente.IDCont}`)
      } 
    } 
    else{
  	alert(datos.mensaje);
...
function opcionesActividad()
{
 ...
 if(document.getElementById('tipoActividad').value=="Cotizacion" || document.getElementById('tipoActividad').value=="CapturaEmision" || document.getElementById('tipoActividad').value=='CambiodeConducto')
 {
  ...
  if(document.getElementById('tipoActividad').value=="Cotizacion")
    {document.getElementById('rfcCliente').parentNode.parentNode.classList.add('ocultarElemento');document.getElementById('rfcCliente').parentNode.parentNode.classList.remove('elementosReqActividades');}
  else
    {document.getElementById('rfcCliente').parentNode.parentNode.classList.remove('ocultarElemento');document.getElementById('rfcCliente').parentNode.parentNode.classList.add('elementosReqActividades');}
 }
 else
 {
