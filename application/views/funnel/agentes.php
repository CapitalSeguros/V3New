<?php
 $i=0;
 //var_dump($prospectosAgentes);
    foreach ($prospectosAgentes as $key => $value) {
        $nombre[$i]=$key;
        $valor[$i]=count($value);
        $i++;
    }
...
.suspecto, .perfilado, .tercero, .cotizado, .emitido, .pagado{margin-bottom:4px;height: 40px;border-radius: 0px 0px 7px 7px;padding-top: 5px;color:#fff;}
.suspecto{width: 90%;background-color: #2e5082;}
.perfilado{width: 80%;background-color: #3c5e90;margin-left: 5%;}
.tercero{width: 70%;background-color: #48699a;margin-left: 10%;}
...
.tercero:hover{height: 52px;background-color: silver;}
...
<div class="funnel" style="width:100%;">
    ...
    <a href="#" data-toggle="modal" data-target="#generar" onclick="verDetalleX('<?php echo str_replace("_"," ",$nombre[0]);?>','agentes')">
        <div class="suspecto no-contactado">
            <div class="badge badge-success"><?php echo $valor[0]?></div> 
            <?php echo str_replace("_"," ",$nombre[0]);?>
        </div>
    </a>

    <a href="#" data-toggle="modal" data-target="#generar" onclick="verDetalleX('<?php echo str_replace("_"," ",$nombre[1]);?>','agentes')">
        <div class="perfilado contactado">
            <div class="badge badge-success"><?php echo $valor[1];?></div>
            <?php echo str_replace("_"," ",$nombre[1]);?>
        </div>
    </a>

     <a href="#" data-toggle="modal" data-target="#generar" onclick="verDetalleX('<?php echo str_replace("_"," ",$nombre[2]);?>','agentes')">
        <div class="tercero en-proceso">
            <div class="badge badge-success"><?php echo $valor[2];?></div> 
            <?php echo str_replace("_"," ",$nombre[2]);?>
        </div>
    </a>

    <a href="#" data-toggle="modal" data-target="#generar" onclick="verDetalleX('<?php echo str_replace("_"," ",$nombre[3]);?>','agentes')">
        <div class="cotizado descartado">
            <div class="badge badge-success"><?php echo $valor[3];?></div>
            <?php echo str_replace("_"," ",$nombre[3]);?>
        </div>
    </a>

    <a href="#" data-toggle="modal" data-target="#generar" onclick="verDetalleX('<?php echo str_replace("_"," ",$nombre[4]);?>','agentes')">
        <div class="emitido reclutado">
