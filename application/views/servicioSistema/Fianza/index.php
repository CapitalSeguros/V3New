                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-8 text-right" style="margin-top: 15px;">
                            ...
                            <a id="btnADD" class="btn btn-primary Nuevo" href="<?= base_url() ?>servicioSistema/FianzaAdd"><i class="fa fa-plus" aria-hidden="true"></i> Nuevo</a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    $(document).ready(function() {
        ...
        const datatable = $('#polizas').DataTable({
            ...
            ajax: {
                url: `${$pathServicio}capture/fianzas`,
                type: 'GET',
                "data": function(d) {
                    return $.extend({}, d, {
                        "search": $("#txSearch").val(),
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `<div class="row" ondblclick="EditItem(${row.IDDocto})" style="cursor:pointer;">
                        <div class="col-md-12">
                        ${row.Documento!='' && row.Documento !=null ?row.Documento:row.Solicitud}
                        </div>
                        </div>`
                    }
                },
        ...
        window.EditItem = function(Id) {
            window.location = $path + 'servicioSistema/FianzaEdit/' + Id;
        }

        window.DowloadFile= function(){
            var Filter= $("#txSearch").val();
            return window.open(`${$pathServicio}capture/docFianzas?search=${Filter}`, '_blank');
        }
    });
</script>