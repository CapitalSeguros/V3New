...
<div class="edicionFactura">
	<div>
		<div><div><label>Apertura</label></div><div><input type="text" id="idAperturaEFG" class="form-control" disabled=""></div></div>
		<div><div><label>Sucursal</label></div><div><select class="form-control" id="sucursalEFG">
			                  <option value="">Seleccione una opcion</option>
                  <option value="MERIDA">MERIDA</option>
                  <option value="NORTE">NORTE</option>
                  <option value="CANCUN">CANCUN</option>
		</select></div></div>
		<div><div><label>Tarjeta( escoger si es necesario)</label></div><div><select class="form-control" id="tarjetasEFG"></select></div></div>
		<div><div><label>Proveedor</label></div><div><select id="proveedorEFG" class="form-control"></select></div></div>
    </div>
 	<div>
 		<div><div><label>Cuentas Contables</label></div><div><select id="cuentaContableEFG" class="form-control" onchange="calculaPagoPorCanalEFG()"></select></div></div>
 		<div><div><label>Forma de pago</label></div><div><select id="formaDePagoEFG" class="form-control"></select></div></div>
 	</div>
    <div>
    	<div><div><label>Folio de factura</label></div><div><input type="text" id="folioFacturaEFG" name="" class="form-control"></div></div>
    	<div><div><label>Fecha de Factura o Documento o Fecha Ingreso:</label></div><div><input id="fechaFacturaEFG" type="date" name="" class="form-control"></div></div>    	
    </div>
    <div>
    	<div><div><label>Concepto</label></div><div><input type="text" id="conceptoEFG" name="" class="form-control"></div></div>
    	<div><div><label>Subtotal</label></div><div><input value="0.00"type="text" id="subTotalEFG" name="" class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="calculaPagoPorCanalEFG()"></div></div>
    </div>
    <div>
    	<div><div><label>Cargo Fianzas</label></div><div><input type="text" value='0.00' id="cargoFianzasEFG" name="" class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="sumaEFG()"></div></div>
    	<div><div><label>Cargo Institucional</label></div><div><input value='0.00' type="text" id="cargoInstitucionalEFG" name=""  class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="sumaEFG()"></div></div>
    	<div><div><label>Cargo Coorporativo</label></div><div><input value='0.00' type="text" id="cargoCoorporativoEFG" name="" class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="sumaEFG()"></div></div>
    	<div><div><label>Cargo Asesores</label></div><div><input value='0.00' type="text" id="cargoAsesoresEFG" name="" class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="sumaEFG()"></div></div>
    	<div id="cargosEspecialesDivEFG" class="ocultarObjetoEFG"><div><label>Cargos Especiales</label></div><div style="display: flex;flex-direction: row;"><div><input value='0.00' type="text" id="cargoEspecialesEFG" name="" class="form-control" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="sumaEFG()" disabled></div><div><button class="btn btn-info" id="mostrarGastosEspecialesBTN">+</button></div></div><div><div id="gastosEspecialesEFG" style="display: none">
  <div><div><label>GASTOS CCC</label></div><div><input value="0.00" type="text" id="gastosCCCEDF" name="" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"></div></div>
  <div><div><label>GASTOS CCO</label></div><div><input value="0.00" type="text" id="gastosCCOEDF" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"></div></div>
  <div><div><label>INVERSIONES</label></div><div><input value="0.00" type="text" id="gastosInversionesEDF" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"></div></div>
  <div><div><label>ESTRATEGIA</label></div><div><input value="0.00" type="text" id="gastosEstrategiaEDF" onkeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"></div></div>
  <div style="display: flex;justify-content: center"><button id="guardarGastosEspecialesBTN" class="btn btn-success" onclick="sumaGastosEspecialesEFG()">Guardar</button></div>
</div></div></div>
    </div>
    <div><div style="flex: 1"><div><label>Motivo del cambio</label></div><div><input type="text" id="motivoCambioEDF" class="form-control"></div></div></div>
    <div>
    	<div><div><label>Tipo de gasto</label></div><div>
    	...
    	<div><div><label>Total a pagar</label></div><div><input value="0.00" type="text" id="totalDePagoEFG" name="" class="form-control"></div></div>
    	<div><div></div><div><button class="btn btn-primary" id="guardarEFG_BTN"  >&#128190</button></div></div>
    </div>
</div>
<div class="gifEspera ocultarObjetoEFG" id="gifDeEsperaModalEFG"><img src="<?php echo(base_url().'assets\img\loading.gif')?>"></div>
