    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['meses', 'gastos', 'depositos'],
        <?php 
         foreach ($Asig as $row) {
          if($row->mes <> "Diciembre")
            {
            echo "['".$row->mes."',".str_replace(',','',$row->gasto).",".str_replace(',','',$row->deposito)."],";  
            $fp = fopen('resultadoJason.txt', 'a');fwrite($fp, print_r("['".$row->mes."',".str_replace(',','',$row->gasto).",".str_replace(',','',$row->deposito)."],", TRUE));fclose($fp);        
            }
          else
           {
             echo "['".$row->mes."',".str_replace(',','',$row->gasto).",".str_replace(',','',$row->deposito)."]";  
             $fp = fopen('resultadoJason.txt', 'a');fwrite($fp, print_r("['".$row->mes."',".$row->gasto.",".$row->deposito."]", TRUE));fclose($fp);
           }   
         }
         ?>
        ]);
