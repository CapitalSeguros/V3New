...
<? }?>
  ...
  <div id="contenedorActividades">
  	<table class="table"><thead><tr><th>Folio</th><th>Comentario</th><th></th></tr></thead><tbody id="seguimientoActividadesTbody"></tbody></table>
	</div>
...
function traerSeguimientoActividades(datos='')
{
 ...
 if(datos=='')
 {
   ...
   peticionAJAX(controlador,params,'traerSeguimientoActividades')
 }
 else
 {
   ...
   datos.seguimientoActividades.forEach(s=>{
     let band=false;
     let dir="<?=base_url();?>";
     for(let i=0;i<cant;i++){if(tabla.rows[i].dataset.folioactividad==s.folioActividad){band=true;i=cant;}}
     if(!band)
     {
     	...
     	buttonFin.innerHTML='Eliminar';
     	buttonFin.classList.add('btn');
     	buttonFin.classList.add('btn-danger');
     	buttonFin.dataset.folioactividad=s.folioActividad;
     	buttonFin.addEventListener('click',eliminarSeguimientoActividad);
...
function eliminarSeguimientoActividad(datos='')
{
	alert(this.dataset.folioactividad);
	alert(datos)
	if(datos=='')
	{
