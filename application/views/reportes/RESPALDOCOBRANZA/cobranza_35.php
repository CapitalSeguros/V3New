  ...
  <div id="divContenedorTabla" style="overflow-x: scroll; width: 80%; margin-left: 20px;margin-right: 20px">
	<div style="width:100%;border:double;overflow:hidden" id="scrollCabeceraCP">
      <table border="1" id="">
	    <thead>
		  <tr valign="top">
			<th class="divTD25" ><input type="checkBox"  onclick="seleccionarCB(this,'bodyTablaCobPen','cbSeleccionCP')" class="cbSeleccionCP"></th>
...
<div class="contMenuFlotanteMinimizado" id="divMenuFlotante"><div><button class="buttonContMenuFlotante" onclick="maxminContMenuFlotante()">&#128070</button></div><hr><div><h3 id="labelNombreClienteMF"></h3><div><button onclick="verOcultarPanel(0)" class="btn btn-primary">DOCUMENTOS</button><button onclick="verOcultarPanel(7);traerDocumentosCliente()" class="btn btn-primary">DOC. PERSONALES</button><button onclick="verOcultarPanel(8);traeContactoCliente()" class="btn btn-primary">CONTACTO DE CLIENTE</button><button onclick="verOcultarPanel(1)" class="btn btn-primary">ALTA TARJETA</button><button onclick="verOcultarPanel(2);mostrarTarjeta('')" class="btn btn-primary">TARJETAS</button><button onclick="verOcultarPanel(3);mostrarHistorial('')" class="btn btn-primary">HISTORIAL</button><button onclick="verOcultarPanel(4);mostrarComentarios('')" class="btn btn-primary">COMENTARIOS</button></button><button onclick="verOcultarPanel(6)" class="btn btn-primary">SOLIC. DE COBRO</button></button>
...
<div class="panelMenuFlotante" id="divLinksDocumentos" name="panelMenuFlotante"></div><div class="panelMenuFlotante ocultarObjeto" id="divTarjetaClienteAlta" name="panelMenuFlotante">
 ...
 <div class="panelMenuFlotante"  id="divSolicitudDeCobro" name="panelMenuFlotante">
  ...
  <div class="panelMenuFlotante ocultarObjeto" id="divContactoCliente" name="panelMenuFlotante">
  	 <div class="row">
  	 	<div class="col-md-3 col-sm-3"><label>Tipo Contacto</label><select id="idContactoClienteSelect" class="form-control"><?=imprimirTipoContacto($tipoContacto)?></select></div>
  	 	<div class="col-md-3 col-sm-3"><label>Agregar el contacto</label><input type="text" id="contactoClienteInput" class="form-control"></div>
  	 	<div class="col-md-3 col-sm-3"><label>Comentario</label><input type="text" id="comentarioContactoInput" class="form-control"></div>
  	 	<div class="col-md-1 col-sm-1"><button class="btn btn-success" onclick="guardarContactoCliente('')">&#128190</button></div>
  	 </div>
  	 <div class="row">
  	 	<div class="col-md-12 col-sm-12">
  	 		<table class="table">
  	 			<thead><tr><th>CONTACTO</th><th>TIPO DE CONTACTO</th><th>COMENTARIO</th><th>ASIGNAR</th><th>BAJA</th></tr></thead>
...
<!--div style="position: fixed;z-index: 2;top: 94%;left: 2%;width: 100%">
<div class="row">
    ...
    <div class="row" >
    <div class="col-md-11 col-sm-11 col-xs-11 ocultarObjeto" style="overflow: auto;height: auto;width:100%;height: 400px;background-color: white" id="divHistorialDeCorreos">
...
<div class="divGeneralBitacoraCorreos" style="top:90%">
	<div><button class="btn btn-warning" style="width: 5%;min-width: 5%" id="btnHistorialDeCorreos" data-char="128071">&#128071</button></div>    
...
function GuardarSolicitudDeCobro(datos='')
{
  ...
  let cadena='';
   ...
   if(cadena==''){alert('Escoger un tipo de solicitud de pago')}
   	...
   	{   		    
   		let params='datos='+cadena+'&idDocto='+iddoctoSolicitudComentario.value+'&idRecibo='+idreciboSolicitudComentario.value+'&documento='+documentoSolicitudComentario.value+'&idSerie='+documentoSolicitudidSerie.value+'&endoso='+documentoSolicitudEndoso.value+'&IDCli='+documentoSolicitudIDCli.value+'&documento';
...
function buscarPorCliente(datos)
{
  ...
  {    
   ...
   let tabla='<table class="table" border="1"><thead><tr ><td align="left" colspan="20"><button onclick=verOcultarDivPoliza() style="color:white;background-color:red">X</button><h2>Seleccionar Recibo</h2></td></tr><tr><th class="divTD15 ocultarObjeto"></th><th class="divTD150">DOCUMENTO</th><th class="divTD75"></th><th class="divTD25"></th><th class="ocultarObjeto divTD100" >RECIBO</th><th class="divTD75">SERIE</th><th class="divTD150">ENDOSO</th><th class="divTD400">NOMBRE</th><th class="divTD400">VENDEDOR</th><th class="divTD150">RAMO</th><th class="divTD150 ocultarObjeto">RAMO</th><th class="divTD150">EMAIL</th><th class="divTD150">TELEFONO</th><td class="divTD125"><th class="divTD125">COMPANIA</th><th class="divTD125">COBRO</th><th class="divTD125"></th><th class="divTD150"></th><th class="divTD150"></th><th class="divTD150">PRIMA</th><th class="divTD125">STATUS</th></tr></thead><tbody>';
      ...
      tabla=tabla+'<td data-value="'+datos.TableInfo[i].Documento+'" class="divTD200" >'+datos.TableInfo[i].Documento+'<br><br><label class="label label-info">Correos:</label><label class="label label-warning">'+datos.TableInfo[i].historialCorreos+'</label><label class="label label-info">Whats:</label><label class="label label-warning">'+datos.TableInfo[i].historialWhats+'</label><label class="label label-info">SMS:</label><label class="label label-warning">'+datos.TableInfo[i].historialSMS+'</label><br><br></td>';
...
if((int)$cobranzaPendiente->TableControl->MaxRecords>0)
 {
  foreach ($cobranzaPendiente->TableInfo as  $value) {
   ...
   $info.='<td class="divTD200"   data-value="'.$value->Documento.'">'.$value->Documento.'<br>'.$value->tipoSolicitud.' <br><br> <label class="label label-info">Correos:</label><label class="label label-warning">'.$value->historialCorreos.'</label><label class="label label-info">Whats:</label><label class="label label-warning">'.$value->historialWhats.'</label><label class="label label-info">SMS:</label><label class="label label-warning">'.$value->historialSMS.'</label><br><br>'.$btnSolCobranza.'<button class="btn-chico" onclick="abrirVentanaOpciones(this)">&#128070</button><br></td>';
...
function cambiarCorreo(valor='')
{
  ...
  if(row)
  {
  	...
  	row.dataset.email=valor;
  	row.cells[11].innerHTML=`<div class="divCell divCellEditable" contenteditable="true" onfocusout="cambiaContenidoCelda(this,'email')">${valor}</div>`
...
function cambiarTelefono(valor='')
{
  ...
  if(row)
  {
   if(valor==''){valor=document.getElementById('telefonosClienteSelect').value;}
  	row.dataset.telefono=valor;
  	row.cells[12].innerHTML=`<div class="divCell divCellEditable" contenteditable="true" onfocusout="cambiaContenidoCelda(this,'tel')">${valor}</div></div>`
...
function bajaContactoCliente(value){
if(comprobarCobranzaSeleccionada())
  	 {
     let params='IDCli='+documentoSolicitudIDCli.value;
     params+='&idClienteLealtadTipoContacto='+value;
     params+='&delete=true';
     controlador="cobranza/guardarContactoCliente/?";
    peticionAJAX(controlador,params,'armarTablaContactoCliente');
   }	
...
function armarTablaContactoCliente(datos)
{
  ...
  datos.informacion.forEach(i=>{
  	...
  	row+=`<tr data-contacto="${i.contacto}" data-idcontacto="${i.idClienteLealtadTipoContacto}" data-idtipocontacto="${i.idTipoContacto}"><td>${i.contacto}</td><td>${i.tipoContacto}</td><td>${i.comentario}</td><td><button class="btn btn-warning" onclick="${evento}('${i.contacto}')">&#9997</button></td><td><button class="btn btn-danger" onclick="bajaContactoCliente(${i.idClienteLealtadTipoContacto})">&#9940</button></td></tr>`;
  })
...
<script type="text/javascript">
    document.getElementById('btnHistorialDeCorreos').addEventListener("click", function(){
        ...
        btnHistorialDeCorreos.parentNode.parentNode.style.top="20%";
    }
        else{this.dataset.char=128071;this.innerHTML='&#128071';btnHistorialDeCorreos.parentNode.parentNode.style.top="90%";}
    })
...
function historialEnviosPorUsuario(datos)	
{
  ...
  else
  {
    ...
    datos.envios.forEach(e=>{
     tablaBody+=`<tr data-filtropara="${e.envioDestinoCH}" data-filtrofecha="${e.fechaCreacion}" data-filtronombre="${e.NombreCompleto}" data-filtrocomentario="${e.comentarioDelEnvio}" data-filtrodocumento="${e.documento}"><td>${e.NombreCompleto}</td><td>${e.envioDestinoCH}</td><td>${e.documento}</td><td>${e.idSerie}</td><td><a href="${e.hRefCH}" target="_blank">DOCUMENTO:${e.documento} RECIBO:${e.idSerie}</a></td><td>${e.fechaCreacionHora}</td><td>${e.comentarioDelEnvio}</td>`;
     bandEnvio=true;
     bandNombre=true;
     bandFecha=true;
     bandComentario=true;
