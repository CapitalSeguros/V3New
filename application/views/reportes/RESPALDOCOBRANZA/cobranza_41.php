...
<div class="collapse mt-4 mb-4" id="guion_tel">
	...
	<div class="row">
		<div id="divTotalCob" class="col-md-3"><label class="label label-info">COBRANZA PENDIENTE:</label><label id="spanTotalCob" class="label label-warning" ></label></div>
		
		<div id="" class="col-md-2" id="divTotalCobranzaConDoc"><label   class="label label-info">SIN DOCUMENTO:</label><label class="label label-warning"  id="divTotalCobPenSinDoc"></label></div>
		<div class="col-md-2"><label class="label label-info">CON RECIBO:</label><label  id="divTotalCobranzaConDocRec" class="label label-warning" ></label></div>
    <div class="col-md-2">
  ...
  <div id="divContenedorTabla" style="overflow-x: scroll; width: 80%; margin-left: 20px;margin-right: 20px">
	<div style="width:100%;border:double;overflow:hidden" id="scrollCabeceraCP">
      <table border="1" id="cobranzaPendienteTable">
	    <thead>
		  <tr valign="top">
			<th class="divTD25" ><input type="checkBox" id="seleccionCBDocumento"  onclick="seleccionarCB(this,'bodyTablaCobPen','cbSeleccionCP')" class="cbSeleccionCP"></th>
			<th class="divTD200" ><div>Documento</div><div><?= $cobranzaPendienteArmado['filtroDocumento']?></div></th>
			<th class="divTD75">
 ...
 req.open('POST', url, true); req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  ...
  { 
	 ...
	 if(this.responseText!=''){	
	 	var respuesta=JSON.parse(this.responseText); 
	 	if(respuesta.cesionExpiro){alert('LA SESION CADUCO EN UNOS MOMENTOS SERA REEDIRECCIONADO LA PAGINA DE LOGIN');window.location.replace("<?=base_url();?>");}
	 	...
	 	else
	 	{
             let cantidad=respuesta.children.length
              ...
              { 
                 ...
                 {
                    let textEndoso=respuesta.text+'_'+respuesta.periodo;
                 	let textEndosoComprobante=respuesta.text+'_'+respuesta.periodo+'Comprobante';                 	
                 	if(respuesta.children[t].text==textEndoso || respuesta.children[t].text==textEndosoComprobante){bandEntrada=1;}
                 	if(respuesta.children[t].text==textEndoso){dataHref=respuesta.children[t].href;}
                 	if(respuesta.children[t].text==textEndosoComprobante){dataHrefComprobante=respuesta.children[t].href;}
                 }
