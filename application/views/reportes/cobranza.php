...
<style type="text/css">
  ...
  .tab_gap button:hover {
    background-color: #6e5b99;
  }

  .tab_gap button.active {
    background-color: #361666;
  }
...
?>
<div class="tab_gap">
  <button class="tablinks" onclick="openTab(event, 'Tab1')">Cobranza</button>
  <button class="tablinks" onclick="openTab(event, 'Tab2')" style="white-space: nowrap; width: auto;">Reportes de Pagos</button>
</div>

<div id="Tab2" class="tabcontent">
  <?php $this->load->view('reportes/cobranza_pagos'); ?>
</div>
<?php } ?>

<div id="Tab1" class="divPrincipal tabcontent">
<div class="divMenu">
...
.botonesMenu[title="RECHAZO DOMICILIADO"]{background: url(<?echo(base_url().'assets/images/icon-rejected.png') ;?>) no-repeat; background-size: contain;}
.botonesMenu[title="DESCARGAR REPORTE PARA SOLICITUD DE COBRO"]{background: url(<?echo(base_url().'assets/images/office_xls.png') ;?>) no-repeat; background-size: contain;}
...
.contenedorOpciones>div:nth-child(3){display: flex;}
  ...
  <div id="divContenedorTabla">
        ...
        <?php if($permisoAplicacionPago) { ?><div class="botonesMenuDiv"><button title="RECHAZO DOMICILIADO" class="botonesMenu" id="btnRechazoDomiciliado" onclick="mostrarRechazoDomiciliado()"></button></div> <?php } ?>
        <div class="botonesMenuDiv"><button title="DESCARGAR REPORTE PARA SOLICITUD DE COBRO" class="botonesMenu" id="btnReporteSoliCobro" onclick="descargarReporteSoliCobro()"></button></div>
	    	</div>
...
<? 
   /*SE VERIFICARA SI LO VUELVEN A PEDIR 27-11-2024 */
   $band=0;
   if($band==1){
?>
<button onclick="verOcultarPanel(1)" class="btn btn-primary btn-xs">ALTA TARJETA</button>
<button onclick="verOcultarPanel(2);mostrarTarjeta('')" class="btn btn-primary btn-xs">TARJETAS</button>
<?}?>
<button onclick="verOcultarPanel(3);mostrarHistorial('')" class="btn btn-primary btn-xs">HISTORIAL</button><button onclick="verOcultarPanel(4);mostrarComentarios('')" class="btn btn-primary btn-xs">COMENTARIOS</button></button><button onclick="verOcultarPanel(6)" class="btn btn-primary btn-xs">SOLIC. DE COBRO</button></button>
...
<div class="panelMenuFlotante" id="divLinksDocumentos" name="panelMenuFlotante"></div><div class="panelMenuFlotante ocultarObjeto" id="divTarjetaClienteAlta" name="panelMenuFlotante">
 ...
 <div id="divComentariosContenedor" class="row"></div>
  ...
  <div class="panelMenuFlotante ocultarObjeto" id="divPagoCobranza" name="panelMenuFlotante">
    ...
    <div><div><label class="etiquetaSimple">Método de pago:</label></div><div>
      <select  name="" class="form-control" id="selectMetodoPago">
        <option value="-1">Seleccione una opción</option>
        <?php
        foreach($metodosPagos as $mp) {
 ...
 <div class="panelMenuFlotante"  id="divSolicitudDeCobro" name="panelMenuFlotante">
          ...
          <div class="row">
            <div class="col-md-3 col-sm-3"><label>Tokenización</label></div>
            <div class="col-md-1 col-sm-1"><input type="checkBox" name="checkSolicitudDeCobro" value="6" data-descripcion="Tokenización" class="form-control"></div>
            <div class="col-md-6 col-sm-6"><input type="text" id="comentarioSolicitudDeCobro6" style="text-align: left" placeholder="agregar comentario" class="form-control"></div>
          </div>
          <div class="row">
            <div class="col-md-3 col-sm-3"><label>Devolución</label></div>
            <div class="col-md-1 col-sm-1"><input type="checkBox" name="checkSolicitudDeCobro" value="7" data-descripcion="Devolución" class="form-control"></div>
            <div class="col-md-6 col-sm-6"><input type="text" id="comentarioSolicitudDeCobro7" style="text-align: left" placeholder="agregar comentario" class="form-control"></div>
          </div>
...
function agregarComentariosRenovacion(datos)
{
	...
	if(datos=='')
	{
		...
		let fecha=rowSeleccionado[0].getAttribute('data-fhasta').split(' ');//'2020-10-15T00:00:00-06:00'.split(' ');
         let fecha2=fecha[0];
...
{	
 	...
 	if(comprobarCobranzaSeleccionada())
 {
  ...
  let EndosoT=rowSel.dataset.endoso!="[object Object]"?rowSel.dataset.endoso:null;
   let params=`idRecibo=${idreciboSolicitudComentario.value}&status=${status}&IDRecibo=${rowSel.dataset.idrecibo}&idDocto=${rowSel.dataset.iddocto}&serie=${rowSel.dataset.serie}&IDCli=${rowSel.dataset.idcli}&endoso${EndosoT}&comentario=${comentario}`;
...
function aplicarPago(datos)
{
  if(datos=='')
  {
   ...
   // params=params+'&='+document.getElementById('').value;    
    let metodo_pago_value = document.getElementById('selectMetodoPago').value;
    let metodo_pago = document.getElementById('selectMetodoPago').options[document.getElementById('selectMetodoPago').selectedIndex].text;
    if(metodo_pago_value === '-1'){
      swal("Aviso", "Por favor, seleccione un método de pago.", "info");
      return;
    }
    params = params + '&MetodoPago=' + metodo_pago;
...
function traeDocumento(parametros,respuesta)
{
	...
	else
	{
       ...
       //let desde=respuesta.FDesde.split(' ');
       //let hasta=respuesta.FHasta.split(' ')
...
function mostrarSemaforo(fecha, periodo) {
    if($('#opcionFechaSelect').val() !== 'FDesde') return '';
    const DIAS_LIMITE = 14;
    let html_semaforo = '';
    let fecha_desde = fecha.split(' ')[0];
    ...
    //let fecha_actual = new Date(`${anio}-${mes}-${dia}`); // Cambié a Date para asegurarme de que es un objeto de fecha
    let fecha_actual = new Date(anio, mes - 1, dia);

    const fecha_inicio = new Date(fecha_desde);
    let dias_diferencia;

    if (periodo === '1') {
      // Calcular la diferencia hacia adelante (fecha desde + 14 días)
      const fecha_limite = new Date(fecha_inicio);
      fecha_limite.setDate(fecha_inicio.getDate() + DIAS_LIMITE);
      dias_diferencia = (fecha_limite - fecha_actual) / (1000 * 60 * 60 * 24); // Diferencia en días
    } else {
      // Calcular la diferencia hacia atrás (fecha desde - 14 días)
      const fecha_limite = new Date(fecha_inicio);
      fecha_limite.setDate(fecha_inicio.getDate() - DIAS_LIMITE);
      dias_diferencia = (fecha_actual - fecha_limite) / (1000 * 60 * 60 * 24); // Diferencia en días
    }

    if (dias_diferencia >= 12) {
      html_semaforo = '<div style="width: 20px; height: 20px; background-color: green; border-radius: 50%; border: 1px solid black;"></div>';
    } else if (dias_diferencia >= 5) {
      html_semaforo = '<div style="width: 20px; height: 20px; background-color: #FFBF00; border-radius: 50%; border: 1px solid black;"></div>';
    } else {
      html_semaforo = '<div style="width: 20px; height: 20px; background-color: red; border-radius: 50%; border: 1px solid black;"></div>';
    }

    //console.log('periodo', periodo);
    //console.log('fecha_actual', fecha_actual);
    //console.log('fecha_desde', fecha_desde);
    //console.log('diferencia en dias: ' + dias_diferencia);

    return html_semaforo;
}
...
function rowTablaCobranza(c,padre,evento,devolverObjeto=false)
{
	...
	if(c.cobranzaConSolicitud>0 || c.cobranzaSinSolicitud>0 || c.cobranzaComenatarios>0)
      ...
      tabla += `
        <td data-value="${c.Documento}" class="divTD200 ${claseStatus}">
            ${c.Documento}
            <br><br>
            <div style="display:flex">
                <div class="iconosTipoEnvio" hover="1">
                    <img src="assets/images/clickEnviado.png" style="background:#005fff" onclick="abrirVentanaOpciones(this)">
                </div>
                <div class="iconosTipoEnvio">
                    <img src="assets/images/emailEnviado.png">
                    <label id="${c.IDRecibo}${c.Serie}CORREOS">${c.historialCorreos}</label>
                </div>
                <div class="iconosTipoEnvio">
                    <img src="assets/images/whatsEnviado.png">
                    <label id="${c.IDRecibo}${c.Serie}WHATS">${c.historialWhats}</label>
                </div>
                <div class="iconosTipoEnvio">
                    <img src="assets/images/smsEnviado.png">
                    <label id="${c.IDRecibo}${c.Serie}SMS">${c.historialSMS}</label>
                </div>
                <div class="iconosTipoEnvio">
                    ${siniestro}
                </div>
                ${btnSolCobranza}
                <div class="iconosTipoEnvio">
                  ${mostrarSemaforo(c.FDesde, c.Periodo)}
                </div>
            </div>
            ${requiereFactura}
        </td>
...
<script type="text/javascript">
  ...
  */
  ...
  /**
   * Muestra un mensaja de espera
   */
  function mostrarSwalEspera() {
    swal({
      title: "Espere",
      text: "Cargando...",
      content: {
  ...
  /**
   * Mostramos la ventana para redactar el rechazo domiciliado
   */
  function mostrarRechazoDomiciliado() {
    let seleccionados = $('input[name="cbSeleccionCP"]:checked');
    if (seleccionados.length !== 1) {
      swal("Aviso", "Debe seleccionar solo una póliza.", "info");
      return;
    }

    let trPadre = seleccionados.first().closest('tr');

    let datos_rechazo = {
      nombre_cliente: trPadre.find('td').eq(7).text(),
      email_cliente: trPadre.find('td').eq(11).text(),
      nombre_vendedor: trPadre.find('td').eq(8).text(),
      folio_doc: trPadre.find('td').eq(1).attr('data-value'),
      periodo: trPadre.find('td').eq(5).text(),
      vigencia: trPadre.attr('data-fdesde').split(' ')[0],
      endoso: trPadre.find('td').eq(6).text(),
      compania: trPadre.find('td').eq(14).text(),
      prima_total: trPadre.find('td').eq(16).text(),
      IDRecibo: trPadre.attr('data-idrecibo'),
      idDocto: trPadre.attr('data-iddocto'),
      IDCli: trPadre.attr('data-idcli'),
    };
    ...
    let newWindow = window.open('', 'ViewRechazoDomiciliado', windowFeatures);

    // Creamos el formulario dinámico
    let form = document.createElement("form");
    form.method = "POST";
    form.action = '<?php echo site_url('cobranza/verRechazoDomiciliado'); ?>';
    form.target = "ViewRechazoDomiciliado";

    for (let name in datos_rechazo) {
      let input = document.createElement("input");
      input.type = "hidden";
      input.name = name;
      input.value = datos_rechazo[name];
      form.appendChild(input);
    }

    // Agregamos el formulario al document y lo enviamos
  ...
  /**
   * Dispara el proceso para generar y descargar el reporte de solicitudes de cobro como un archivo excel
   */
  async function descargarReporteSoliCobro() {
    mostrarSwalEspera();
    let filas = obtenerDatosTabla();
    let datos_v3 = await obtenerDatosCSC(filas);
    let datos_sicas = await obtenerDatosRecibosSicas(filas);
    let datos_excel = generarDatosAExportar(filas, datos_sicas, datos_v3);
    descargarExcelSC(datos_excel);
    swal.close();
  }

  /**
   * Obtiene los datos que ya estan incrustados en la tabla
   */
  function obtenerDatosTabla() {
    let datos = [];
    $('#cobranzaPendienteTable tr[name="solicitudCobranza"]').each(function () {
      let fila = {
        IDRecibo: $(this).attr('data-idrecibo'),
        documento: $(this).find('td:nth-child(2)').text().trim(),
        endoso: $(this).find('td:nth-child(7)').text().trim(),
        periodo: $(this).find('td:nth-child(6)').text().trim(), //idSerie
        fdesde: $(this).attr('data-fdesde'),
        fhasta: $(this).attr('data-fhasta'),
        nombre_completo: $(this).find('td:nth-child(8)').text().trim(),
        vendedor: $(this).find('td:nth-child(9)').text().trim(),
        id_statuscsc: $(this).attr('data-statuscsc'),
        compania: $(this).find('td:nth-child(15)').text().trim(),
        ramo: $(this).find('td:nth-child(10)').text().trim(),
        subramo: $(this).find('td:nth-child(11)').text().trim(),
        conducto_cobro: $(this).find('td:nth-child(16)').text().trim(),
        prima_total: $(this).find('td:nth-child(17)').text().trim()
      };
      datos.push(fila);
    });
    return datos;
  ...
  /**
   * Obtiene la informacion desde la tabla cobranzasolicitudcobro
   */
  async function obtenerDatosCSC(datos){
    return new Promise((resolve, reject)=>{
      let id_recibos = [];
      datos.forEach((dato)=>{
        id_recibos.push(dato.IDRecibo);
      });

      $.ajax({
        url: '<?= base_url("cobranza/obtenerCSC_SC"); ?>',
        type: 'POST',
        data: {
          idrecibos: id_recibos
        },
        dataType: 'json',
  ...
  /**
   * Obtiene la informacion de los recibos registrados en sicas usando los IDRecibo
   */
  async function obtenerDatosRecibosSicas(datos_v3) {
    return new Promise((resolve, reject) => {
      let id_recibos = '';
      datos_v3.forEach((registro) => {
        id_recibos += `|${registro.IDRecibo}`;
      });

      $.ajax({
        url: '<?= base_url("cobranza/obtenerDatosRecibosSicas"); ?>',
        type: 'POST',
        data: {
          idrecibos: id_recibos
        },
  ...
  /**
   * Genera el arreglo de datos que servira para generar el archivo excel
   */
  function generarDatosAExportar(datos_tabla, datos_sicas, datos_v3) {
    let encabezado_excel = [
      'documento',
      'Endoso',
      'idSerie',
      'Desde',
    ...
    let datos_excel = [];
    datos_excel.push(encabezado_excel);

    datos_tabla.forEach((fila) => {
      datos_excel.push([
        fila.documento.split('\n')[0],
        fila.endoso,
        fila.periodo,
        fila.fdesde.split(' ')[0],
        fila.fhasta.split(' ')[0],
        obtenerDatoPorId(datos_v3, fila.IDRecibo, 'fechaInsercion'),
        `${obtenerDatoPorId(datos_v3, fila.IDRecibo, 'emailUser')} (${obtenerDatoPorId(datos_v3, fila.IDRecibo, 'nombre_usuario')})`,
        `${obtenerDatoPorId(datos_v3, fila.IDRecibo, 'usuarioResponsable')} (${obtenerDatoPorId(datos_v3, fila.IDRecibo, 'nombre_responsable')})`,
        fila.nombre_completo,
        obtenerDatoPorId(datos_sicas, fila.IDRecibo, 'ApellidoP'),
        obtenerDatoPorId(datos_sicas, fila.IDRecibo, 'ApellidoM'),
        obtenerDatoPorId(datos_sicas, fila.IDRecibo, 'Nombre'),
        fila.vendedor,
        obtenerDatoPorId(datos_sicas, fila.IDRecibo, 'GerenciaNombre'),
        fila.id_statuscsc,
        obtenerNombreTipoSolicitudCobro(fila.id_statuscsc),
        fila.compania,
        fila.ramo,
        fila.subramo,
        fila.conducto_cobro,
  ...
  /**
   * Crear y descarga automaticamente un archivo excel con el reporte
   */
  function descargarExcelSC(datos) {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(datos);
    XLSX.utils.book_append_sheet(wb, ws, "Solicitud de cobro");
    let fechaActual = new Date().toISOString().split(' ')[0];
    XLSX.writeFile(wb, `ReporteSolicitudDeCobro_${fechaActual}.xlsx`);
  }

  /**
   * Regresa el nombre de la solicitud de cobro dependiendo de su ID
   */
  function obtenerNombreTipoSolicitudCobro(id_tipo_cobro) {
    const nombres_tipos_cobros = {
      0: 'Aplicación de pago',
      1: 'Rehabilitación de póliza',
      2: 'Cobro de recibo',
    ...
    if (nombres_tipos_cobros.hasOwnProperty(id_tipo_cobro)) {
      return nombres_tipos_cobros[id_tipo_cobro];
    } else {
      return 'N/A';
    }
  }
  ...
  /**
   * Busca y devuelve el valor del campo solicitado en un arreglo de datos proporcionado
   */
  function obtenerDatoPorId(datos, id, campoDeseado) {
    const objetoEncontrado = datos.find(obj => obj.IDRecibo === id);

    if (objetoEncontrado && objetoEncontrado.hasOwnProperty(campoDeseado)) {
      if (typeof objetoEncontrado[campoDeseado] !== 'object') {
        return objetoEncontrado[campoDeseado];
      } else {
        return '';
      }
    }
  ...
  /**
   * Función auxiliar para convertir string a binario
   */
  function s2ab(s) {
