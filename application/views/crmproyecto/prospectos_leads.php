...
<div class="well" style="width: 50%;padding: 10px;margin-top: 8px;margin-left: 5px;">
    <input type="text" name="nombre" class="form-control" placeholder="Busqueda por nombre de prospecto"  onkeyup="filtroProspectoKey(this,'Nombre')">
</div>

<section>
    <h3 class="titulo-secciones" style="font-size: 18px;">&nbsp;&nbsp;&nbsp;Seguimiento Prospección Telemarketing (Leads)</h3>
...
<div id="pantalla">
    <?php include('prospectos_leads_filtrado.php');?>
</div>
...
function filtroProspectoKey(valor,param){
    divResultado = document.getElementById('pantalla');  
    ajax=objetoAjax();  
    valor=valor.value; 
    var base=document.getElementById('base').value;
    var URL=base+"crmproyecto/prospectos_leads_filtradoKey?param="+param+"&valor="+valor;
    ajax.open("GET", URL);
...
function eliminar_prospecto(id){
    var opt=confirm("¿Esta seguro de eliminar este Prospecto?");
    if(opt==1){
        divResultado = document.getElementById('pantalla');  
        ajax=objetoAjax();  
        var base=document.getElementById('base').value;
        var URL=base+'crmproyecto/EliminarProspectoLeads?id='+id;
        ajax.open("GET", URL);
        ajax.onreadystatechange=function() {
            if (ajax.readyState==4) {
                divResultado.innerHTML = ajax.responseText
