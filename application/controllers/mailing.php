...
//----------------------------------------------------------------------	
	function index(){
		if(!$this->tank_auth->is_logged_in()){
			...
			$data['infoCampaigns'] = $this->db->query($sqlCampaigns)->result();
		
						$this->load->view('mailing/principal', $data);
		}
	}
	
	function hijoCampaigns($idPadre){
			$sqlCampaigns = "
				Select * From 
					`mailing`
				Where
			...
			$result	= $this->db->query($sqlCampaigns)->result();
...
//----------------------------------------------------------------------
	function addCampaigns($padre = NULL){
		
		if(isset($padre)){
			$padre	= $padre;
			$sqlNombre	= "
				Select `nombre`, `tipo_prospecto` From 
					`mailing`
				Where
					`idMailing` = '".$padre."'
						  ";
			$mailing	= $this->db->query($sqlNombre)->row();
			$nombre			= $mailing->nombre;
			$tipo_prospecto	= $mailing->tipo_prospecto;
		} else {
			$padre	= NULL;
			$nombre	= NULL;
			$tipo_prospecto	= NULL;
		...
		$data['padre']	= $padre;
		$data['nombre']	= $nombre;
		$data['tipo_prospecto']	= $tipo_prospecto;
		
		
		$this->load->view('mailing/agregar', $data);
	}
	
	function editCampaigns($idMailing){
		...
		$sqlMailing	= "
			...
			Where
				`idMailing` = '".$idMailing."'
							 ";
		$data['campaigns'] = $this->db->query($sqlMailing)->row();
		
		$this->load->view('mailing/edicion', $data);
		
	}
	
	function deleteCampaigns($idMailing){
		$sqlDelete	= "
			Delete From
				`mailing`
			Where
...
//----------------------------------------------------------------------
	...
	function editadoCampaign(){
		
		$idMailing		= $this->input->get_post('idMailing', TRUE);
		$correos		= $this->input->get_post('correos', TRUE);
		$informacion	= $this->input->get_post('informacion', TRUE);
		$tiempo			= $this->input->get_post('tiempo', TRUE);
		$tipo_prospecto	= $this->input->get_post('tipo_prospecto', TRUE);
	...
	function agregadoCampaign(){

		$padre			= $this->input->get_post('padre', FALSE);
