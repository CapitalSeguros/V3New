...
function aplicarPago(){
    ...
    if (isset($recibo->TableInfo)) {
      ...
      if ((int)$recibo->TableInfo->Status == 3) {
        ...
        $pago['IDTarjeta'] = -1;
        $respuesta = $this->ws_sicas->aplicarPagoRecibo($pago);
...
function traerDigitalVigente()
{
  ...
  $data_result=array();
        ...
        if(!isset($informacion['documentosPoliza'])){
          $informacion['documentosPoliza']['children']=array();
        }
...
$tiempoEspera=(int)$tiempoEspera+6;      
       $destination = '/var/www/html/V3/assets/img/tmp/'.$_FILES['imagenes']['name'][$i];       
...
//------------------------------------------
 ...
 function subirComprobante(){
 	...
 	if ($_FILES['DocumentoFiles']["error"] > 0){echo "Error: " . $_FILES['DocumentoFiles']['error'] . "<br>";} 
		   ...
		   $destination	= '/var/www/html/V3/assets/img/tmp/'.$nameFileSicas.'Comprobante.'.$ext;
		   move_uploaded_file($_FILES['DocumentoFiles']['tmp_name'], $destination);
  ...
  $datos=array();
   ...
   if($Status!=2)
   {
    ...
    $array=array();
     ...
     //$test=isset($renovacion->PROCESSDATA->DATA);
     //$test2=$renovacion->PROCESSDATA->DATA;
...
function polizaNoRenovada()
{
    ...
    // $vigente=$this->ws_sicas->obtenerRenovacionesFecha(null,null,null,$renovacion->PROCESSDATA->DATA,null);
     ...
     /*============ RENOVACION==============*/
      ...
      $update['IDSRamo']=(string)$documento->TableInfo->IDSSRamo;
      $respuesta=$this->ws_sicas->actualizaOT($update);
