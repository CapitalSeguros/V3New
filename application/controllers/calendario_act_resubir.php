...
class calendario extends CI_Controller{
	...
	function __construct(){
		...
		$this->load->library(array("Calendarcenis","DriveCenis",'webservice_sicas','webservice_sicas_soap','role'));
		$this->load->model(array("calendar_model",'emailmasivo_model','catalogos_model','personamodelo'));
	...
	function index(){		
		if (!$this->tank_auth->is_logged_in()) {
			...
			$data['Cliente'] = array();
			$data['Catalogo_Perfiles'] = $name_tab; //[Dennis 2020-06-22]: comentado para asignar agentes por coordinador
	...
	public function do_upload(){
		...
		try{
			...
			/*	$custom_error= array();
				...
				die();
			*/
			//$fp = fopen('resultadoJason.txt', 'a+');fwrite($fp, print_r("LLEGA HASTA ACA",TRUE));fclose($fp);	
