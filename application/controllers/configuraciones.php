...
class configuraciones extends CI_Controller{
	...
	$fecha = date("d-m-Y");
   	...
   	$consulta= "select `cs`.`NombreSucursal`,`cc`.`nombre`,`us`.`name_complete`,`ui`.`Giro`,`ui`.`Ranking`,`us`.`CelularSMS`,`us`.`email`,
   	`us`.`FechaIngresoAgente` From users us
	...
	$email = $row['email'];
	$FechaIngresoAgente = $row['FechaIngresoAgente'];
	...
	echo 	"<td HEIGHT=20>".$FechaIngresoAgente."</td> "; 
	...
	function actualizarcontrasenia(){
		...
		$banned			= $this->input->post('banned');
			$ActCreadaPorOtro			= $this->input->post('ActCreadaPorOtro');
        if($banned=='1' && $idTipoUser=='6'	)
        {
           $nuser='12';
           $mail='12';
...
/* */
		...
		$fechaIngreso			= $this->input->post('fechaStart');




//$fp = fopen('resultadoJason.txt', 'a+');fwrite($fp, print_r($Ranking,TRUE));fclose($fp);
		//calculamos valores para cambio en el campo SMSM EMail
$idsms=0;
		if($Ranking=="PLATINO")
		{	
          $idsms="18";
		...
		if($Ranking=="AGENTE EN CESION")
		{	
			$idsms="19";
		}
		if($Ranking=="PROVISIONAL PROM")
		{	
			$idsms="20";
		}
		...
		if($banned=='1' && $idTipoUser=='6'	)
        {
			...
			Set
	            ...
	            `idTipoUserSMSmail` = '".$mail."',


	            `IdSucursal` = '".$Sucursal."',
 `ActCreadaPorOtro` = '".$ActCreadaPorOtro."',
	            `IdCanal` = '".$Canal."',
	            `FechaIngresoAgente` = '".$fechaIngreso."'
...
$cadena="";
if($this->input->post("cb_Facultades")==""){
$this->db->query('update user_miInfo set agenteFacultades="" where emailUser="'.$emailUser.'"');
}
else{$count=count($this->input->post("cb_Facultades"));
  for($i=0;$i<$count;$i++){
     $cadena=$cadena.$this->input->post("cb_Facultades")[$i].';';
    }
    $this->db->query('update user_miInfo set agenteFacultades="'.$cadena.'" where emailUser="'.$emailUser.'"');
//$fp = fopen('resultadoJason.txt', 'w');fwrite($fp, print_r($cadena,TRUE));fclose($fp);
   }
