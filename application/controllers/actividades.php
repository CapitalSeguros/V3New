...
class Actividades extends CI_Controller
{
	...
	function verActividadesBeta()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if (isset($_GET['selectTipoFiltro'])) {
				$data['selectTipoFiltro'] = $_GET['selectTipoFiltro'];
				$tipoFiltro = $_GET['selectTipoFiltro'];
			}
			if (isset($_GET['selectValorFiltro'])) {
				$data['selectValorFiltro'] = $_GET['selectValorFiltro'];
				$valorFiltro = $_GET['selectValorFiltro'];
			}
	...
	{
		$destination	= 'C:\wamp64\www\Capsys\www\V3\assets\img\archivosClientesNuevos\\' . $_POST['fol'];
		$ruta = 'carpeta1/';
		$destino = 'carpeta2/';
		$archivos = glob($ruta . '*.*');
		...
		if (!$this->tank_auth->is_logged_in()) {
			...
			if ($this->input->post('tipoActividad', TRUE) == 'Emision') {
				if (isset($_POST['selectCompania'])) {
					$idPromotoria = 0;
					if ($this->input->post('selectCompania', TRUE) != '') {
						$idPromotoria = $this->input->post('selectCompania', TRUE);
					}
			...
			if ($this->input->post('tipoActividad', TRUE) == 'Emision' || $this->input->post('tipoActividad', TRUE) == 'Cotizacion'  || $this->input->post('tipoActividad', TRUE) == 'CambiodeConducto' || $this->input->post('tipoActividad', TRUE) == 'CapturaEmision') {
				if ($this->input->post('tipoCliente', TRUE) == "Nuevo") {
					...
					if ($this->input->post('IDPcien') <= 0) {
						if ($this->input->post('tipoEntidad') == 'Fisica') {
							$insertCRM['Nombre'] = $this->input->post('Nombre', TRUE);
						} else {
							$insertCRM['RazonSocial'] = $this->input->post('Nombre', TRUE);
						}
						$insertCRM['IDCli'] = -1;
					...
					if (!$existencia) {
						...
						if (isset($_POST['preferenciaComunicacion'])) {
							$insertar['preferenciaComunicacion'] = $_POST['preferenciaComunicacion'];
						}
						if (isset($_POST['horarioComunicacion'])) {
							$insertar['horarioComunicacion'] = $_POST['horarioComunicacion'];
						}
						if (isset($_POST['diaComunicacion'])) {
							$insertar['diaComunicacion'] = $_POST['diaComunicacion'];
						}
             	...
             	{$nombreCliente=$this->input->post('nombreCliente');}else{$nombreCliente=$this->input->post('clienteEscogido');}*/
					if ($this->input->post('tipoActividad') == 'Emision') {
						if ($this->input->post('nombreCliente') != '') {
							$nombreCliente = $this->input->post('nombreCliente');
						}
					} else {
						if ($this->input->post('clienteEscogido') != '') {
							$nombreCliente = $this->input->post('clienteEscogido');
						}
					}
					if (isset($_POST['Telefono1'])) {
						if ($infoCliente->Telefono1 != $this->input->post('Telefono1')) {
							$datosParaComprobar = $datosParaComprobar . 'Telefono1:' . $this->input->post('Telefono1') . ';';
						}
					}
					if (isset($_POST['EMail1'])) {
						if ($infoCliente->EMail1 != $this->input->post('EMail1')) {
							$datosParaComprobar = $datosParaComprobar . 'EMail1:' . $this->input->post('EMail1') . ';';
						}
					}
					// if($infoCliente[0]->NombreCompleto!=$nombreCliente){$datosParaComprobar=$datosParaComprobar.'NombreCompleto:'.$nombreCliente.';';}
					if ($datosParaComprobar != '') {
						$insertar['IDCli'] = $idCliente;
						$insertar['emailUsuario'] = $this->tank_auth->get_usermail();
						$insertar['idPersona'] = $this->tank_auth->get_idPersona();
						$insertar['campos'] = $datosParaComprobar;
						$this->clientemodelo->guardarClienteVerifica($insertar);
					}
				}
			}
			/*==================================================================================================================*/
			...
			switch ($tipoActividadSicas) { // Tipo Creacion Sicas
				...
				case "tarea":
					...
					if ($this->input->post('actividadUrgente', TRUE) == 1) {
						$Comentario	.= "(*)Urgente !!! \r";
					}
					...
					try {
						if (empty($idSicas) or is_null($idSicas)) {
				...
				case "ot":
					...
					$bitacoraSicas	= $this->capsysdre_actividades->CrearBitacora($ClaveBit, $Procedencia, $Comentario, $idSicas); //++
			...
			if ($this->input->post('actividadImportante', TRUE) != 0) {
				$importante = 1;/* Mandamos Correos*/
			} else {
				$importante = 0;
			}
			...
			if ($this->input->post('tipoActividad', TRUE) != "CapturaEmision" && $this->input->post('tipoActividad', TRUE) != "AclaraciondeComisiones") {
				...
				if ($this->input->post('tipoActividad', TRUE) == "AclaraciondeComisiones") {
					$userResponsable = "AUDITORINTERNO@AGENTECAPITAL.COM";
				} else {
					$userResponsable = "CAPTURA@ASESORESCAPITAL.COM";
				}
			}
			if ($this->input->post('IDSRamo', TRUE) == 20) {
				$userResponsable = "AUTOSRENOVACIONES@ASESORESCAPITAL.COM";
			}
			$email = $this->tank_auth->get_usermail();
			if ($email == "CLIENTECORPORATIVO") {
				$userResponsable = "EJECUTIVOCORPORATIVO@AGENTECAPITAL.COM";
				$idColaboradorArea = "6";
			} else {
				$idColaboradorArea = "0";
			}
			...
			$pagoConducto = 0;
			$pagoFactura = 0;
			if ($this->input->post('pagoFormas', TRUE) >= 1 || $this->input->post('pagoFormas', TRUE) <= 4) {
				$pagoFormas = $this->input->post('pagoFormas', TRUE);
			}
			if ($this->input->post('pagoConducto', TRUE) >= 1 || $this->input->post('pagoConducto', TRUE) <= 4) {
				$pagoConducto = $this->input->post('pagoConducto', TRUE);
			}
			if ($this->input->post('pagoFactura', TRUE) >= 1 || $this->input->post('pagoFactura', TRUE) <= 3) {
				$pagoFactura = $this->input->post('pagoFactura', TRUE);
			}
			$seRequiereFactura = 0;
			if ($pagoFormas == '') {
				$pagoFormas = 0;
			...
			if ($this->input->post('tipoActividad', TRUE) == 'Endoso') {
				...
				if ($_FILES['DocumentoFiles']["error"] > 0) {
					...
					$destination	= RUTA_ASSETS . 'img/tmp/' . $nameFileSicas;
			...
			if (isset($_POST['tipoAjax'])) {
				return $folioActividad;
			} else {
	...
	function agregarSeguimiento()
	{
		...
		if (!$this->tank_auth->is_logged_in()) {
			...
			$bitacoraSicas	= $this->capsysdre_actividades->CrearBitacora($ClaveBit, $Procedencia, $Comentario, $IDDocto);
		...
		if (!$this->tank_auth->is_logged_in()) {
			...
			if ($_FILES['DocumentoFiles']["error"] > 0) {
				...
				if ($folio->bandera) {
					...
					$destination	= RUTA_ASSETS . 'img/archivosClientesNuevos/' . $FolderDestino;
...
XML;
	...
	function agregarEmitir()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if ($folio->bandera) {
				...
				if ($clienteActualiza->Sexo != '0' and $clienteActualiza->Sexo != '1') {
					$clienteActualiza->Sexo = 0;
				}
				if ($clienteActualiza->fecha_nacimiento == '0000-00-00' or $clienteActualiza->fecha_nacimiento == NULL) {
					$clienteActualiza->fecha_nacimiento = '2000-01-01';
				}
				if ($clienteActualiza->fecha_constitucion == '0000-00-00' or $clienteActualiza->fecha_constitucion == NULL) {
					$clienteActualiza->fecha_constitucion = '2000-01-01';
				}
				...
				$_POST['tipoActividadSicas'] = 'ot';
				$_POST['TipoDocto'] = 0;
				$_POST['RFC'] = $clienteActualiza->RFC; //Nuevo
				$clienteContactoSicas	= $this->capsysdre_actividades->CrearClienteContacto($TipoEnt);
				...
				$Comentario	   .= "(*) " . $this->tank_auth->get_usernamecomplete() . " ";
				$Comentario	   .= "[" . $this->tank_auth->get_usermail() . "]";
				$bitacoraSicas	= $this->capsysdre_actividades->CrearBitacora($ClaveBit, $Procedencia, $Comentario, $infoActividad->idSicas); //++
				$IDBit			= $bitacoraSicas[0]->NewIDValue;
			...
			if ($this->input->post('cruce') != false) {
				$data['cruce_cartera']	= "Si";
			}
	...
	function guardarCalificacion()
	{
		...
		foreach ($calificaciones as  $value) {
			if ($value != "") {
				...
				$insert['idInternoActividad'] = (int)$_POST['idInterno'];
	...
	function modificarActividad()
	{
		...
		if ($folio->bandera) {
			if (isset($_POST['motivoCambio'])) {
				$actualizar['motivoCambio'] = $_POST['motivoCambio'];
			}
			...
			if (isset($_POST['FEmision'])) {
				$update['FEmision'] = $_POST['FEmision'];
			}
			/*=============================PARA CAPTURA======================================*/
			if (isset($_POST['cambioPropietario'])) {
				$update['IDUSERR'] = 20;
				$actualizar['captura'] = 1;
			}
			/*===============================FIN DE COMENTARIO===============================*/
...
XML;
				...
				$data['infoCliente'] = $this->db->query($cliente)->result();
				$data["ClienteContact"]['cliente']	= $this->db->query($cliente)->result()[0];
				if ($data["ClienteContact"]['cliente']->tipoEntidad == 'Moral') {
					$data["ClienteContact"]['cliente']->TipoEnt = 1;
				} else {
					$data["ClienteContact"]['cliente']->TipoEnt = 0;
				}
			...
			session_start();
			if (!isset($_SESSION['versionNuevaActividad'])) {
			...
			if (isset($_GET['servidor'])) {
				...
				if ($_SESSION['versionNuevaActividad']) {
					$this->load->view('actividades/verS', $data);
				} else {
					$this->load->view('actividades/verS', $data);
				}
	...
	function agregarDocumento_test()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if ($_FILES['DocumentoFiles']["error"] > 0) {
				...
				echo $destination	= RUTA_ASSETS . 'img/tmp/' . $nameFileSicas;
...
if ($datosCanal){
	if($datosCanal[0]->d_catalog_canales=2){
		...
		if ($ramo == "ACCIDENTES_Y_ENFERMEDADES") {
		...
		if (count($datos) == 1) {
			$userResponsable = $datos[0]->email;
		} else {
			if (count($datos) == 0) {
				$userResponsable = 'GERENTECOMERCIAL@AGENTECAPITAL.COM';
			} else {
				$userResponsable = $datos[0]->email;
			}
		}
	...
	function comprobarDatosActividad()
	{
		...
		if (trim($this->input->post('datosExpres', TRUE)) == '') {
			$array['mensaje'] = 'Es necesario agregar un comentario';
			$array['idObjeto'] = 'datosExpres';
		}
		if ($this->input->post('tipoActividad', TRUE) == 'Cotizacion') {
			if (!isset($_POST['cbCompania'])) {
				$array['mensaje'] = 'Debe seleccionar Compania';
				$array['idObjeto'] = 'cbCompania';
			}
		} else {
			if ($this->input->post('tipoActividad', TRUE) == 'Emision') {
				if (isset($_POST['selectCompania'])) {
					if ($_POST['selectCompania'] == '') {
						$array['mensaje'] = 'Debe seleccionar Compania';
						$array['idObjeto'] = 'tipoActividad';
					}
				}
				if (isset($_POST['pagoConducto'])) {
					if ($_POST['pagoConducto'] == '') {
						$array['mensaje'] = 'Debe seleccionar un conducto de pago';
						$array['idObjeto'] = 'pagoFormas';
					}
				}
				if (isset($_POST['pagoFormas'])) {
					if ($_POST['pagoFormas'] == '') {
						$array['mensaje'] = 'Debe seleccionar forma de pago';
						$array['idObjeto'] = 'pagoFormas';
					}
				}
			} else {
				if (isset($_POST['polizaNew'])) {
					if (trim($_POST['polizaNew']) == '') {
						$array['mensaje'] = 'Debe escribir el numero impreso de la poliza';
						$array['idObjeto'] = 'polizaNew';
					}
				}
			}
			if (isset($_POST['rfcCliente'])) {
				if ($_POST['rfcCliente'] == '') {
					$array['mensaje'] = 'Escribir el RFC';
					$array['idObjeto'] = 'rfcCliente';
				} else {
		...
		if ($this->input->post('IDVend', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar un agente';
			$array['idObjeto'] = 'IDVend';
		}
		if (trim($this->input->post('EMail1', TRUE)) == '') {
			$array['mensaje'] = 'Agregar un Email';
			$array['idObjeto'] = 'textEmail';
		} else {
			if (!filter_var($this->input->post('EMail1', TRUE), FILTER_VALIDATE_EMAIL)) {
				$array['mensaje'] = 'No es un Email valido';
				$array['idObjeto'] = 'textEmail';
			}
		}


		if (trim($this->input->post('celular', TRUE)) == '') {
			$array['mensaje'] = 'Agregar Celular';
			$array['idObjeto'] = 'textCelular';
		}
		if ($this->input->post('giroCliente', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar un giro';
			$array['idObjeto'] = 'giroCliente';
		}
		if ($this->input->post('entidad', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar un Tipo Entidad';
			$array['idObjeto'] = 'entidad';
		}
		...
		if ($this->input->post('entidad', TRUE) == 'Fisica') {
			...
			if (trim($this->input->post('Nombre', TRUE)) == '') {
				$array['mensaje'] = 'Debe agregar un nombre';
				$array['idObjeto'] = 'textNombres';
			}
			if (trim($this->input->post('ApellidoM', TRUE)) == '') {
				$array['mensaje'] = 'Debe agregar un apellido materno';
				$array['idObjeto'] = 'textApellidoMaterno';
			}
			if (trim($this->input->post('ApellidoP', TRUE)) == '') {
				$array['mensaje'] = 'Debe agregar un apellido paterno';
				$array['idObjeto'] = 'textApellidoPaterno';
			}
			if ($this->input->post('fecha_nacimiento', TRUE) == '') {
				$array['mensaje'] = 'Debe seleccionar fecha de nacimiento';
				$array['idObjeto'] = 'fecha_nacimiento';
			}
			if ($this->input->post('Sexo', TRUE) == '') {
				$array['mensaje'] = 'Debe seleccionar un Sexo';
				$array['idObjeto'] = 'selectSexo';
			}
		} else {

			if ($this->input->post('razonSocial', TRUE) == '') {
				$array['mensaje'] = 'Debe agregar la razon social';
				$array['idObjeto'] = 'textRazonSocial';
			}
			if ($this->input->post('fechaConstitucion', TRUE) == '') {
				$array['mensaje'] = 'Debe seleccionar fecha de constitucion';
				$array['idObjeto'] = 'fechaConstitucion';
			}
		}

		if ($this->input->post('entidad', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar una entidad';
			$array['idObjeto'] = 'selectRazon';
		}
		if ($array['mensaje'] != '') {
			$array['valida'] = false;
		}
		if ($this->input->post('IDSRamo', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar un SubRamo';
			$array['idObjeto'] = 'selectSubRamo';
		}
		if ($this->input->post('tipoRamo', TRUE) == '') {
			$array['mensaje'] = 'Debe seleccionar un Ramo';
			$array['idObjeto'] = 'selectRamo';
		}
		return $array;
	}
	//---------------------------------------------------
	...
	function actualizarClienteActualiza($array = null)
	{
		...
		if (isset($_POST['tipoEntidad'])) {
			$actualizarCliente['tipoEntidad'] = $_POST['tipoEntidad'];
		}
		if (isset($_POST['fecha_nacimiento'])) {
			if ($_POST['fecha_nacimiento'] != '') {
				$actualizarCliente['fecha_nacimiento'] = $_POST['fecha_nacimiento'];
			}
		}
		if (isset($_POST['fechaConstitucion'])) {
			if ($_POST['fechaConstitucion'] != '') {
				$actualizarCliente['fecha_constitucion'] = $_POST['fechaConstitucion'];
			}
		}
		if (isset($_POST['Sexo'])) {
			if ($_POST['Sexo'] != '') {
				$actualizarCliente['Sexo'] = $_POST['Sexo'];
			}
		}
		if (isset($_POST['estado'])) {
			$actualizarCliente['claveEstado'] = $_POST['estado'];
		}
		$this->db->where('IDCli', $IDCli2);
	...
	function actualizarCliente_Actualiza($array)
	{
		if (isset($array['IDCli'])) {
			if (isset($array['fecha_nacimiento'])) {
				...
				if (count($valFN) == 3) {
					if (!checkdate($valFN[1], $valFN[2], $valFN[0])) {
						unset($array['fecha_nacimiento']);
					}
				} else {
					unset($array['fecha_nacimiento']);
				}
			...
			if (isset($array['fecha_constitucion'])) {
				...
				if (count($valFC) == 3) {
					if (!checkdate($valFC[1], $valFC[2], $valFC[0])) {
						unset($array['fecha_constitucion']);
					}
				} else {
					unset($array['fecha_constitucion']);
				}
	...
	function guardarActividad()
	{
		...
		if ($valida['valida']) {
			...
			if ($nombreRamoSubRamo->ramo == 'ACCIDENTES Y ENFERMEDADES') {
				$nombreRamoSubRamo->ramo = 'ACCIDENTES_Y_ENFERMEDADES';
			}
			if ($nombreRamoSubRamo->ramo == 'DAÑOS') {
				$nombreRamoSubRamo->ramo = 'DANOS';
			}
	...
	function agregrActividadParaClienteNuevo()
	{
		...
		if ($this->input->post('enlaceProyecto100', TRUE) == '') {
			$data['IDCliClienteActualiza'] = 0;
		} else {
			$data['IDCliClienteActualiza'] = $this->input->post('enlaceProyecto100', TRUE);
		}
	...
	function actividadNuevaProspeccion()
	{
		...
		if ($datos[0]->tipoEntidad == 'Moral') {
			$_POST['tipoEntidad'] = 'Moral';
		} else {
			$_POST['tipoEntidad'] = 'Fisica';
		}
	...
	function comprobarActividadClienteNuevo($folioActividad)
	{
		...
		if ($folio->idSicas == 0 and $folio->IDCliClienteActualiza > 0) {
			$return->bandera = true;
		}
		return 	$return;
	}
	...
	function obtenerCompaniaPorSubRamo()
	{
		...
		$data['companias'] = $this->capsysdre_actividades->obtenerCompaniasSubRamo($_POST['idSubRamo']);
		echo json_encode($data);
	}
...
ac where ac.folioActividad='" . $this->uri->segment(3) . "' order by ac.idComentario desc")->result();
	...
	function obtenerPromotoriaPorActividad()
	{
		...
		switch ($_POST['tipoActividad']) {
			case 'Cotizacion':
				$respuesta['promotorias'] = $this->capsysdre_actividades->obtenerPromotoriaPorActividad($_POST['folioActividad'], 'Cotizacion');
				break;
			case 'Emision':
				$respuesta['promotorias']	= $this->capsysdre_actividades->obtenerPromotoriaPorActividad($_POST['folioActividad'], 'Emision');
				break;

			default:
				$respuesta['promotorias']	= $this->capsysdre_actividades->obtenerPromotoriaPorActividad($_POST['folioActividad'], 'Endoso');
				break;
		}
		echo json_encode($respuesta);
	}
				...
				if ($folio->idColaboradorArea == 6) {
					...
					foreach ($respuesta['verDocumentosActividad']['children'] as $key => $value) {
						if ($value['Tipo'] == '0') {
							...
							if (!strpos($value['href'], 'Target_' . $_POST['tipoActividad'])) {
								array_push($borrar, $key);
							}
			...
			foreach ($_FILES as $key => $value) {
				...
				switch ($idTipo) {
					case 34:
						...
						if ($_POST['endosoGDD'] != '') {
							if ($_POST['endosoIdGDD'] > 0) {
								$nameFileSicas = $_POST['documentoGDD'] . '_' . $_POST['endosoGDD'] . '_' . $_POST['periodoGDD'];
							}
						}
						$destination	= RUTA_ASSETS . 'img/tmp/' . $nameFileSicas . 'XML.' . $ext;
					...
					case 33:
						...
						$destination	= RUTA_ASSETS . 'img/tmp/' . $nameFileSicas . 'Factura.' . $ext;
						move_uploaded_file($value['tmp_name'], $destination);
					...
					case 19:
						...
						foreach ($correos as  $value) {
							...
							if ($_POST['endosoGDD'] != '') {
								$guardaMensaje['mensaje'] = '<div>Se acaba de agregar un  recibo a la poliza: ' . $_POST['documentoGDD'] . '</div><div>Endoso' . $_POST['endosoGDD'] . '</div><div> Periodo ' . $_POST['periodoGDD'] . '</div></div>';
							}
					...
					case 23:
						...
						$destination	= RUTA_ASSETS . 'img/tmp/' . $descripcion . '.' . $ext;
						move_uploaded_file($value['tmp_name'], $destination);
			...
			if ($folio->bandera) {
				...
				foreach ($_FILES as $key => $value) {
					...
					if ($idTipo == 5 || $idTipo == 7 || $idTipo == 11 || $idTipo == 12 || $idTipo == 21 || $idTipo == 22) {
						...
						$ListFilesURL			= base_url() . 'assets/img/tmp/' . $nameFileSicas;
						$TypeDestinoCDigital	= strtoupper($objEnviado["tipoActividad"]) == "ENDOSO" ? "ENDOSO" : $this->input->post('TypeDestinoCDigital', TRUE);
						$IDValuePK				= strtoupper($objEnviado["tipoActividad"]) == "ENDOSO" ? $objEnviado["IDDocto"] : $this->input->post('IDValuePK', TRUE);
						$FolderDestino			= $this->input->post('FolderDestino', TRUE);

						if ($folio->idColaboradorArea == 6) {
							#$destination='C:\wamp64\www\Capsys\www\V3\assets\img\centroDigital\\'.$idCliente.'\\'.$folio->IDDocto.'\\'.$folio->folioActividad.'\\';
							$insertarEnSicas = 0;
							$FolderDestino = $idCliente . '/' . $folio->IDDocto . '/' . $folio->folioActividad;
							$destination	= RUTA_ASSETS . 'img/centroDigital/' . $FolderDestino;
							if (!file_exists($destination)) {
								mkdir($destination, 0777, true);
							}
							$destination .= $nameFileSicas;
					...
					if ($insertarEnSicas == 1) {
						$this->capsysdre_actividades->CargarDocumento($TypeDestinoCDigital, $IDValuePK, $ListFilesURL, $FolderDestino);
					}
					$insertar['folioActividad'] = $_POST['folioActividad'];
					sleep(2);
					if ($this->input->get_post('folioActividad', TRUE) != '') {
						$this->agregarNotificacionActividad($this->input->get_post('folioActividad', TRUE), 'Se agrego el documento ' . $nameFileSicas . ' al folio ');
					}
				}
				if (isset($_POST['pasarActividadEnCurso'])) {
					if ($_POST['pasarActividadEnCurso']) {
						...
						$IDDocto = $this->input->post('IDDocto', TRUE);
						if ($this->input->post('tipoActividadSicas', TRUE) == "ot") {
							$StatusUser = $this->capsysdre_actividades->CambiaStatusUser($IDDocto, 5);
						} else {
							$StatusUserTarea = $this->capsysdre_actividades->CambiaStatusUserTarea($IDDocto, 5);
						}
					}
				}
			}
	...
	//-----------------------------------------------------------
	//<------------------------------------EDIT.Ing.Roberto_Alvarez-08-Marzo-2025------------------------------------>
	function verActividadesprueba()
	{
		if (!$this->tank_auth->is_logged_in()) {
			redirect('/auth/login/');
		} else {
			...
			if ($this->tank_auth->get_View() != "App") {
				$this->load->view('actividades/verActividadesprueba', $data);
			} else {
				$this->load->view('actividades/verActividadesprueba', $data);
			}
		}
	...
	public function obtener_semaforo($valor)
	{
		// forza a $valor a convertirse en BOOLEANO
		...
		/*$usermail = $this->tank_auth->get_usermail();
			$datos = $this->capsysdre_actividades->ActividadesTrabajandoseOperativos($usermail);
			$data['ActividadesTrabajandose'] = $datos['actividades'];
			$data['ramos'] = $datos['ramos'];*/
		...
		//<!------------------------------------Ing.Roberto_Alvarez-28-Marzo-2025------------------------------------>
		//<!------------------------Extraer información relevante------------------------>
		$arrayVal = [];
		foreach ($data['ActividadesTrabajandose'] as $ramo => $actividades) {
			if (!empty($actividades)) {
				foreach ($actividades as $valActividad) {

					$nuevoSemaforo =  $this->controlaSemaforos(
						$valActividad->nuevoSemaforo,
						$valActividad->Status,
						$valActividad->horasOficinaCP,
						$valActividad->horasPortalCP,
						$valActividad->tipoActividad,
						$valActividad->usuarioResponsable,
						$valActividad->idInterno,
						$valActividad->folioActividad
					);
					if ($valor == true) { //obtiene unicamente el semaforo
						$arrayVal[] = [
							'ramo'                  		=> $ramo,
							'tiempoSemaforo'        		=> $nuevoSemaforo['tiempo'],
							'nuevoSemaforo'         		=> $nuevoSemaforo['porcentajeTiempo'],
							'idInterno'             		=> $valActividad->idInterno,
							'folioActividad'        		=> $valActividad->folioActividad,
							'Status_Txt'            		=> $valActividad->Status_Txt,
							'Status'                		=> $valActividad->Status,
							'tipoActividad'         		=> $valActividad->tipoActividad,
							'usuarioResponsable'    		=> $valActividad->usuarioResponsable,
							'horasOficinaCP'        		=> $valActividad->horasOficinaCP,
							'horasPortalCP'         		=> $valActividad->horasPortalCP,

						];
					} else { //en caso de que el javascript detecte un nuevo elemento devuelve todos los valores 
						$arrayVal[] = [
							'ramo'                  		=> $ramo,
							'IDRamo'                		=> $valActividad->IDRamo,
							'RamosNombre'           		=> $valActividad->RamosNombre,
							'Abreviacion'           		=> $valActividad->Abreviacion,
							'tiempoSemaforo'        		=> $nuevoSemaforo['tiempo'],
							'nuevoSemaforo'         		=> $nuevoSemaforo['porcentajeTiempo'],
							'idInterno'             		=> $valActividad->idInterno,
							'folioActividad'        		=> $valActividad->folioActividad,
							'idSicas'               		=> $valActividad->idSicas,
							'NumSolicitud'          		=> $valActividad->NumSolicitud,
							'IDSRamo'               		=> $valActividad->IDSRamo,
							'Documento'             		=> $valActividad->Documento,
							'ClaveBit'              		=> $valActividad->ClaveBit,
							'IDBit'                 		=> $valActividad->IDBit,
							'Status'                		=> $valActividad->Status,
							'Status_Txt'            		=> $valActividad->Status_Txt,
							'tipoActividadSicas'    		=> $valActividad->tipoActividadSicas,
							'tiempoRespuesta'       		=> $valActividad->tiempoRespuesta,
							'inicio'                		=> $valActividad->inicio,
							'fin'                   		=> $valActividad->fin,
							'idCliente'             		=> $valActividad->idCliente,
							'nombreCliente'         		=> $valActividad->nombreCliente,
							'idContacto'            		=> $valActividad->idContacto,
							'nombreContacto'        		=> $valActividad->nombreContacto,
							'tipoActividad'         		=> $valActividad->tipoActividad,
							'ramoActividad'         		=> $valActividad->ramoActividad,
							'subRamoActividad'      		=> $valActividad->subRamoActividad,
							'actividadUrgente'  			=> $valActividad->actividadUrgente,
							'actividadUrgente_Txt'  		=> $valActividad->actividadUrgente_Txt,
							'actividadImportante' 			=> $valActividad->actividadImportante,
							'actividadImportante_Txt' 		=> $valActividad->actividadImportante_Txt,
							'viaOficina'            		=> $valActividad->viaOficina,
							'tipoEndoso'            		=> $valActividad->tipoEndoso,
							'datosExpres'           		=> $valActividad->datosExpres,
							'IDVend'                		=> $valActividad->IDVend,
							'usuarioCreacion'       		=> $valActividad->usuarioCreacion,
							'usuarioVendedor'       		=> $valActividad->usuarioVendedor,
							'usuarioResponsable'    		=> $valActividad->usuarioResponsable,
							'usuarioBolita'         		=> $valActividad->usuarioBolita,
							'nombreUsuarioCreacion' 		=> $valActividad->nombreUsuarioCreacion,
							'nombreUsuarioVendedor' 		=> $valActividad->nombreUsuarioVendedor,
							'nombreUsuarioResponsable'  	=> $valActividad->nombreUsuarioResponsable,
							'fechaCreacion'         		=> $valActividad->fechaCreacion,
							'fechaActualizacion'    		=> $valActividad->fechaActualizacion,
							'semaforo'              		=> $valActividad->semaforo,
							'importePago'           		=> $valActividad->importePago,
							'sucursal'              		=> $valActividad->sucursal,
							'superior'              		=> $valActividad->superior,
							'rankingVendedor'       		=> $valActividad->rankingVendedor,
							'sicasDelete'           		=> $valActividad->sicasDelete,
							'notasDre'              		=> $valActividad->notasDre,
							'tarjetaNumero'         		=> $valActividad->tarjetaNumero,
							'tarjetaMes'            		=> $valActividad->tarjetaMes,
							'tarjetaYear'           		=> $valActividad->tarjetaYear,
							'tarjetaCcv'            		=> $valActividad->tarjetaCcv,
							'tarjetaTitular'        		=> $valActividad->tarjetaTitular,
							'tarjetaTipo'           		=> $valActividad->tarjetaTipo,
							'tarjetaBanco'          		=> $valActividad->tarjetaBanco,
							'tarjetaTipoPago'       		=> $valActividad->tarjetaTipoPago,
							'idPagoConducto'        		=> $valActividad->idPagoConducto,
							'idPagoFactura'         		=> $valActividad->idPagoFactura,
							'idPagoFormas'          		=> $valActividad->idPagoFormas,
							'direccionFactura'      		=> $valActividad->direccionFactura,
							'cpFactura'             		=> $valActividad->cpFactura,
							'rfcFactura'            		=> $valActividad->rfcFactura,
							'polizaVerde'           		=> $valActividad->polizaVerde,
							'cruce_cartera'         		=> $valActividad->cruce_cartera,
							'captura'               		=> $valActividad->captura,
							'comentarioActividad'   		=> $valActividad->comentarioActividad,
							'idPersonaTrabaja'      		=> $valActividad->idPersonaTrabaja,
							'trabajandoseActividad' 		=> $valActividad->trabajandoseActividad,
							'emailSemaforoRojo'     		=> $valActividad->emailSemaforoRojo,
							'pasarCobranza'         		=> $valActividad->pasarCobranza,
							'comentarioTemporal'    		=> $valActividad->comentarioTemporal,
							'motivoCambio'          		=> $valActividad->motivoCambio,
							'idInternoPadre'        		=> $valActividad->idInternoPadre,
							'depuracionAutomatica'  		=> $valActividad->depuracionAutomatica,
							'IDCliClienteActualiza' 		=> $valActividad->IDCliClienteActualiza,
							'inicioSemaforo'        		=> $valActividad->inicioSemaforo,
							'incrementoSemaforo'    		=> $valActividad->incrementoSemaforo,
							'semaforoIncremento'    		=> $valActividad->semaforoIncremento,
							'responsableAntesDeCobranza' 	=> $valActividad->responsableAntesDeCobranza,
							'seRequiereFactura'     		=> $valActividad->seRequiereFactura,
							'folioPadre'            		=> $valActividad->folioPadre,
							'folioActividadSiniestros'  	=> $valActividad->folioActividadSiniestros,
							'IDDocto'               		=> $valActividad->IDDocto,
							'idColaboradorArea'     		=> $valActividad->idColaboradorArea,
							'Serie'                 		=> $valActividad->Serie,
							'horasOficinaCP'        		=> $valActividad->horasOficinaCP,
							'horasPortalCP'         		=> $valActividad->horasPortalCP,
							'promotorias'           		=> $valActividad->promotorias,
							'filtroPromotorias'     		=> $valActividad->filtroPromotorias
		...
		//<!------------------------Extraer información relevante------------------------>
		//<!------------------------------------Ing.Roberto_Alvarez-28-Marzo-2025------------------------------------>
		//<!------------------------------------Ing.Roberto_Alvarez-31-marzo 2025------------------------------------>
		$arrayOrganizado = [
			"DANOS" => [],
			"VEHICULOS" => [],
			"ACCIDENTES_Y_ENFERMEDADES" => [],
			"VIDA" => [],
			"CREDITO" => [],
			"FIANZAS" => []
		];

		foreach ($arrayVal as $actividad) {
			$ramo = $actividad['ramo'];

			if (isset($arrayOrganizado[$ramo])) {
				$arrayOrganizado[$ramo][] = $actividad;
			}
		}
		//<!------------------------------------Ing.Roberto_Alvarez-31-marzo 2025------------------------------------>
		log_message('error', print_r($arrayOrganizado, true)); // Para depuración en logs
		$json = json_encode($arrayOrganizado, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
		header('Content-Type: application/json'); // Asegura que el navegador lo interprete como JSON
		echo $json;

		// $json = json_encode($arrayVal);
		// header('Content-Type: application/json'); // Asegura que el navegador lo interprete como JSON
		// echo $json;
		// echo '<pre>';
		// print_r($data['ActividadesTrabajandose']);
		// echo '</pre>';
		exit;
		//<!--------------------------------link de la consulta-------------------------------->
		//                           V3/actividades/obtener_semaforo
		// SELECT * FROM `actividades` a WHERE a.`idInterno` = 62214
		//<!--------------------------------link de la consulta-------------------------------->
	...
	// 		$datos								= $this->capsysdre_actividades->ActividadesTrabajandoseOperativos($usermail);
	// 		$data['actividadesNoTrabajandose']	= $this->capsysdre_actividades->actividadesNoTrabajandose(null);
	// 		$data['ActividadesTrabajandose']	= $datos['actividades'];//DEVOLVER COMO JSON 
	// 		$data['ramos']						= $datos['ramos'];
	// 		$data['totalesPorRamo']				= $datos['totalesPorRamo'];
	// 		$data['personaTrabajaActividad']	= $datos['personaTrabajaActividad'];
	// 		$data['bandera']					= 1;
	// 		$data['tipoUsuario']				= $this->tank_auth->get_idTipoUser();
	// 		$data['ejecutivos']=$this->db->query('select p.idPersona,u.email from persona p left join users u on u.idPersona=p.idPersona where p.idColaboradorArea=2 and u.activated=1 and u.banned=0')->result();
	// 		$data['tutoriales'] = $this->preguntamodel->obtenerTutorialesPorModulo(self::modulo_tutorial);
	// 		// $data['datos'] = $datos;
	...
	// 	  $arrayVal=array();
	// 	  $stringArray="";
	// 	  foreach ($data['ActividadesNoTrabajandose'] as $key => $value) 
	// 	  {
	...
	// 		//   $fp =fopen('resultadoJason.txt', 'a');fwrite($fp, print_r($data['ActividadesTrabajandose'], true));fclose($fp);
	// 		echo '<pre>';
	// 		print_r($data['actividadesNoTrabajandose']);
	// 		echo '</pre>';
	// 		exit; // Para evitar que se cargue alguna vista después

	// 	}
	// }/*! verActividades */
	public function verActNoTrab()
	{
		// 1) Validar sesión
		if (! $this->tank_auth->is_logged_in()) {
			redirect('/auth/login/');
		...
		$original = $data['actividadesNoTrabajandose'];
		$salida = [
			'cotizaciones'       => [],   // array vacío si no hay cotizaciones
			'otrasActividades'   => []    // se llenará con cada stdClass
		];

		// Si hubiera cotizaciones en $original['cotizaciones'], las agregamos:
		if (! empty($original['cotizaciones'])) {
			foreach ($original['cotizaciones'] as $cot) {
				$salida['cotizaciones'][] = $cot;
			}
		}

		// Llenar 'otrasActividades' con los objetos recibidos
		if (! empty($original['otrasActividades'])) {
			foreach ($original['otrasActividades'] as $act) {
				$salida['otrasActividades'][] = $act;
			}
		}
		// $original = $data['actividadesNoTrabajandose'];
		// var_dump($original); // temporalmente
		// 4) Devolver JSON y terminar
		...
		//SELECT * from actividades a WHERE a.usuarioCreacion='SISTEMAS@ASESORESCAPITAL.COM' 
	}
	// public function verActNoTrab()
	// {
	...
	// 	//  INICIO DE DETECCIÓN DE SALIDA INESPERADA
	// 	ob_start();
	// 	$data['ActividadesCerrar'] = $this->capsysdre_actividades->devuelveActCerrar($usermail);
	// 	$datos = $this->capsysdre_actividades->ActividadesTrabajandoseOperativos($usermail);
	// 	$data['actividadesNoTrabajandose'] = $this->capsysdre_actividades->actividadesNoTrabajandose(null);
	...
	// 	if ($buffer) {
	// 		echo "¡Hay salida no esperada antes del JSON!:<br><pre>$buffer</pre>";
	// 		exit;
	// 	}
	// 	//  FIN DE DETECCIÓN
	...
	// 	// exit;
	// 	header('Content-Type: application/json');
	// 	echo json_encode(['mensaje' => 'respuesta desde PHP']); // ¿sale esto?
	// 	exit;
	...
	// 	$cerrar = $this->capsysdre_actividades->devuelveActCerrar($usermail);
	// 	$datos = $this->capsysdre_actividades->ActividadesTrabajandoseOperativos($usermail);
	// 	$actividades = $this->capsysdre_actividades->actividadesNoTrabajandose(null);
	// 	// $data['actividadesNoTrabajandose']	= $this->capsysdre_actividades->actividadesNoTrabajandose(null);
	...
	// 	if (!empty($actividades['cotizaciones'])) {
	// 		$salida['cotizaciones'] = $actividades['cotizaciones'];
	// 	}

	// 	if (!empty($actividades['otrasActividades'])) {
	// 		$salida['otrasActividades'] = $actividades['otrasActividades'];
	// 	}
	...
	// 	// exit;
	// 		echo '<pre>';
	// 		print_r($salida['actividadesNoTrabajandose']);
	// 		echo '</pre>';
	// 		exit; // Para evitar que se cargue alguna vista después
	// }
	// 		public function verActNoTrab()
	...
	//     $usermail = $this->tank_auth->get_usermail();
	//     $this->capsysdre_actividades->devuelveActCerrar($usermail); // no se usa en salida
	//     $this->capsysdre_actividades->ActividadesTrabajandoseOperativos($usermail); // tampoco
	...
	//     // IMPORTANTE: limpiar cualquier contenido anterior
	...
	function  controlaSemaforos($tiempoSemaforo, $Status, $horasOficinaCP, $horasPortalCP, $tipoActividad, $responable = '', $idInterno = '', $folioActividad = '')
	{
		...
		if ($array['tiempo'] == 'tiempoAcabando') {
			...
			if ($result == 0) {
				...
				if ($responable == 'EJECUTIVOCOTIZACIONES@AGENTECAPITAL.COM') {
					$idPersona = '552';
					$email = 'COORDINADOROPERATIVO@ASESORESCAPITAL.COM';
				}
	...
	*/
	...
	function comprobarDatosParaCrearActividad($datos)
	{
		...
		if (!(int)$_POST['IDCli']) {
			array_push($respuesta['errorVacio'], 'IDCli');
		}
		if ($_POST['datosExpres'] == '') {
			array_push($respuesta['errorVacio'], 'datosExpres');
		}
		if (!(int)$_POST['IDVend']) {
			array_push($respuesta['errorVacio'], 'IDVend');
		}
		switch ($datos['tipoActividad']) {
			case 'Cotizacion':
				if ($_POST['cbCompania'] == '') {
					array_push($respuesta['errorVacio'], 'cbCompania');
				}
				if ($_POST['IDSRamo'] == '17' || $_POST['IDSRamo'] == '19' || $_POST['IDSRamo'] == '21') {
					if ($_POST['SerieAuto'] == '') {
						array_push($respuesta['errorVacio'], 'SerieAuto');
					}
				}
				break;
			...
			case 'CambiodeConducto':
				if ($_POST['rfcCliente'] == '') {
					array_push($respuesta['errorVacio'], 'rfcCliente');
				}
				break;
			case 'Endoso':
				if ($_POST['poliza'] == '') {
					array_push($respuesta['errorVacio'], 'poliza');
				}
				if ($_POST['tipoEndoso'] == '') {
					array_push($respuesta['errorVacio'], 'tipoEndoso');
				}
				break;
			case 'CapturaEmision':
				if ($_POST['rfcCliente'] == '') {
				...
				if ($_POST['polizaNew'] == '') {
					array_push($respuesta['errorVacio'], 'polizaNew');
				}
				break;
			case 'Sustitucion':
				if (!(int)$_POST['IDDocto']) {
					array_push($respuesta['errorVacio'], 'poliza');
				}
				break;
			case 'Cancelacion':
		...
		if (count($respuesta['errorVacio']) > 0) {
			$respuesta['success'] = false;
		}
		return $respuesta;
	}
