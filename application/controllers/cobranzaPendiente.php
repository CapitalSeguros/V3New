...
class cobranzaPendiente extends CI_Controller
{
 ...
 function __construct()
  ...
  $this->load->library('Ws_sicas');
   ...
   {	
	...
	{
     $this->load->view('reportes/cobranzaPendiente');	
	}
    }
    function traeDatos(){
 		if (!$this->tank_auth->is_logged_in()) 
	...
	{ 
		if($this->input->post('fIni')=='' or $this->input->post('fFin')=='' )	
       {
 	    ...
 	    $this->load->view('reportes/cobranzaPendiente',$datos);	
	   }
       else{       		
 ...
 $vendedor=$this->tank_auth->get_IDVend();
   /* if($vendedor>0)
    {
     $datos['ConditionsAdd']='
       Desde|Hasta;3;0;'.$fechaI.'|'.$fechaF.';0;-1;VDatRecibos.FLimPago ! Documento;0;0;1;1;DatDocumentos.TipoDocto ! Status;0;0;0;0;VDatRecibos.Status   ! VendedorID;2;0;'.$vendedor.';'.$vendedor.';CatVendedores.IdVend';
    }
    else{
       $datos['ConditionsAdd']='
       Desde|Hasta;3;0;'.$fechaI.'|'.$fechaF.';0;-1;VDatRecibos.FLimPago ! Documento;0;0;1;1;DatDocumentos.TipoDocto ! Status;0;0;0;0;VDatRecibos.Status  ';
    }*/
        ...
        $xml=$this->ws_sicas->obtenerReporteCobranza($vendedor,$fechaI,$fechaF,0);
...
$contador=0;
                 ...
                 if($contador>1){
                   ...
                   $this->load->view('reportes/cobranzaPendiente',$xml);
                 }
                 else
                 { 
                   ...
                   $this->load->view('reportes/cobranzaPendiente',$x);
                 }
