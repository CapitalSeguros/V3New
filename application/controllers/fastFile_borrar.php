...
function vacaciones_(){ //Obsoleto Dennis Castillo [2022-06-26]
   ...
   function verificarCalificaciones($idPersona,$year){
    ...
    function getByMonth(){ //Modificado - Puestos 18/01/2024
        $render='';
        ...
        if($descripcion=='vacacion'){
            $acum=0;
            $dias=$this->getDiasVacaciones($idPersona, $anio);
            $dias=$dias[0];
            $render="<div class='col-md-12' style='max-height: 400px;overflow: auto;'>".$labelMessage."<table class='table table-hover' style='font-size: 14px;width: 100%;'><thead style='position:sticky;top:0;'><tr style='background: #396da1;'><th colspan='4'><i class='fa fa-plane'></i> VACACIONES</th></tr><tr style='background: #396da1;'><th>FECHA SALIDA</th><th>DÍAS CORRESPONDIENTES</th><th>DIAS SOLICITADOS</th><th>DÍAS RESTANTES</th></tr></thead><tbody>"; 
            foreach ($rs as $row) {
                $render.="<tr><td>".date('d-m-Y',strtotime($row->fecha))."</td>";
                $render.="<td style='text-align: center'>".$dias."</td>";
                $render.="<td style='text-align: center'>".$row->valor."</td>";
                $render.="<td style='text-align: center'>".($dias-$row->valor)."</td></tr>";
            }
        }
    ...
    function getDiasVacaciones($idPersona, $year){
...
$sql="SELECT user_miInfo.* from user_miInfo,persona,users where persona.idPersona=user_miInfo.idPersona AND persona.idPersona = users.idPersona AND persona.bajaPersona=0 AND users.activated=1 AND users.banned=0 AND persona.tipoPersona=1";
    ...
    if ($email == "DATACENTER@AGENTECAPITAL.COM" || $email == "COORDINADOROPERATIVO@ASESORESCAPITAL.COM") {
        $this->datos['empleados'] = $this->db->query('SELECT idPersona, email FROM personapuesto WHERE statusPuesto = 1 AND padrePuesto = '.$idPuesto)->result();
    }
    //Encontrar años
    $option = '';
    $count = date('Y') - 2016;
...
function getAllRepositories(){ //Modificado [Suemy][024-03-25]

    $job = $_GET["job"];
    $response = array();
    $dataInRepositories = $this->getGeneralRepositories($job);
    $personHistorical = $this->getPersonalHistorical($job);    
    ...
    if ($this->tank_auth->get_usermail() == "ASISTENTEDIRECCION@AGENTECAPITAL.COM" || 
        ...
        $this->tank_auth->get_usermail() == 'DIRECTORGENERAL@AGENTECAPITAL.COM') {
...
//------------------------------ //Dennis Castillo [2022-03-22]
function getPersonalHistorical($job){

    $persons = $this->PersonaModelo->getAllPersonByJob(array("idPersonaPuesto" => $job));
...
//------------------------------ //Dennis Castillo [2022-03-22]
function getGeneralRepositories($job){ //Modificado [Suemy][024-03-25]

    $getRepositories =  $this->capitalhumano->getAllDocumentsofJob($job);
    $repositories = $this->capitalhumano->getRepositories();
        $filter = array("vacaciones", "permiso", "documentos", "incapacidad","formatos","solicitudes","psicosometrico","otros","cambiopuesto");
    if ($this->tank_auth->get_usermail() == "ASISTENTEDIRECCION@AGENTECAPITAL.COM" || 
        $this->tank_auth->get_usermail() == 'CAPITALHUMANO@AGENTECAPITAL.COM' || 
        $this->tank_auth->get_usermail() == 'DIRECTORGENERAL@AGENTECAPITAL.COM') {
...
function getDashboard(){
    ...
    try{
        ...
        if(!empty($dataJob->idPersona)){
            ...
            $param['prestamos'] = $this->filterDates($this->getDescripcion($dataJob->idPersona,'prestamo'), $year, $month); //$this->getDescripcion($dataJob->idPersona,'prestamo');
            ...
            $param['sueldos'] = $this->filterDates($this->getDescripcion($dataJob->idPersona,'sueldo'), $year, $month); //$this->getDescripcion($dataJob->idPersona,'sueldo');
...
function getReportDaily(){
    ...
    $tabla.='<tr style="background-color: #08298A;color:#fff"><td style="text-align: left"><b>PROSPECCION</b></td><td style="text-align: right"><b>TOTAL</b></td><td colspan="3"></td></tr>';
    
    $tabla.='<tr style="background-color: #E6E6E6"><td colspan="5"><b>LEADS:</b></td></tr>';   
    $tabla.='<tr><td style="text-align: right">- Langing Page (Fianzas): </td><td style="text-align: right">'.$ctProspectosLeadsFianzas.'</td><td colspan="3"></td></tr>';
    $tabla.='<tr><td style="text-align: right">- Langing Page (Gmm): </td><td style="text-align: right">'.$ctProspectosLeadsGMM.'</td><td colspan="3"></td></tr>';
   
    $tabla.='<tr style="background-color: #E6E6E6"><td colspan="5"><b>PROSPECTOS:</b></td></tr>';   
    $tabla.='<tr><td style="text-align: right">- Fianzas: </td><td style="text-align: right">'.$ctProspectosFianzas.'</td><td colspan="3"></td></tr>';
    $tabla.='<tr><td style="text-align: right">- Persona: </td><td style="text-align: right">'.$ctProspectosPersona.'</td><td colspan="3"></td></tr>';

    $tabla.='<tr><td colspan="5">&nbsp;</td></tr>';
    $tabla.='<tr style="background-color: #E6E6E6"><td><b>COTIZADOS: </b></td><td style="text-align: right">'.$ctCotizados.'</td><td colspan="3"></td></tr>';

    $tabla.='<tr><td colspan="5">&nbsp;</td></tr>';
    
    $tabla.='<tr style="background-color: #E6E6E6"><td><b>CERRADOS:</b> </td><td style="text-align: right">'.$ctCerrados.'</td><td colspan="3"></td></tr>';
    ...
    $asunto="Resumen Diario de Movimientos Operativos - Capital Seguros y Fianzas";
    $desde="Avisos GAP<avisos@agentecapital.com>";
    $fechaEnvio=date('Y-m-d h:m:s');
...
//--------------------------------
    ...
    function traerFiltroAsistencias(){
        $arrayColaboradores = $this->PersonaModelo->getNombresColaboradores();
...
(day(CURRENT_TIMESTAMP())) as dia ,(hour(CURRENT_TIMESTAMP())) as hora , (minute(CURRENT_TIMESTAMP())) as minuto,(time(CURRENT_TIMESTAMP())) as horaCompleta,ELT(WEEKDAY(date(CURRENT_TIMESTAMP())) + 1, "entradaLunes", "entradaMartes", "entradaMiercoles", "entradaJueves", "entradaViernes", "entradaSabado", "entradaDomingo") AS diaSemana')->result()[0];
    ...
    if(count($asistencia)==0)
    {
      ...
      $entradaConsulta="select (DATE_FORMAT(".$fecha->diaSemana.",'%H:%i:%S' )) as entrada from persona where idPersona=".$_POST['idPersona'];
        //$fp = fopen('resultadoJason.txt', 'a');fwrite($fp, print_r($entradaConsulta,TRUE));fclose($fp);
