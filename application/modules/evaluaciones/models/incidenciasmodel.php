...
class incidenciasModel extends CI_Model
{
    ...
    function coincidenciaIncidencias($datos)
    {
        $sql='select * FROM `incidencias` WHERE `tipo_incidencias_id`='.$datos["tipo_incidencias_id"].' and `fecha_alta`="'.$datos["fecha_alta"].' 00:00:00" and `fecha_inicio`="'.$datos["fecha_inicio"].' 00:00:00" and `empleado_id`='.$datos["empleado_id"].' and `dias`='.$datos["dias"];
    ...
    function getIncidenciasTable() //Modificado [Suemy][2024-05-31]
    {
        $this->db->select('tipo_incidencias.nombre, incidencias.idincidencias as id, incidencias.empleado_id, incidencias.tipo_incidencias_id, incidencias.fecha_inicio, incidencias.fecha_alta as fecha_registro, incidencias.fecha_autorizacion as fecha_modificado, incidencias.dias, users.name_complete,incidencias.comentario, incidencias.estatus');
        ...
        $this->db->order_by('incidencias.idincidencias', 'desc'); //incidencias.fecha_inicio
        $tabla = $this->db->get();
    ...
    function tipoincidenciaByFechaColaborador($start, $end, $filtro = null)
    {
        ...
        $query = $this->db->get("incidencias i");
        $row = [];
        $test=is_array($colaborador) ? $colaborador : array();
        if ($query->num_rows() > 0) {
            //if (count($colaborador) == $query->result()) {
            if (count($test) == $query->result()) {
                $row = $query->result();
            } else {
