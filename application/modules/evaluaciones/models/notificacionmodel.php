...
class notificacionModel extends CI_Model
{
    ...
    function getUserContIncidencias()
    {
        $obj = $this->db->query('SELECT idPersona, email FROM `users` WHERE email = "CONTABILIDAD@AGENTECAPITAL.COM"');
        return $obj->result();
    }
    ...
    {   $obj=$this->db->query("select ep.titulo,date_format(ep.fecha_final,'%d-%m-%Y') fecha_final, date_format(ep.fecha_inicio,'%d-%m-%Y') fecha_inicio 
        from evaluacion_periodos ep where ep.id=".$id);
        /* $obj = $this->db
        ...
        switch ($tipo) {
            ...
            case 'INCIDENCIAS':
                $obj = $this->db
                    ...
                    ->get('incidencias i');
                break;
            case 'PERIODOS'://cambios TIC_CONSULTORES 17/03/2021
                $obj = $this->db
            ...
            case 'ALERTA':
                    $obj = $this->db
                       ->select("*")
                       ->where('sr.id', $id)
                       ->get('project_implementation_plan sr');
                   break;
            case 'PIP':
                    $obj = $this->db
                       ->select("*")
            ...
            case "LIBERAR": 
                $obj = $this->db->where("idPersona", $id)->get("persona");

                break;
            case "ALTA": 
                $obj = $this->db->where("idPersona", $id)->get("persona_temporal");
            ...
            case "RESPUESTA_ALTA": 
                ...
                break;
            //----------------------------------------------
            //Dennis Castillo [2021-11-03]
            case "PROGRESO_DOCUMENTACION_COLABORADOR": //PROGRESO_INDUCCION
                $obj = $this->db->join("persona b", "a.idPersona = b.idPersona", "inner")->where("a.idPersona", $id)->get("employe_to_user a");
            
                break;
            //----------------------------------------------
            //Dennis Castillo [2021-11-03]
            case "PROGRESO_DOCUMENTACION_AGENTE": //PROGRESO_INDUCCION
                $obj = $this->db->join("persona b", "a.idPersona = b.idPersona", "inner")->where("a.idPersona", $id)->get("prospective_to_user a");
    ...
    function getInfoPIP($id){
        $obj=$this->db->query("select pip.*,concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) nombre from project_implementation_plan pip
        inner join persona p on pip.empleado_id=p.idPersona where pip.id=".$id);
        return $obj->result_array();
    }
