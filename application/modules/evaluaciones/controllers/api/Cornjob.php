...
class Cornjob extends CI_Controller
{
    ...
    public function index()
    {
        ...
        try {
            $this->servicios->SendAllNewCorreos();
            $this->servicios->SendAdminCorreo();
            ...
            foreach ($allother as $key => $value) {
                $this->escalaSiniestrosAutosI($value);
                $this->escalaSiniestrosGMM($value);
                $this->escalaSiniestrosDanos($value);
             }
 */
        } catch (\Throwable $th) {
            throw $th;
    ...
    function escalaSiniestros($aseguradora,$cliente){
       ...
       foreach ($usuarios as $key => $value) {
           ...
           if(!empty($full)){
                ...
                //Nuevo metodos
                $subject="Escalamiento"; //Notificación Capsys-Escalamiento
                $data=array(
                    ...
                    "Tabla"=>$full,
                    "URL"=>base_url().'Siniestros/registros', //Original: Siniestros/registros
                    "Funcion"=>$this
                );
...
//------------------- Módulo de Siniestros: Autos Individual, GMM y Danos -------------------    
    function escalaSiniestrosAutosI($usuario){
        ...
        $full=array_merge( $es_0A, $es_1A, $es_2A);
        if(!empty($full)){
            ...
            echo 'Sin datos de escala por el usuario <br/>';
        }
    }
    function escalaSiniestrosGMM($usuario){
        ...
        $full=array_merge( $es_0G, $es_1G, $es_2G);
        if(!empty($full)){
            //$this->sendNotificacionManual("ALERTA", array("tabla" => $tabla, "usuario" =>  $usuario, "referencia" => "20", "Tipo" => 1), array('web', 'email'));
            $subject="Escalamiento"; //Notificación Capsys-Escalamiento
            $data=array(
    ...
    function escalaSiniestrosDanos($usuario){
        ...
        $full=array_merge( $es_0D, $es_1D, $es_2D);
        if(!empty($full)){
            //$this->sendNotificacionManual("ALERTA", array("tabla" => $tabla, "usuario" =>  $usuario, "referencia" => "20", "Tipo" => 1), array('web', 'email'));
            $subject="Escalamiento"; //Notificación Capsys-Escalamiento
            $data=array(
...
//---------------- Módulos de Siniestros escalado de Autos I., GMM y Daños ------------------
    function escalaSiniestrosTestAutosI($usuario){
        $es_0A = $this->servicios->escalamientoAutosI('esc_0', $usuario); ///escala 0
        $es_1A = $this->servicios->escalamientoAutosI('esc_1', $usuario); ///escala 1
        $es_2A = $this->servicios->escalamientoAutosI('esc_2', $usuario); ///escala 2
        $full=array_merge($es_0A, $es_1A, $es_2A);
    ...
    function escalaSiniestrosTestGMM($usuario){
        $es_0G = $this->servicios->escalamientoGMM('esc_0', $usuario); ///escala 0
        $es_1G = $this->servicios->escalamientoGMM('esc_1', $usuario); ///escala 1
        $es_2G = $this->servicios->escalamientoGMM('esc_2', $usuario); ///escala 2
        $full=array_merge($es_0G, $es_1G, $es_2G);
    ...
    function escalaSiniestrosTestDanos($usuario){
        $es_0D = $this->servicios->escalamientoDanos('esc_0', $usuario); ///escala 0
        $es_1D = $this->servicios->escalamientoDanos('esc_1', $usuario); ///escala 1
        $es_2D = $this->servicios->escalamientoDanos('esc_2', $usuario); ///escala 2
        $full=array_merge($es_0D, $es_1D, $es_2D);
...
//-------------------------------------------------------------------------------------------
    ...
    function SendNotificationEscala($user,$subject,$body){
        ...
        $data=array(
            ...
            //"para"=>"desarrollo@agentecapital.com",
