...
class calendar_model extends CI_Model{
	...
	public function Insert_Cal($data){
		...
		try{
			...
			//Insersion en Calendario de citas general
			$fechaGuardado=substr($data["created_on"], 0, -6);
			 $data_tableX = array(
					'emailUsuario' => $data["created_by"],
					'titulo' => $data["title"],
					'fechaInicial' => $fechaGuardado
			);
			$this->db->insert('citascalendar', $data_tableX);
	...
	//-----------------------------------------------------------------------------------------------
	function inserta_invitados($datosInvitado){ //Dennis [2021-08-23]
	...
	//------------------------------------------------------------------------------------------------------
	function obtenerInvitado($idInvitado,$tipo){ //Nuevo Dennis [2021-08-23]

		$resultado=array();
		$condicion = $tipo == "interno" ? "id_invitado" : "idCorreoExt";
		$tabla = $tipo == "interno" ? "invitados_eventos" : "catalog_correos_externos";

		$this->db->where($condicion, $idInvitado);
		$query=$this->db->get($tabla);
	
		return $query->num_rows() > 0 ? $query->row() : array();
	}
	//-----------------------------------------------------
	/*function obtenerInvitado($idInvitado){ //obsoleta
	...
	//------------------------------------------------------------------------------------------------------
	function actualizaEstadoInterno($invitado, $estado){ //$evento, //Nuevo
	...
	//-----------------------------------------------
	/*function actualizaEstadoInterno($evento, $invitado, $estado){ //obsoleto
	...
	//--------------------------------------------------------------------------------------------------------
	function cancelaCorreoExterno($idIE, $cambio){ //Nuevo Dennis [2021-08-23]

		$bandera=false;

		$this->db->where("idCorreoExt", $idIE);
		//$this->db->where("id_evento",$evento);
	...
	//----------------------------------------
	/*function cancelaCorreoExterno($correo, $evento, $cambio){ //obsoleta
	...
	function consultaEvento($evento){
		$query = $this->db->query("SELECT cal_id,title,categoria_capacitacion,sub_categoria_capacitacion,ramo_capacitacion,created_on from cal_events where cal_id='$evento'");
		$result = $query->result();
      	return $result;
	}
	//-------------------------------------------------------------------------------------------------------
	function eliminaInfoCapacitacion($columna, $evento, $tabla){ //Nuevo [2021-08-23]

		$this->db->where($columna, $evento);
		$delete = $this->db->delete($tabla);

		return $delete; //$this->db->affected_rows();
	}
	//---------------------------------------------------
	/*function eliminaInfoCapacitacion($evento){ //obsoleta

		$tablas=array("tmp_events_hours","registro_certificacion","invitados_eventos","calendar_zoom_meetings");
	...
	//-------------------------------------------------
	function agregarCorreo($data_table){
		$this->db->insert('envio_correos', $data_table);
	}	
	//-------------------------------------------------
	function insertaInvitadoExterno($concentrado, $tabla){ //Nuevo Dennis [2021-08-23]

		$return = false;

		foreach($concentrado as $d_c){

			$this->db->insert($tabla, $d_c);
	...
	//-------------------------------------------------
	function devuelveCapacitaciones($correo = null, $tipoClasificacion){ //Nuevo

		if(!empty($correo)){
			$this->db->where("correo", $correo);
		}
		
		$this->db->where("clasificacion", $tipoClasificacion);
		$this->db->order_by("id", "desc");
		$query = $this->db->get("cal_events_json");

		return $query->num_rows() > 0 ? $query->result() : array();
	}
	//-----------------------------------------------------------------------------------------------
	function getDataEvent($idCal){ //Nuevo Dennis [2021-08-23]

		$this->db->from("cal_events_json a");
		$this->db->join("invitados_eventos b", "a.id_cal = b.id_evento", "left");
		$this->db->where("a.id_cal", $idCal);
		$this->db->group_by("b.id_invitado");
		$query = $this->db->get();

		return $query->num_rows() > 0 ? $query->result() : array();
	}
	//-------------------------------------------------------------------------------------------------------
	function insertaRegistro($values, $table, $returnType){ //Nuevo Dennis [2021-08-23]

		$return = false;

		$this->db->insert($table, $values);
		$idResult = $this->db->insert_id();
		...
		return $returnType == 1 ? $return : $idResult;
	}
	//---------------------------
	function updateCalendarEvent($arrayEvent){ //Nuevo Dennis [2021-08-23]

		$this->db->where("id_cal", $arrayEvent["id_cal"]);
		$this->db->update("cal_events_json", $arrayEvent);

	}
	//---------------------------
	function getDataGuestByEmail($email, $type, $event){ //Nuevo Dennis [2021-08-23]

		$condition = $type == "interno" ? "correo_lectronico" : "correo_externo";
		$table = $type == "interno" ? "invitados_eventos" : "catalog_correos_externos";

		$this->db->where("id_evento", $event);
		$this->db->where($condition, $email);
	...
	function getEventsNotification(){
		...
		$this->db->limit(10);
		$query = $this->db->get("calendar_event_notification");

		return $query->num_rows() > 0 ? $query->result() : array();
	}
	//----------------------------------------------------------
	function updateNotificationStatus($id, $sets){

		$this->db->where("id", $id);
		$query = $this->db->update("calendar_event_notification", $sets);

		return $query;
	}
	//----------------------------------------------------------
	function getEventsOfGuests(){ //Nuevo Dennis [2022-01-24]
	...
	//----------------------------------------------------------
	function getTrainingOrganizers(){ //Nuevo Dennis [2022-01-27]

		$query = $this->db->distinct()
				->select("correo")
				->where("clasificacion", "capacitacion")
				->get("cal_events_json");
