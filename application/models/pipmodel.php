...
class pipmodel extends CI_Model
{
    function getUsers()
    {
        $tipos = $this->db->query('select idPersona as id, name_complete from users where activated=1 and banned=0 and name_complete <>"SNOMBRE"');
        return $tipos->result_array();
    }

    function getDataPIP($idU, $idPerido)
    {
        $tipos = $this->db->query('select * from project_implementation_plan pp  where evaluacion_periodo_id="' . $idPerido . '" and empleado_id="' . $idU . '"');
        return $tipos->result_array();
    }
    function getTaks($idU, $idPerido)
    {
        ...
        pp.evaluacion_periodo_id="' . $idPerido . '" and pp.empleado_id="' . $idU . '"');
        return $tipos->result_array();
    }
    ...
    function getSeguimientoByEmpleado2($id,$puesto){
        $year=date('Y');
        $res=$this->db->query('select pip.id,pip.empleado_id,u.name_complete nombre,ep.titulo,pip.created,pip.estatus
        from project_implementation_plan_manager pipm
        inner join project_implementation_plan pip on pipm.id_PIP=pip.id
        inner join users u on pip.empleado_id=u.idPersona
        left join evaluacion_periodos ep on pip.evaluacion_periodo_id=ep.id
        where (pipm.id_manager='.$puesto.' or pip.empleado_id='.$id.') and year(pip.created)='.$year.' group by pipm.id_PIP order by pip.id desc;');
        return $res->result_array();
    }
    ...
    function getAllInfoMonitoreoTable()
    {
        ...
        where ep.id=pi.evaluacion_periodo_id and p.idPersona=u.idPersona and p.idPersonaPuesto=pp.idPuesto and p.idPersona=pi.empleado_id');
        return $tipos->result_array();
    }
    ...
    function getUserByPuesto($puesto){
        $query = $this->db
    ...
    function getSeguimientoByEmpleadoByDate($id,$puesto,$year){
        $res=$this->db->query('select pip.id,pip.empleado_id,u.name_complete nombre,ep.titulo,pip.created from project_implementation_plan_manager pipm
        inner join project_implementation_plan pip on pipm.id_PIP=pip.id
         inner join users u on pip.empleado_id=u.idPersona
        left join evaluacion_periodos ep on pip.evaluacion_periodo_id=ep.id
        where pip.estatus="ACTIVO" and (pipm.id_manager='.$puesto.' or pip.empleado_id='.$id.') and year(pip.created)='.$year.' group by pipm.id_PIP order by pip.id desc;');
        return $res->result_array();
    }
    ...
    function deleteAllPIP($id){
        ...
        $this->db->where('project_imp_plan_id', $id);
        $this->db->delete('project_imp_plan_id');
    }
}
