...
class autos_model extends CI_Model
{
	...
	function getAllAutos($id_estatus, $year, $month = null, $agente = null)
	{
		...
		YEAR(sr.inicio_ajuste) ano_filtro, tramite.fecha_fin tram_fin", false)
			...
			//->join("(SELECT id_siniestro, MAX(id) as max_id FROM siniestro_tramite GROUP BY id_siniestro) ultimo_tramite", "sr.id = ultimo_tramite.id_siniestro", "left")
			//->join("siniestro_tramite tramite", "tramite.id = ultimo_tramite.max_id", "left")
			->join("siniestro_tramite tramite", "sr.ultimo_tramite=tramite.id", "left")
		...
		$obj = $this->db->order_by('sr.id', 'DESC');
		//$obj=$this->db->limit(10,0);
	...
	function getAllAutosV2($Post)
	{
		...
		YEAR(sr.inicio_ajuste) ano_filtro, tramite.fecha_fin tram_fin", false)
			...
			->join("siniestro_estatus_tramites setr", "tramite.estatus=setr.id", "left");
		if ($Post["Search"] != '') {
			$this->db->like('st.nombre', $Post["Search"]);
			$this->db->or_like('sc.nombre', $Post["Search"]);
			$this->db->or_like('staa.nombre', $Post["Search"]);
			$this->db->or_like('sr.ajustador_nombre', $Post["Search"]);
			$this->db->or_like('sr.asegurado_nombre', $Post["Search"]);
			$this->db->or_like('sr.siniestro_id', $Post["Search"]);
			$this->db->or_like('sr.poliza', $Post["Search"]);
			//$this->db->or_like('causa_nombre', $Post["Search"]);
			//$this->db->or_like('causa_nombre', $Post["Search"]);
		}
		if ($Post["Year"] != '') {
			$this->db->where("year(sr.inicio_ajuste)", $Post["Year"]);
		}
		if ($Post["FInicio"] != '') {
			$this->db->where("sr.inicio_ajuste >=", $Post["FInicio"]);
		}
		if ($Post["FFin"] != '') {
			$this->db->where("sr.inicio_ajuste <=", $Post["FFin"]);
		}
		if (count($Post["Evento"]) > 0) {
			$this->db->where_in("sr.tipo_siniestro_id", $Post["Evento"]);
		}
		if (count($Post["Aseguradora"]) > 0) {
			$this->db->where_in("sr.aseguradora_id", $Post["Aseguradora"]);
		}
		if ($Post["TTramite"] != '') {
			$this->db->where("tramite.tipo_tramite", $Post["TTramite"]);
		}
		if ($Post["ESiniestro"] != '') {
			$this->db->where("sr.siniestro_estatus", $Post["ESiniestro"]);
		}
		if ($Post["ETramite"] != '') {
			if ($Post["ETramite"] == "NULL") {
				$this->db->where('setr.nombre IS NULL');
			} else {
				$this->db->where('setr.nombre', $Post["ETramite"]);
			}
			//$this->db->like('setr.nombre', $Post["ETramite"]);
		}
		/* if ($Post["TTramite"] != ''){
			$this->db->wherwhere_in("tramite.tipo_tramite", $Post["TTramite"]);
		} */
		$obj = $this->db->order_by('sr.id', 'DESC');
		$rowCount = clone ($obj);
		$obj = $this->db->limit(10, $Post["Offset"]);
		$obj = $this->db->get('siniestro_reportes sr');
		$row = [];
		$rowCount = $rowCount->get('siniestro_reportes sr')->num_rows();
		//$rowCount = $this->db->where("sr.tipo_r", 'A')->get('siniestro_reportes sr')->num_rows();
		$row = $obj->result_array();
		//return $obj->result_array();
		foreach ($row as $key => $value) {
			$Sestatus = json_decode($value["json_tram"], true);
		...
		//echo "La consulta tardó: ".$execution_time." segundos";
		//return $row;
		return array("Data" => $row, "Count" => $rowCount);
	}
