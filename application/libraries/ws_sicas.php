...
class Ws_sicas
{
    ...
    function getDatosSICAS_old($body, $header = null, $url = "", $format = "XML")
    {
        ...
        if ($err) {
            ...
            //$test = $this->jsonToXmlSimplified($response);
            ...
            //echo "La consulta tardó: " . $execution_time . " segundos";
            return $returnedResponse;
        }
    }

    function getDatosSICAS($body, $header = null, $url = "", $format = "XML") //esto es el nuevo metodo optimizado
    {
        $start_time = microtime(true);
        $xml = '<ProcesarWS><oDataWS>';
        foreach ($body as $key => $value) {
    ...
    function convertResponseV2($respuesta)
    {
        $Reponse = null;
        $ReturnedResponse = $this->arrayToXmlString2($respuesta, 'DATAINFO');
        $text = preg_replace('/&(?!#?[a-z0-9]+;)/', '', $ReturnedResponse);
        //$text = str_replace(' & ', '', html_entity_decode((htmlspecialchars_decode($text))));
        $text = str_replace('&', '&amp;', html_entity_decode((htmlspecialchars_decode($text))));
        $resXmlConsumo = str_replace($this->DeleteXml, $this->ClearXml, $text);
        //$convert = htmlspecialchars_decode($resXmlConsumo);
        $Responde = simplexml_load_string($resXmlConsumo);
        if ($Responde == false) {
            ///echo $convert;
            //echo json_encode(array("RESPUESTA" => $resXmlConsumo));
        }
        //return $ReturnedResponse;
        return $Responde;
    }

    function convertResponseXML($respuesta)
    {
        $Reponse = null;
        //$ReturnedResponse = $this->arrayToXmlString2($respuesta, 'DATAINFO');
        //$text = preg_replace('/&(?!#?[a-z0-9]+;)/', '', $respuesta);
        $text = preg_replace('/&(?!#?[a-z0-9]+;)|[\\\\"]/', '', $respuesta);
        //$text = str_replace(' & ', '', html_entity_decode((htmlspecialchars_decode($text))));
        $text = str_replace('&', '&amp;', html_entity_decode((htmlspecialchars_decode($text))));
        $resXmlConsumo = str_replace($this->DeleteXml, $this->ClearXml, $text);
    ...
    function arrayToXmlString2($data, $rootElement)
    {
        $xmlParts = array();
        $xmlParts[] = "<$rootElement>";

        foreach ($data as $key => $value) {
            if (is_array($value)) {
                // Verificar si el array es indexado (numérico)
                if (array_keys($value) === range(0, count($value) - 1)) {
                    // Si es un array indexado, usamos la misma clave padre para cada elemento
                    foreach ($value as $subValue) {
                        $xmlParts[] = $this->arrayToXmlString2($subValue, $key);
                    }
                } else {
                    // Si es un array asociativo, se trata como un objeto normal
                    $xmlParts[] = $this->arrayToXmlString2($value, $key);
                }
            } else {
                $xmlParts[] = "<$key>" . htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . "</$key>";
            }
        }
    ...
    public function obtenerCobranzaEfectuadaPorFolio($folio, $vendedor, $nombreCLiente = null, $statusRecibo = null)
    {
        ...
        $datos['ItemForPage'] = '10000000';
        //$datos['InfoSort'] = 'VDatDocumentos.IDDocto';
        $datos['InfoSort'] = 'VDatDocumentos.IDDocto|VDatDocumentos.Periodo';
        $datos['IDRelation'] = '0';
    ...
    public function cobranzareporte($array, $paginacion = 0, $pagina = 0)
    {
        ...
        switch ($array['tipoReporte']) { //Status;0;0;0;0;
            ...
            case 'merida':
                //$datos['ConditionsAdd'] = 'Desde|Hasta;3;0;' . $array['fechaInicial'] . '|' . $array['fechaFinal'] . ';0;-1;VDatRecibos.' . $array['tipoFecha'] . ' ! Documento;0;1;1;1;DatDocumentos.TipoDocto ! Status;0;0;5;0;VDatRecibos.Status ! despacho;2;0;2|7;2|7;DatDocumentos.IDDespacho ! gerencia;1;0;9;0;1;DatDocumentos.IDGerencia ';
                $datos['ConditionsAdd'] = 'Desde|Hasta;3;0;' . $array['fechaInicial'] . '|' . $array['fechaFinal'] . ';0;-1;VDatRecibos.' . $array['tipoFecha'] . ' ! Documento;0;1;1;1;DatDocumentos.TipoDocto ! Status;0;0;0;0;VDatRecibos.Status';
                break;
            case 'grupocer':
                $datos['ConditionsAdd'] = 'Desde|Hasta;3;0;' . $array['fechaInicial'] . '|' . $array['fechaFinal'] . ';0;-1;VDatRecibos.' . $array['tipoFecha'] . ' ! Documento;0;1;1;1;DatDocumentos.TipoDocto ! Status;0;0;0;0;VDatRecibos.Status ! VendedorID;0;0;7;0;0;CatVendedores.IdVend';
                break;
            case 'grupoflotillas':
                $datos['ConditionsAdd'] = 'Desde|Hasta;3;0;' . $array['fechaInicial'] . '|' . $array['fechaFinal'] . ';0;-1;VDatRecibos.' . $array['tipoFecha'] . ' ! Documento;0;1;1;1;DatDocumentos.TipoDocto ! Status;0;0;0;0;VDatRecibos.Status ! VendedorID;0;0;7;0;0;CatVendedores.IdVend ! subramo;0;0;20;0;0;DatDocumentos.IDSRamo';
                break;
            case 'todos':
...
XML;
    ...
    //nuevo metodo para poder convertir las cosas
    function jsonToXmlSimplified($jsonString)
    {
        // Decodificar el JSON
        ...
        // Convertir la estructura principal
        $this->arrayToXmlSicas($data, $xml);
    ...
    function arrayToXmlSicas($array, &$xml)
    {
        foreach ($array as $key => $value) {
            // Manejar claves numéricas (como en arrays indexados)
            if (is_numeric($key)) {
                $key = 'item_' . $key;
            }

            // Reemplazar caracteres no válidos en nombres de nodos XML
            $key = preg_replace('/[^a-z0-9_\-]/i', '_', $key);

            if (is_array($value)) {
                if (!empty($value)) {
                    $subnode = $xml->addChild($key);
                    $this->arrayToXmlSicas($value, $subnode);
                } else {
                    $xml->addChild($key, '');
                }
            } else {
    ...
    function CrearArbol2($Array)
    {
        ...
        foreach ($Array as $key => $value) {
            ...
            if ($key === count($keys) - 1) continue;

            if ($Array[$keys[$key + 1]]->Tipo != 0) {
               $tree['children'][] = $object;
            }
            /* if ($Array[count($Array) < $key ? ($key + 1) : $key]->Tipo > 0) {
                $tree['children'][] = $object;
            } */
        }
        $tree["all"] = $Array;
