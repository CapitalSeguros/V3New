...
// DreNavegador
function DreNavegador($user_agent) {
     $navegadores = array(
		  'movil' => 'Mobile'
		  ,'movil' => 'mobile'
		  ,'movil' => 'android'
...
function SelectHoraCitaDreOnChange($idHora,$tipo){
	...
	$return = "<select name='hora".$tipo."' id='hora".$tipo."' onChange='JavaScript: cambiaDatosHoraFin();'>";
...
function SelectActividadDinamico($URL, $CLAVE, $TIPO, $grupo){
	switch($grupo){
		case "2":
			$filtradoGrupo = "
				And
	...
	$return.= "	<option value=''>-- Seleccione --</option>";
		...
		while($rowTiposActividad = mysql_fetch_assoc($resTiposActicidad)){
			$return.= " <option 
							value='".urlencode($rowTiposActividad['titulo'])."'>
...
function SelectRamoDinamico($Ramo, $grupo){
	...
	$return = "";
	$return.= "	&Aacute;rea: ";
	$return.= "<select name='Ramo' id='Ramo' onchange='JavaScrip: document.formSelectActividad.submit();'>";  //onChange='actualizaRamoDinamico();'
	$return.= "<option value=''>-- Seleccione --</option>";
		$sqlRamo = 	"
						Select * From
							`ramosconfigdre`
