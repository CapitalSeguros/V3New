...
{
	var $CI;
	var $uri_service	= "http://localhost/V3API/sicas/addData";
		...
		if(is_array($wsdata)){
			try {
				...
				if(isset($wsdata["XML"])){
					...
					//$this->oDataWS->DataXML = $dataConvert;
					$xmlData=$this->ConvertArrayToXML($wsdata['XML']);
					$this->oDataWS->DataXML = $xmlData;
				}
...
$vendedor=$this->CI->tank_auth->get_IDVend();
 if($vendedor>0){
	//$customConditionsAdd.=' ! VendedorID;2;0;'.$vendedor.';'.$vendedor.';DatDocumentos.IDVend';
}
		$data_body = array(
			//"Page" => $page,
			"ItemForPage" => $itemforPage,
