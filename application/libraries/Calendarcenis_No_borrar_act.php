...
//define("PATH_CONFIGURE" , __DIR__ . "/configure/client_secret_703168943524-uu1tip7pkcpkkj3gncaosjgs00rb6llt.apps.googleusercontent.com.json");
...
class calendarcenis {
	...
	public function __construct(){
		...
		//$this->calendarId="m5jgg42kud3i8q0k5q4ppf5avk@group.calendar.google.com";
	...
	public function agregar_evento($json_event){	
		...
		try{
			...
			//$event->setConferenceData($meetGoogle);
			...
			/*$array_conferencia=array(
				"id_evento"=>$this->oEventCreate->getId(),
				"url_evento"=>$Json_Event->urlZoom,
				"id_sesion"=>$Json_Event->idZoom,
				"psw_sesion"=>$Json_Event->pswZoom,
			); */
		...
		//	 fwrite($fp, print_r($event, TRUE));
          ...
          //         fwrite($fp, print_r($data_table, TRUE));
			...
			//$CI->calendar_model->insertaConferencia($array_conferencia);
	...
	*/
	...
	function notificacion_del_evento($datosCorreo){
		...
		/*$config=array(
			...
			"smtp_host"=>"mail.agentecapital.com",
			//"smtp_host"=>"174.136.52.204",
			"smtp_port"=>587,
			//"smtp_timeout"=>"10",
			//"smtp_user"=>"auxiliardesarrollo@agentecapital.com",
			"smtp_user"=> "envioinvitacion@agentecapital.com", //"invitacion@asesorescapital.com",
			"smtp_pass"=> "PSWAGINV2020!", //"Invitacion2020#",
			//"newline"=>"\r\n",
			"mailtype"=>"html",
			"wordwrap"=>TRUE
		...
		$CI->load->library("email", $config); //Cargamos la libreria de email.
				...
				if(isset($valor["email"])){
					...
					if(isset($consulta)){
						foreach($consulta as $datos){
							...
							$CI->email->from("envioinvitacion@agentecapital.com","AGENTE CAPITAL SEGUROS Y FIANZAS");
							//$CI->email->to($datos->correo_lectronico);
							$CI->email->to("auxiliardesarrollo@agentecapital.com");
							$CI->email->subject($subject);
							//$CI->email->cc($datosCorreo["correo"]);
							...
							//$CI->email->attach(str_replace("personalInterno",$datos->id_invitado,$datosCorreo["ics"]));

							$nameFile=$datosCorreo["ics"];
							$datosIcs=file_get_contents($nameFile);

							if(strpos($datosIcs,"personalInterno")) {
								$nuevoDato=str_replace("personalInterno",$datos->id_invitado,$datosIcs);
							} else{
								$nuevoDato=str_replace("r=".$datos->id_invitado,"r=".$datos->id_invitado,$datosIcs);
							}
							file_put_contents($nameFile,$nuevoDato);
							...
							foreach($datosCorreo["adjuntos"] as $arrayAdjuntos){
								...
								//$adjunto=$_SERVER["DOCUMENT_ROOT"]."/Capsys/www/V3/".$ruta.$arrayAdjuntos["title"];
								$adjunto=$_SERVER["DOCUMENT_ROOT"]."/V3/".$ruta.$arrayAdjuntos["title"];
				...
				if(isset($consulta)){
					foreach($consulta as $datos){
						...
						/*if($datos->email==$this->email){
							$datosInvitado["tipo_invitado"]="organizador";
						} else{
							$datosInvitado["tipo_invitado"]="pendiente";
						} */
						//$datosInvitado["tipo_evento"]="calendario";
	...
	function notificacion_de_actualizacion($datosCorreo, $title){
		...
		//Configuración Sendmail para servidor
		$config['protocol'] = 'sendmail';
		$config['mailpath'] = '/usr/sbin/sendmail';
		$config['charset'] = 'iso-8859-1';
		$config['wordwrap'] = TRUE;
				...
				if(isset($valor["email"])){
					...
					if(isset($consulta)){
						foreach($consulta as $datos){
							...
							$CI->email->from("invitacion@asesorescapital.com","AGENTE CAPITAL SEGUROS Y FIANZAS");
							//$CI->email->to($datos->correo_lectronico);
							$CI->email->to("auxiliardesarrollo@agentecapital.com");
							$CI->email->subject($subject);
							$CI->email->cc($datosCorreo["correo"]);
	...
	function creaIcsFile($datosCorreo){
		...
		//base_url()."accesoAEvento/habilitaEvento?q=".$datos["idEvento"]."&r=".$datos["invitado"]."&p=".$datos["TInvitado"]
		...
		ACTION:EMAIL
		DESCRIPTION:".$datosCorreo["descripcion"]."  ->  Acceso al evento: ".base_url()."accesoAEvento/habilitaEvento?q=".$datosCorreo["idEvento"]."&r=personalInterno&p=interno
		END:VEVENT
		END:VCALENDAR";
