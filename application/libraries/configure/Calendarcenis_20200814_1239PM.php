...
define("PATH_CONFIGURE" , __DIR__  . "/configure/client_secret_903153672620-k1b1d246acd78vajisujd5as78l616uh.apps.googleusercontent.com.json");


class calendarcenis extends CI_Controller {
	...
	public function __construct(){
		...
		//$this->load->model("calendar_model"); //[Dennis 2020-06-11]
	  	...
	  	if (file_exists($credentialsPath)) {
			...
			//$authCode = "4/2AExxodp0yQah9TqiV9LI2xDFCS2PGbyivOwYg1NXpaB_BHaKep3y0o";
			//$authCode="4/2AFrHlzNaVexxfS42Mq_JqZj3OlO2wKJikO_Zj1xEF_MegmWum_mc1o";
	...
	public function agregar_evento($json_event){	
		...
		try{
			...
			//$fp = fopen('resultadoJason.txt', 'a');fwrite($fp, print_r($json_event,TRUE));fclose($fp);
			...
			//[Dennis 2020-06-16]
			$nuevoTitulo="";
			$nuevoTitulo=str_replace("_",": ",$Json_Event->TituloEvento);
			//----------------------------------------------------------
			if (!in_array(array('email' => $this->email), $attendees,false)) {				
				...
				array_push($attendees,array('email' =>  $this->email,'organizer'=> true,'responseStatus' => 'accepted' )); //[Dennis 2020-07-02]:  comentado para prueba de recabado de invitados internos
				//array_push($attendees,array('email' =>  'auxiliardesarrollo@agentecapital.com','organizer'=> true,'responseStatus' => 'accepted' ));
			}
			...
			//$event->setSummary(html_entity_decode($Json_Event->TituloEvento)); //Comentado por Dennis 2020-06-12
			$event->setSummary(html_entity_decode($nuevoTitulo)); //[Dennis 2020-06-12]
			...
			//$event_optional = array("sendNotifications" => true , "supportsAttachments" => $supportAttach); //Comentado para prueba [Dennis 202-06-16]: envia mail (propiedad de Google Calender) a los correos seleccionados
			$event_optional = array("sendNotifications" => false , "supportsAttachments" => $supportAttach); //[Dennis 2020-06-16]
		...
		//	 fwrite($fp, print_r($event, TRUE));
          ...
          //         fwrite($fp, print_r($data_table, TRUE));
			...
			$this->creaIcsFile($datosCorreo);
			//--------------------------------------------------------------------------------------------------------------
			//[Dennis 2020-07-02]: inserccion de agentes internos al db.
			if(isset($invitadoInterno)){
				
				$this->registro_invitado($invitadoInterno, $this->oEventCreate->getId());
			}
			//---------------------------------------------------------------------------------------------------------------
			//[Dennis 2020-06-17]
			$this->notificacion_del_evento($datosCorreo);

			//$fp = fopen('resultadoJason.txt', 'w');fwrite($fp, print_r($this->oEventCreate, TRUE));fclose($fp);
			//---------------------------------------------------------------------------------------------------------------
	...
	*/
	public function eliminar_evento($json_event){
		...
		try{	
			...
			if($Json_Event->enviarNotificacion){
				$event_optional = array("sendNotifications" => true );//[Dennis 202-07-16]: comentado para no enviar notificación directo de la api.
			...
			//---------------------------------------------------------------------------------------------------------------
				...
				if($Json_Event->eliminarInstancias){
					...
					$this->oEventDelete = $this->service->events->delete($this->calendarId, $Json_Event->eventId);
				}else{
		...
		try{
			...
			*/
			...
			if(isset($invitadoInterno)){
				...
				$this->actualizacion_invitado($invitadoInterno,$externalAttendees, $this->oEventUpdate->getId());
			}
			//-------------------------------------------------------------------------------------------------------------
			//[Dennis 2020-07-13]
	...
	public function listar_evento(){
		...
		try{
			...
			while(true) {
			  ...
			  foreach ($events->getItems() as $event) {
				...
				/*foreach($eventoBD as $elementos){
					$data["clasificacion"]=$elementos->clasificacion;
				}*/
		...
		//$fp = fopen('resultadoJason.txt', 'w');fwrite($fp, print_r($datas, TRUE));fclose($fp);
	...
	*/
	...
	public function getCalendarEvent($json_event)
	{
		...
		try{			
			...
			//--------------------------------------------------------------------------------------------------
				/*$eventoBD=$CI->calendar_model->select_event($event->getId()); */
			...
			//$fp = fopen('resultadoJason.txt', 'w');fwrite($fp, print_r($event_json["externalGuests"], TRUE));fclose($fp);
	...
	*/
				...
				if(isset($valor["email"])){
					...
					if(isset($consulta)){
						foreach($consulta as $datos){
							...
							$CI->email->from($datosCorreo["correo"],"Aviso");
	...
	function registro_invitado($correos, $evento){
			...
			foreach($correos as $valor){
				...
				if(isset($consulta)){
					foreach($consulta as $datos){
						...
						$datosInvitado["organizacion"]="Agentes Capital";
						...
						$CI->calendar_model->inserta_invitados($datosInvitado);
					}
				} 
			} 
		}
	...
	function actualizacion_invitado($correos, $externos, $evento){
			...
			if($consulta!=true){
				...
				if(isset($datosAgente)){
					foreach($datosAgente as $datos){
						...
						$CI->calendar_model->inserta_invitados($updateAttendes);
					}
				} 
			}
		...
		if(isset($oka)){
			foreach($correoE as $valor){
				if(!in_array($valor->correo_externo, $oka)){
					...
					$_infoE["datos"]["nombre"]="usuario ".$valor->correo_externo;
	...
	function creaIcsFile($datosCorreo){
		...
		//$this->oGetEvent=$this->service->events->get("primary", $datosCorreo["idEvento"]);
		...
		if(!file_exists($direccion."/".$datosCorreo["idEvento"].".ics")){
			//mkdir($direccion."/".$datosCorreo["idEvento"].".ics");
