...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $login_by_username, $login_by_email, $view)
	{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$query = $this->ci->users->db->query("SELECT idPersonaPuesto FROM persona WHERE idPersona = $user->idPersona");
						...
						$this->ci->session->set_userdata(array(
								...
								'usermail'			=> $user->email,
								'usernamecomplete'	=> $user->name_complete,
								'userprofile'		=> $user->profile,
								'uservendedor'		=> $user->idVendedor,
								'IDVend'			=> $user->IDVend,
								'IDVendNS'			=> $user->IDVendNS,
								'profile'			=> $user->profile,
								'IDUserSICAS'		=> $user->IDUser,
								'UserSICAS'			=> $user->email,
								'PassSICAS'			=> $password,
								'status'			=> ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
								'calendar_sync'		=> $user->calendar_sync,
								'idPersona'         => $user->idPersona,
							"idPersonaPuesto"   => $query->num_rows() > 0 ? $query->row()->idPersonaPuesto : 0,
								'view'				=> $view
          			...
          			$insert['idPersona']=$user->idPersona;
		        	$insert['userEmail']=$user->email;
			        $this->ci->users->db->insert('loginbitacora',$insert);
	 ...
	 * @return	string
	 */
	function get_calendarSync()
	{
		return $this->ci->session->userdata('calendar_sync');
	}
	 ...
	 * @return	string
	 */
	function get_idTipoUser()
	{
		return $this->ci->session->userdata('idTipoUser');
	}
	...
	/**
	* Get usernamecomplete
	*
	* @return	string
	*/
	function get_usernamecomplete()
	{
		return $this->ci->session->userdata('usernamecomplete');
	}
	...
	{
		return $this->ci->session->userdata('usermail');
	}
	 ...
	 * @return	string
	 */
	function get_uservendedor()
	{
		return $this->ci->session->userdata('uservendedor');
	}
	...
	{
		return $this->ci->session->userdata('userprofile');
	}
	...
	*/
	function get_IDVend()
	{
		return $this->ci->session->userdata('IDVend');
	}

	/*DEVUELVE idPersona*/
	function get_idPersona(){
		return $this->ci->session->userdata('idPersona');
	}

	/*DEVUELVE idPersonaPuesto*/
	function get_idPersonaPuesto()
	{
		return $this->ci->session->userdata('idPersonaPuesto');
	}
	/**
	 * Get IDVendNS
	*/
	function get_IDVendNS()
	{
		return $this->ci->session->userdata('IDVendNS');
	}
	 ...
	 * @return	string
	 */
	function get_IDUserSICAS()
	{
		return $this->ci->session->userdata('IDUserSICAS');
	}
	 ...
	 * @return	string
	 */
	function get_UserSICAS()
	{
		//return $this->ci->session->userdata('UserSICAS');
		if(
			$this->ci->session->userdata('userprofile') == 1 //
			||
			$this->ci->session->userdata('userprofile') == 2 //
			||
			$this->ci->session->userdata('userprofile') == 5 //
			||
			$this->ci->session->userdata('userprofile') == 6 //
			||
			$this->ci->session->userdata('idPersona') == 1102
			||
			$this->ci->session->userdata('usermail')=="EJECUTIVOCORPORATIVO@AGENTECAPITAL.COM"
			||
			$this->ci->session->userdata('usermail')=="SOPORTEGMM@AGENTECAPITAL.COM"
		  ){
			return "AGENTE@CAPSYS.COM.MX";
		} else {
			return $this->ci->session->userdata('UserSICAS');
		}
	}
	 ...
	 * @return	string
	 */
	function get_PassSICAS()
	{
		//return $this->ci->session->userdata('PassSICAS');
		if(
			$this->ci->session->userdata('userprofile') == 1 //
			||
			$this->ci->session->userdata('userprofile') == 2 //
	...
	function get_View(){
		return $this->ci->session->userdata('view');
	}
	 ...
	 * @param	int //SysJjHe
	 * @param	int //SysJjHe
	 * @param	string //SysJjHe
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation,$tipoUser,$tipoUsertxt ,$idVendedor, $IDGrupo, $profile,$sucursal,$canal)
	{
		$password = strtoupper ($password); //paso a mayusculas
	...
	function change_password_user($idUser, $new_pass)
	{
		if (!is_null($user = $this->ci->users->get_user_by_id($idUser, TRUE))) {
			...
			$hasher = new PasswordHash(
				...
				// Replace old password with new one
				$this->ci->users->change_password($idUser, $hashed_password);
				return TRUE;
		}
		return FALSE;
	}
	...
	function ValidacionLogeo($login, $password, $login_by_username, $login_by_email){
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {
							$resultado->data=[];							// fail - not activated
							$resultado->error='Usuario no activado';

						} else {												// success
							$resultado->data=$dataResult=array(
								'user_id'			=> $user->id,
								// 'idTipoUser'		=> $user->idTipoUser,
								'username'			=> $user->username,
								'usermail'			=> $user->email,
					...
					/* $this->increase_login_attempt($login); */
					$resultado->data=[];
					$resultado->error ='Contrase�a incorrecta';
				}
			} else {															// fail - wrong login
				/* $this->increase_login_attempt($login); */
				$resultado->data=[];
				$resultado->error='Usuario incorrecto';
		}
	}
		return $resultado;
        //print_r($user);
